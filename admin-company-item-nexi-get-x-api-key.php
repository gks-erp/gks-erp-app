...
$my_page_title=gks_lang('Λήψη X Api Key από').' NEXI';
db_open();
stat_record();
...
if ($nexi_username=='' or $nexi_password=='' or $nexi_authorization_code=='') {
...
$url=GKS_NEXI_COM_API.'/authorization/redeem/';
...
$mypost=array();
$mypost['Type']='webecr';
$mypost['Code']=$nexi_authorization_code;
...
if ($myresponse=='') {
  debug_mail(false,'nexi error connection',gks_lang('Σφάλμα κατά την επικοινωνία με το').': '.GKS_NEXI_COM_API);
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Σφάλμα κατά την επικοινωνία με το').': '.GKS_NEXI_COM_API));
  echo json_encode($return); die();}
...
if (isset($mydata['Id']) and isset($mydata['Type']) and $mydata['Type']=='webecr') {
  //debug_mail(false,'nexi OK response',$myresponse);
  $sql="update gks_company set nexi_x_api_key='".$db_link->escape_string($mydata['Id'])."' where id_company=".$id." limit 1";
  $result = $db_link->query($sql);        
  if (!$result) {
    debug_mail(false,'error sql',$sql);
    $return = array('success' => false, 'message' => base64_encode('sql error'));
