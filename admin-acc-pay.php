...
$my_page_title=gks_lang('Πληρωμές/Εισπράξεις');
...
//echo '<pre>';echo gks_acc_pay_cancel_create(10050);die();
...
include_once('admin-acc-pay_filters.php');
...
?>
<link href="css/admin-acc-pay.css?v=<?php echo $gks_cache_version;?>" rel="stylesheet">
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-acc-pay-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέας πληρωμής');?></a>
      <?php
      $sql_export_excel="SELECT id_export_excel, export_excel_descr FROM gks_export_excel WHERE export_excel_object='gks_acc_pay' and export_excel_disable=0 ORDER BY export_excel_descr;";
      ...
      <div class="btn-group" >
        ...
        <div class="dropdown-menu" style="background-color: #fafcff;box-shadow: 0px 4px 24px 12px rgb(0 0 0 / 20%), 0px 6px 20px 0px rgb(0 0 0 / 19%);">
          ...
          foreach ($export_excel as $itemee) {
            echo '<a class="dropdown-item" href="admin-acc-pay-export-excel.php?'.$_SERVER['QUERY_STRING'].'&id_export_excel='.$itemee['id_export_excel'].'" download>'.$itemee['export_excel_descr'].'</a>';
          }?>
        </div>  
      </div> 
      <?php } ?>
  ...
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>">
    ...
    <td nowrap class="mytdcm">
      <table cellpadding=0 cellspacing=0 class="gks_tb1">
        <tr class="gks_tr1">
          <td class="gks_ttd1" colspan="2"><?php echo $row['id_acc_pay'];?></td>
        </tr>  
        <tr class="gks_tr1">
          <td class="gks_ttd2"><a href="admin-acc-pay-item.php?id=<?php echo $row['id_acc_pay'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <?php if ($perm_gks_acc_pay_delete) {?>
          <td class="gks_ttd3" ><i class="fas fa-trash-alt deleterow" data-id="<?php echo $row['id_acc_pay'];?>" data-model="gks_acc_pay"></i></td>
          <?php } ?>
        </tr>  
     </table>
    </td>
    ...
    <td nowrap class="mytdcm"><span class="acc_pay_state_<?php echo $row['pay_state'];?>"><?php echo getAccPayStateDescr($row['pay_state']);?></span></td>
    <td nowrap class="mytdcm"><?php echo showDate(strtotime($row['pay_date']), 'd/m/Y\<\b\r\>H:i:s', 1);?></td>   
<?php if (count($user_companys)>1) {?>
...
<?php } ?>
    ...
    <td nowrap class="mytdcm"><?php if ($row['pay_acc_number_int']<>0) echo $row['pay_acc_number_int'];?></td>
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_acc_pay','edit',  0);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_acc_pay','add',   0);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_acc_pay','delete',0);?>;
...
jQuery(document).ready(function($) {
  ...
  $('.filterselectbox').on('change', function() {
      ...
      if (v==-2) { //is_custom_date
        if (sname == 'fdate_add' || sname=='fprint_date' || gks_custom_filters_date_elems.includes(sname)) {
