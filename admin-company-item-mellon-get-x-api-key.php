...
$my_page_title=gks_lang('Λήψη X Api Key από').' Mellon';
db_open();
stat_record();
...
if ($mellon_username=='' or $mellon_password=='' or $mellon_authorization_code=='') {
...
$url=GKS_MELLONGROUP_COM_API.'/api/v2.1/authorization/redeem/';
...
$mypost=array();
$mypost['Type']='webecr'; //$access_token;
$mypost['Code']=$mellon_authorization_code;
...
if ($myresponse=='') {
  debug_mail(false,'mellon error connection',gks_lang('Σφάλμα κατά την επικοινωνία με το').': '.GKS_MELLONGROUP_COM_API);
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Σφάλμα κατά την επικοινωνία με το').': '.GKS_MELLONGROUP_COM_API));
  echo json_encode($return); die();}
...
if (isset($mydata['Id']) and isset($mydata['Type']) and $mydata['Type']=='webecr') {
  //debug_mail(false,'mellon OK response',$myresponse);
  $sql="update gks_company set mellon_x_api_key='".$db_link->escape_string($mydata['Id'])."' where id_company=".$id." limit 1";
  $result = $db_link->query($sql);        
  if (!$result) {
    debug_mail(false,'error sql',$sql);
    $return = array('success' => false, 'message' => base64_encode('sql error'));
