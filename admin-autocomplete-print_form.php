...
$my_page_title=gks_lang('Αυτόματη συμπλήρωση φόρμας εκτύπωσης');
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_print_forms','autocomplete',0);
...
".($notid<=0 ? '' : "id_print_form<>".$notid." and ")." 
...
foreach ($term_array as $value) {
  ...
  $mywhere.=" (print_form_descr like '%".$db_link->escape_string($value)."%' or print_form_descr like '%".$db_link->escape_string($value_en)."%') and ";
} 
...
while ($row = $result->fetch_assoc()) {
  $fount_count++;
  $out[] = array('id' => $row['id_print_form'], 'value' => $row['print_form_descr']);
}
