...
if ($id==-1) {
  ...
  $row['myip'] ='';
  $row['id_aade_katigoria_fpa_ejeresi']=-1;
  $row['aade_katigoria_fpa_ejeresi_code']='';
  $row['aade_katigoria_fpa_ejeresi_descr'] ='';
  $row['sortorder'] =1000;
  $row['aade_disable'] =0;
  $row['fpa_ejeresi_peppol_code']='';

  $my_page_title=gks_lang('Αιτία Εξαίρεσης ΦΠΑ');

} else {
 $sql ="SELECT gks_aade_katigoria_fpa_ejeresi.*,
 ".GKS_WP_TABLE_PREFIX."users_add.gks_nickname as gks_nickname_add, ".GKS_WP_TABLE_PREFIX."users_edit.gks_nickname as gks_nickname_edit
  FROM gks_aade_katigoria_fpa_ejeresi
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_add on gks_aade_katigoria_fpa_ejeresi.user_id_add = ".GKS_WP_TABLE_PREFIX."users_add.ID
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_edit on gks_aade_katigoria_fpa_ejeresi.user_id_edit = ".GKS_WP_TABLE_PREFIX."users_edit.ID
  where id_aade_katigoria_fpa_ejeresi = ".$id;
  $result = $db_link->query($sql);        
  if (!$result) debug_mail(false,'error sql',$sql);
  if (!$result) die('sql error');
  if ($result->num_rows!=1) die('record not found'); 
  $row = $result->fetch_assoc();
  $my_page_title=gks_lang('Αιτία Εξαίρεσης ΦΠΑ').': '.$row['aade_katigoria_fpa_ejeresi_descr'];
  $object_title=$row['aade_katigoria_fpa_ejeresi_descr'];
}

stat_record();
$nav_active_array=array('manage','manage_aade','manage_aade_katigoria_fpa_ejeresi');



$lang_data_obj=gks_lang_data_obj_prepare('gks_aade_katigoria_fpa_ejeresi','default');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    <div class="col-sm-12" style="text-align:center">
      <?php if ($id > 0) {?>
        <h3><?php echo gks_lang('Αιτία Εξαίρεσης ΦΠΑ');?>: <span class="gks_object_badge_secondary">#<?php echo $id;?></span> <?php echo $object_title;?></h3>
      <?php } else { ?>
        <h3><?php echo gks_lang('Αιτία Εξαίρεσης ΦΠΑ');?>: <span class="gks_object_badge_secondary">#<?php echo gks_lang('Νέα');?></span></h3>
      <?php }?>
    </div>
  </div>
</div>
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>  
          ...
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="aade_katigoria_fpa_ejeresi_descr"><?php echo gks_lang('Περιγραφή');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="aade_katigoria_fpa_ejeresi_descr"  value="<?php echo htmlspecialchars_gks($row['aade_katigoria_fpa_ejeresi_descr']);?>">
            </div>
          </div>
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('aade_katigoria_fpa_ejeresi_descr'));
          ?>
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="aade_katigoria_fpa_ejeresi_code"><?php echo gks_lang('Κωδικός ΑΑΔΕ');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="aade_katigoria_fpa_ejeresi_code"  value="<?php echo htmlspecialchars_gks($row['aade_katigoria_fpa_ejeresi_code']);?>">
            </div>
          </div>
          ...
          <div class="form-group row">
            <label for="aade_disable" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Ενεργή');?>:</label>
            <div class="col-md-8">
          ...
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="fpa_ejeresi_peppol_code"><?php echo gks_lang('Peppol');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="fpa_ejeresi_peppol_code"  value="<?php echo htmlspecialchars_gks($row['fpa_ejeresi_peppol_code']);?>">
            </div>
          </div>
...
<div class="container-fluid" id="gks_rsrv_f">
  <div class="form-group1 row">
    <div class="col-md-12 text-center mt-2">
              ...
              <button type="button" class="btn btn-danger deleterowbtn" data-id="<?php echo $row['id_aade_katigoria_fpa_ejeresi'];?>" data-model="gks_aade_katigoria_fpa_ejeresi" data-backurl="admin-aade-katigoria-fpa-ejeresi.php"><?php echo gks_lang('Διαγραφή');?></button>
    </div>            
  </div>            
</div>            
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>       
          ...
          <div class="row">
            ...
            <div class="col-sm-8"><span class="form-control-plaintext form-control-sm"><?php if ($row['id_aade_katigoria_fpa_ejeresi']>0) echo $row['id_aade_katigoria_fpa_ejeresi'];?></span></div>
          </div>
          <div class="row">
...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    ...
    datasend='';
    datasend+='&aade_katigoria_fpa_ejeresi_code='  +  encodeURIComponent($("#mypostform #aade_katigoria_fpa_ejeresi_code").val().trim());
    datasend+='&aade_katigoria_fpa_ejeresi_descr='  +  encodeURIComponent($.base64.encode($("#mypostform #aade_katigoria_fpa_ejeresi_descr").val().trim()));
    ...
    datasend+='&fpa_ejeresi_peppol_code='  +  encodeURIComponent($.base64.encode($("#mypostform #fpa_ejeresi_peppol_code").val().trim()));
    ...
    $.ajax({
			url: '/my/admin-aade-katigoria-fpa-ejeresi-item-exec.php?id=' + <?php echo $id;?>,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
