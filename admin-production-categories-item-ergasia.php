...
if ($cat_id<=0) {
  debug_mail(false,'the cat_id is not set','');
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Δεν έχει ορισθεί η κατηγορία')));
  echo json_encode($return); die();}

$my_page_title=gks_lang('Προσθήκη εργασίας σε κατηγορία');
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_eshop_products_categories','edit',$cat_id);
...
$sql="SELECT * FROM gks_eshop_products_categories where id_product_category = ".$cat_id;
...
if ($result->num_rows == 0) {
  debug_mail(false,'category not found');
...
if ($result->num_rows == 0) {
  debug_mail(false,'ergasia_id not found');
...
$sql="SELECT * FROM gks_production_ergasies_eidoscat where production_ergasia_id = ".$ergasia_id." and cateidos_id=".$cat_id;
$result = $db_link->query($sql);
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode('sql error'));
...
if ($result->num_rows != 0) {
  debug_mail(false,'production_ergasia_id - cateidos_id exists');
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Η εργασία - κατηγορία υπάρχει ήδη').'<br>'.gks_lang('Ανανεώστε την σελίδα')));
  echo json_encode($return); die();}  


$sql="insert into gks_production_ergasies_eidoscat (cateidos_id,production_ergasia_id,
...
$id_production_ergasies_eidoscat = $db_link->insert_id; 


$row_html='';
if (isset($_POST['from']) and $_POST['from']=='cat') {
$sql_list = "SELECT gks_production_ergasies_eidoscat.*, gks_production_ergasies.production_ergasia_descr
...
WHERE id_production_ergasies_eidoscat=".$id_production_ergasies_eidoscat;
...
$row_html=
'<tr class="ergasies_tr_new" data-id="'.$row_list['id_production_ergasies_eidoscat'].'">'.
  '<th scope="row" nowrap align="right" class="ergasies_aa">*</td>'.
  '<td nowrap align="center">'.
    '<img src="img/delete.png" border="0" width="16" class="deleterow" data-deleteafter="gks_fnc_ergasies_delete_after|'.$row_list['id_production_ergasies_eidoscat'].'" data-id="'.$row_list['id_production_ergasies_eidoscat'].'" data-model="gks_production_ergasies_eidoscat">'.
  '</td>'.
  ...
  '<td nowrap>'.$row_list['production_ergasia_descr'].'</a></td>'.
