...
if ($id==-1) {
  ...
  $row['myip'] ='';
  $row['id_aade_skopos_diakinisis']=-1;
  $row['aade_skopos_diakinisis_code']='';
  $row['aade_skopos_diakinisis_descr'] ='';
  $row['sortorder'] =1000;
  $row['aade_disable'] =0;

  $my_page_title=gks_lang('Σκοπός Διακίνησης');

} else {
 $sql ="SELECT gks_aade_skopos_diakinisis.*,
 ".GKS_WP_TABLE_PREFIX."users_add.gks_nickname as gks_nickname_add, ".GKS_WP_TABLE_PREFIX."users_edit.gks_nickname as gks_nickname_edit
  FROM gks_aade_skopos_diakinisis
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_add on gks_aade_skopos_diakinisis.user_id_add = ".GKS_WP_TABLE_PREFIX."users_add.ID
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_edit on gks_aade_skopos_diakinisis.user_id_edit = ".GKS_WP_TABLE_PREFIX."users_edit.ID
  where id_aade_skopos_diakinisis = ".$id;
  $result = $db_link->query($sql);        
  if (!$result) debug_mail(false,'error sql',$sql);
  if (!$result) die('sql error');
  if ($result->num_rows!=1) die('record not found'); 
  $row = $result->fetch_assoc();
  $my_page_title=gks_lang('Σκοπός Διακίνησης').': '.$row['aade_skopos_diakinisis_descr'];
  $object_title=$row['aade_skopos_diakinisis_descr'];
}

stat_record();
$nav_active_array=array('manage','manage_aade','manage_aade_skopos_diakinisis');



$lang_data_obj=gks_lang_data_obj_prepare('gks_aade_skopos_diakinisis','default');
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>  
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="aade_skopos_diakinisis_descr"><?php echo gks_lang('Περιγραφή');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="aade_skopos_diakinisis_descr"  value="<?php echo htmlspecialchars_gks($row['aade_skopos_diakinisis_descr']);?>">
            </div>
          </div>
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('aade_skopos_diakinisis_descr'));
          ?>
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="aade_skopos_diakinisis_code"><?php echo gks_lang('Κωδικός ΑΑΔΕ');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="aade_skopos_diakinisis_code"  value="<?php echo htmlspecialchars_gks($row['aade_skopos_diakinisis_code']);?>">
            </div>
          </div>
...
<div class="container-fluid" id="gks_rsrv_f">
  <div class="form-group1 row">
    <div class="col-md-12 text-center mt-2">
              ...
              <button type="button" class="btn btn-danger deleterowbtn" data-id="<?php echo $row['id_aade_skopos_diakinisis'];?>" data-model="gks_aade_skopos_diakinisis" data-backurl="admin-aade-skopos-diakinisis.php"><?php echo gks_lang('Διαγραφή');?></button>
    </div>            
  </div>            
</div>            
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>       
          ...
          <div class="row">
            ...
            <div class="col-sm-8"><span class="form-control-plaintext form-control-sm"><?php if ($row['id_aade_skopos_diakinisis']>0) echo $row['id_aade_skopos_diakinisis'];?></span></div>
          </div>
          <div class="row">
...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    ...
    datasend='';
    datasend+='&aade_skopos_diakinisis_code='  +  encodeURIComponent($("#mypostform #aade_skopos_diakinisis_code").val().trim());
    datasend+='&aade_skopos_diakinisis_descr='  +  encodeURIComponent($.base64.encode($("#mypostform #aade_skopos_diakinisis_descr").val().trim()));
    ...
    $.ajax({
			url: '/my/admin-aade-skopos-diakinisis-item-exec.php?id=' + <?php echo $id;?>,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
