...
$nav_active_array=array('transfer','transfer_poi_type');
db_open();
$id=0;if (isset($_GET['id'])) $id=intval($_GET['id']);
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_poi_type',($id==-1 ? 'add' : 'view'),$id);
if ($perm_ret['success']==false) {header('Location: /my/admin-deny.php?message='.rawurlencode($perm_ret['message'])); die();}




$gks_custom_prepare = gks_custom_table_item_prepare('gks_poi_type',['from'=>'item']);
...
if ($id==-1) {
  ...
  $row['poi_type_descr']='';
  $row['poi_type_html_icon']='';
  $row['poi_type_comments']='';
  $row['poi_type_disable']=0;
  
  
  $my_page_title=gks_lang('Νέος Τύπος Σημείων Ενδιαφέροντος');
} else {

  $sql ="SELECT gks_poi_type.*,
  ".GKS_WP_TABLE_PREFIX."users_add.gks_nickname AS gks_nickname_add, ".GKS_WP_TABLE_PREFIX."users_edit.gks_nickname AS gks_nickname_edit
  FROM (gks_poi_type 
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users AS ".GKS_WP_TABLE_PREFIX."users_add ON gks_poi_type.user_id_add = ".GKS_WP_TABLE_PREFIX."users_add.ID) 
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users AS ".GKS_WP_TABLE_PREFIX."users_edit ON gks_poi_type.user_id_edit = ".GKS_WP_TABLE_PREFIX."users_edit.ID
  where id_poi_type = ".$id;
  ...
  $my_page_title=gks_lang('Τύπος Σημείων Ενδιαφέροντος').': '.$row['poi_type_descr'];
  $object_title=$row['poi_type_descr'];
}
...
$lang_data_obj=gks_lang_data_obj_prepare('gks_poi_type','default');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    <div class="col-md-12" style="text-align:center">
      <?php if ($id > 0) {?>
        <h3><?php echo gks_lang('Τύπος Σημείων Ενδιαφέροντος');?>: <span class="gks_object_badge_secondary">#<?php echo $id;?></span> <?php echo $object_title;?></h3>
      <?php } else { ?>
        <h3><?php echo gks_lang('Τύπος Σημείων Ενδιαφέροντος');?>: <span class="gks_object_badge_secondary">#<?php echo gks_lang('Νέος');?></span></h3>
      <?php }?>
    </div>
  </div>
</div>
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>         
          ...
          <div class="form-group row">
            <label for="poi_type_descr" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Περιγραφή');?>:</label>
            <div class="col-md-8">
              <input id="poi_type_descr" type="text" class="form-control form-control-sm myneedsave" value="<?php echo htmlspecialchars_gks($row['poi_type_descr']);?>">
            </div>
          </div>
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('poi_type_descr'));
          ?>
          <div class="form-group row">
            <label for="poi_type_html_icon" class="col-md-4 col-form-label form-control-sm text-md-right">ICON (html):</label>
            <div class="col-md-8">
              <input id="poi_type_html_icon" type="text" class="form-control form-control-sm myneedsave" value="<?php echo htmlspecialchars_gks($row['poi_type_html_icon']);?>" style="width:50%;display: inline-block;">
              <div_gks id="poi_type_html_icon_preview"><?php echo $row['poi_type_html_icon'];?></div_gks>
            </div>
          </div>  
          

          <div class="form-group row">
            <label for="poi_type_comments" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Σχόλιο');?>:</label>
            <div class="col-md-8">
              <textarea id="poi_type_comments" type="text" class="form-control form-control-sm myneedsave" style="min-height:100px;"><?php echo htmlspecialchars_gks($row['poi_type_comments']); ?></textarea>
            </div>
          </div> 
          <div class="form-group row">
            <label for="poi_type_disable" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Ενεργός');?>:</label>
            <div class="col-md-8">
              <input type="checkbox" id="poi_type_disable" value="1" <?php if ($row['poi_type_disable']==0) echo ' checked '; ?> class="switchery1_this">
            </div>
          </div> 
...
<div class="container-fluid" id="gks_rsrv_f">
  <div class="form-group1 row">
    <div class="col-md-12 text-center mt-2">
      ...
      <button type="button" class="btn btn-danger deleterowbtn" data-id="<?php echo $row['id_poi_type'];?>" data-model="gks_poi_type" data-backurl="admin-poi-type.php"><?php echo gks_lang('Διαγραφή');?></button>
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-6">
      <?php 
      echo getObjectRels('gks_poi_type',$id);
      echo getActivityObjectTable('gks_poi_type',$id);
      $obj_fileslist= gks_FilesObjectList(array('objname'=>'gks_poi_type','id'=>$id));
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>       
          ...
          <div class="row">
            ...
            <div class="col-md-8"><span class="form-control-plaintext form-control-sm"><?php if ($row['id_poi_type']>0) echo $row['id_poi_type'];?></span></div>
          </div>
          <div class="row">
...
var from_php_dialog_object_rel_curr='gks_poi_type';
var from_php_activity_model='gks_poi_type';
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_poi_type','edit',  $id);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_poi_type','add',   $id);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_poi_type','delete',$id);?>;
...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    datasend='';
    datasend+='&poi_type_descr='  + encodeURIComponent($.base64.encode($("#mypostform #poi_type_descr").val().trim()));
    datasend+='&poi_type_html_icon='  + encodeURIComponent($.base64.encode($("#mypostform #poi_type_html_icon").val().trim()));
    datasend+='&poi_type_comments='  + encodeURI($("#mypostform #poi_type_comments").val().trim());
    datasend+='&poi_type_disable=' + (($('#mypostform #poi_type_disable').is(':checked')) ? '0':'1');
    ...
    $.ajax({
			url: '/my/admin-poi-type-item-exec.php?id=' + <?php echo $id;?>,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  function poi_type_comments_change() {gks_resize_textarea($(this));}
  $('#poi_type_comments').on(mychange, poi_type_comments_change);
  gks_resize_textarea($('#poi_type_comments'));

  $('#poi_type_html_icon').on('input keyup paste', function() {
    $('#poi_type_html_icon_preview').html($('#poi_type_html_icon').val()); 
  });
