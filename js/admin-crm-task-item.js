...
jQuery(document).ready(function($) {
  ...
  $('#task_planned_date_from').datetimepicker(jQuery.extend({},gks_datetimepicker_defaults,{mask:'39/19/9999 29:59',format:'d/m/Y H:i', timepicker:true,dayOfWeekStart:1,step:30,onChangeDateTime:
    function(ct,$i){
      need_save=true;
      ...
      if (tval.length==5) {
        ...
        if (date_from>date_to) {
          $('#task_planned_date_to').datetimepicker('setOptions', {value:date_from});
        }        
      }
    }
  }));
  $('#task_planned_date_to').datetimepicker(jQuery.extend({},gks_datetimepicker_defaults,{mask:'39/19/9999 29:59',format:'d/m/Y H:i', timepicker:true,dayOfWeekStart:1,step:30,onChangeDateTime:
    function(ct,$i){
      need_save=true;
      date_from=$('#task_planned_date_from').datetimepicker('getValue');
...
//  }  
  ...
  function mysubmit() {
    ...
    datasend='';
    datasend+='&task_date=' + encodeURIComponent($("#task_date").val().trim());
    //datasend+='&task_status_id='  + encodeURIComponent($("#mypostform #task_status_id").val().trim());

    datasend+='&task_status_id=' + $('.task_status_selected').attr('data-id');


    datasend+='&subject='  + encodeURIComponent($.base64.encode($("#mypostform #subject").val().trim()));
    datasend+='&message='  + encodeURIComponent($.base64.encode(tinyMCE.get('message').getContent()));
    datasend+='&esoda=' + $('#mypostform #esoda').val();
    
    datasend+='&task_planned_date_from=' + encodeURIComponent($("#task_planned_date_from").val().trim());
    datasend+='&task_planned_date_to=' + encodeURIComponent($("#task_planned_date_to").val().trim());
    //console.log('from' + $("#task_planned_date_from").val().trim());
    //console.log('to  ' + $("#task_planned_date_to").val().trim());
    
    
    
    datasend+='&task_color='  + encodeURIComponent($.base64.encode($("#mypostform #task_color").val().trim()));
    ...
    var machine_ids=[];
    $('#crm_tasks_machine_table .machine_tr').each(function() {
      temp=parseInt($(this).attr('data-id')); if (isNaN(temp)) temp=-1;
      if (temp>0) machine_ids.push(temp);
    });
    datasend+='&machine_ids_str=' + encodeURIComponent($.base64.encode(JSON.stringify(machine_ids)));
    
    var employee_ids=[];
    $('#crm_tasks_employee_table .employee_tr').each(function() {
      temp=parseInt($(this).attr('data-id')); if (isNaN(temp)) temp=-1;
      if (temp>0) employee_ids.push(temp);
    });
    datasend+='&employee_ids_str=' + encodeURIComponent($.base64.encode(JSON.stringify(employee_ids)));
    ...
    $.ajax({
			url: '/my/admin-crm-task-item-exec.php?id=' + from_php_id,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  function gks_admin_get_user_data(user_id, dialog_gsis_result=false) {
    ...
    datasend='cmd=get&id=' + user_id + '&task_id=' + from_php_id;
    $.ajax({
  ...
  dialog_gsis = $( "#dialog_gsis" ).dialog({
    ...
    buttons: [
      {
        id: "dialog_gsis_ok",
        html: '<i class="fa fa-pen-square"></i> '+gks_lang('Ενημέρωση Εργασίας'),
            ...
            for (i=0;i < dialog_gsis_result.firm_act_tab.length; i++) {
              if (dialog_gsis_result.firm_act_tab[i].kind=='1') {
                ...
                break;
    				...
    				if ($('#arithmos').val()=='') $('#arithmos').val(mynymber.trim());
            //$('#perioxi').val('');
...
//    datasend+='&form_ea_nomos_id=' +      encodeURIComponent($('#form_ea_nomos_id').val().trim());
    ...
    datasend+='&crm_task_id=' + from_php_id;
  ...
  function calc_pliroteo_run() {
    ...
    calc_pliroteo_xhr = $.ajax({
			url: '/my/admin-crm-task-item-calc-basket.php?id=' + from_php_id,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  $('.task_status_this').click(function() {
    if (from_php_perm_ret_edit==false) return;
    $('.task_status_this').each(function() {
      $(this).removeClass('task_status_selected');
    });
    $(this).addClass('task_status_selected');
    need_save=true;
  });
  ...
  $('#add_links_url').click(function(event) {  
    if (from_php_id<=0) {
      myalert('error:'+gks_lang('Αποθηκεύστε πρώτα την εργασία'));
      return;
    }
    ...
    $.ajax({
			url: '/my/admin-crm-task-item-add-link.php',
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  function download_action_func(data_id, myaction) {
    ...
    $.ajax({
			url: '/my/admin-crm-task-item-link-action.php',
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  function download_timer_run() {
    ...
    //console.log(fff);
    $.ajax({
			url: '/my/admin-crm-task-item-link-timer.php?id=' + from_php_id,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: '',
  ...
  $('#machine').autocomplete({
    source: function(request, response) {
      mydata={
        ...
        notids: function() {
          ...
          //console.log(existrecs);
          return encodeURIComponent($.base64.encode(JSON.stringify(existrecs)))
        },
        user_id: function () {
          uuu=parseInt($('#user_id').val());
          if (isNaN(uuu)) uuu=0;
          if (uuu<=0) { 
            myalert('error:'+gks_lang('Επιλέξτε πρώτα τον πελάτη'));
          }
          return uuu;
        },
        users_extra_address_id: $('#form_select_apostoli').val(),        
      };
      $.ajax({
        url: 'admin-autocomplete-machine.php',
        dataType: "json",
        cache: false,
        data: mydata,
        error : function(jqXHR ,textStatus,  errorThrown) {
    ...
    delay: 300, //default
    select: function( event, ui ) {
      $('#machine').attr('data-id',ui.item.id);
    },
    change: function (event, ui) {
        if(!ui.item){
          $("#machine").val('').attr('data-id','0');
        }
    }
  });

  $('#add_machine').click(function(event) {  
      id_crm_machine=parseInt($('#machine').attr('data-id'));if (isNaN(id_crm_machine)) id_crm_machine=0;
      if (id_crm_machine<=0) {
        myalert('error:'+gks_lang('Επιλέξτε πρώτα μία συσκευή'));
        return;
      }
      crm_machine_name=$('#machine').val();
      
      
      if ($('#crm_tasks_machine_table .machine_tr[data-id=' + id_crm_machine + ']').length > 0) {
        myalert('error:'+gks_lang('Αυτή η συσκευή υπάρχει ήδη στην λίστα'));
      } else {
        need_save=true;  
        //console.log(ui.item);
        html='<tr class="machine_tr" data-id="' + id_crm_machine + '">' + 
          '<th scope="row" nowrap="" class="mytdcm machine_aa">' +
          '</th><td nowrap="" class="mytdcm">' +
            '<i class="fas fa-trash-alt machine_tr_delete" data-id="' + id_crm_machine + '" style="font-size:120%;color:#dc3545;cursor:pointer;"></i>' +
          '</td>' +
          '<td class="mytdcm">' + '' + '</td>' +
          '<td class="mytdcml" nowrap=""><a href="admin-crm-machine-item.php?id=' + id_crm_machine + '">' + crm_machine_name + '</a></td>' +
          '<td class="mytdcm">'+gks_lang('Τώρα')+'</td>' + 
          '<td class="mytdcml">'+gks_lang('Εγώ')+'</td>' + 
        '</tr>';
        
        $('#crm_tasks_machine_table tbody').append(html);
        $('#crm_tasks_machine_table .machine_tr_delete[data-id=' + id_crm_machine + ']').click(machine_tr_delete_click);
        machine_tr_aa();
  ...
  function machine_tr_aa() {
    var aa=0;
    $('#crm_tasks_machine_table .machine_aa').each(function() {
      aa++;
      $(this).html(aa);  
    });
  }

  function machine_tr_delete_click() {
    need_save=true;
    pid=$(this).attr('data-id');
    $('#crm_tasks_machine_table .machine_tr[data-id=' + pid + ']').remove();
    machine_tr_aa();
  }
  $('.machine_tr_delete').click(machine_tr_delete_click);
  ...
  $('#employee').autocomplete({
    ...
    delay: 300, //default
    select: function( event, ui ) {
      $('#employee').attr('data-id',ui.item.id).attr('data-photo',$.base64.encode(ui.item.photo));
    },
    change: function (event, ui) {
        if(!ui.item){
          $('#employee').val('').attr('data-id','0').attr('data-photo','');
        }
    }
  });

  $('#add_employee').click(function(event) {  
      id_employee=parseInt($('#employee').attr('data-id'));if (isNaN(id_employee)) id_employee=0;
      if (id_employee<=0) {
        myalert('error:'+gks_lang('Επιλέξτε πρώτα έναν υπάλληλο'));
        return;
      }
      employee_name=$('#employee').val();
      employee_photo=$('#employee').attr('data-photo');
      if (employee_photo!='') {
        employee_photo=$.base64.decode(employee_photo);
      }
      //console.log(employee_photo);
      
      if ($('#crm_tasks_employee_table .employee_tr[data-id=' + id_employee + ']').length > 0) {
        myalert('error:'+gks_lang('Αυτός ο υπάλληλος υπάρχει ήδη στην λίστα'));
      } else {
        need_save=true;  
        //console.log(ui.item);
        html='<tr class="employee_tr" data-id="' + id_employee + '">' + 
          '<th scope="row" nowrap="" class="mytdcm employee_aa">' +
          '</th><td nowrap="" class="mytdcm">' +
            '<i class="fas fa-trash-alt employee_tr_delete" data-id="' + id_employee + '" style="font-size:120%;color:#dc3545;cursor:pointer;"></i>' +
          '</td>' +
          '<td class="mytdcm p-0">' + employee_photo + '</td>' +
          '<td class="mytdcml" nowrap=""><a href="admin-user-item.php?id=' + id_employee + '">' + employee_name + '</a></td>' +
          '<td class="mytdcm">'+gks_lang('Τώρα')+'</td>' + 
          '<td class="mytdcml">'+gks_lang('Εγώ')+'</td>' + 
        '</tr>';
        
        $('#crm_tasks_employee_table tbody').append(html);
        $('#crm_tasks_employee_table .employee_tr_delete[data-id=' + id_employee + ']').click(employee_tr_delete_click);
        employee_tr_aa();
      }
  ...
  function employee_tr_aa() {
    var aa=0;
    $('#crm_tasks_employee_table .employee_aa').each(function() {
      aa++;
      $(this).html(aa);  
    });
  }

  function employee_tr_delete_click() {
    need_save=true;
    pid=$(this).attr('data-id');
    $('#crm_tasks_employee_table .employee_tr[data-id=' + pid + ']').remove();
    employee_tr_aa();
  }
  $('.employee_tr_delete').click(employee_tr_delete_click);
  ...
  function dialog_print_button(ispreview) {
    ...
    $('body').addClass("myloading");
    $.ajax({
			url: 'admin-crm-task-item-pdf.php?id=' + from_php_id,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
			...
			success: function(data) {
              ...
              if (preview_win==null) {
                ...
                '<a href="' + data.preview_url + '" class="gks_link" target="_blank">'+gks_lang('σύνδεσμος')+'</a>');
              } else {
                preview_win.focus();
              }
  ...
  $('#submit_button_print').click(function() {
    ...
    sel_company_id_sub_id=$('#company_id').val() + '|' + $('#company_sub_id').val();
    sel_transfer_reservation_journal_id=0; //parseInt($('#transfer_reservation_journal_id').val()); if (isNaN(sel_transfer_reservation_journal_id)) sel_transfer_reservation_journal_id=0;
    sel_transfer_reservation_seira_id=0; //parseInt($('#transfer_reservation_seira_id').val()); if (isNaN(sel_transfer_reservation_seira_id)) sel_transfer_reservation_seira_id=0;
    
    var temp=$('#user_lang').val();
    if (temp=='') temp='el-GR';
    var temp_cc=0;
    var temp_hh=0;
    if (temp!='') {
          ...
          for (i=0;i<from_php_perm_print_forms.length;i++) {
            if (from_php_perm_print_forms[i].id == data_form_id) {
              ...
              if (typeof(from_php_perm_print_forms[i].perm_acc_journal_ids) != 'undefined') {
                if (from_php_perm_print_forms[i].perm_acc_journal_ids.includes(sel_transfer_reservation_journal_id)==false) {
                  will_show=false;
                  break;
                }
              }
              if (typeof(from_php_perm_print_forms[i].perm_acc_seires_ids) != 'undefined') {
                if (from_php_perm_print_forms[i].perm_acc_seires_ids.includes(sel_transfer_reservation_seira_id)==false) {
                  will_show=false;
                  break;
                }
              }
