...
//        }
  ...
  function gks_open_product_history(event) {
    myhref='admin-whi-mov-product-history.php?fwarehouse_id=' + warehouse_id + '&fproduct_id=' + product_id;
    //console.log(myhref);
    ...
    buttons: [
      {
        ...
        click: function() {
          ...
          $.ajax({
      			url: '/my/admin-whi-mov-balance-apografi-exec.php',
      			type: 'POST',
      			cache: false,
      			dataType: 'json',
      			data: datasend,
      			...
      			success: function(data) {
      				...
      				if (!data) {
      					...
      					if (data.success == true) {
        					...
        					$('.mytdypoloipo[data-warehouse=' + this.gks_warehouse_id + '][data-product=' + this.gks_product_id + ']').html(data.balance).attr('data-val',data.balance_val);
        					$('.mytdtotal[data-product=' + this.gks_product_id + ']').html(data.total_balance);
        					dialog_posotitaonhand.dialog('close');
      					} else {
