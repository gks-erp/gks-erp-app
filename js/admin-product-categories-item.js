...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    ...
    datasend+='&product_category_descr='  + encodeURI($("#mypostform #product_category_descr").val().trim());
    datasend+='&product_category_parent_id='  + encodeURI($("#mypostform #product_category_parent_id").val().trim());
    datasend+='&category_comments='  + encodeURIComponent($.base64.encode(tinyMCE.get('category_comments').getContent()));
    datasend+='&category_disable=' + (($('#mypostform #category_disable').is(':checked')) ? '0':'1');
    datasend+='&category_photo='  + encodeURI($("#form_product_category_photo").val().trim());
    ...
    $.ajax({
			url: '/my/admin-product-categories-item-exec.php?id=' + from_php_id,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  $('#add_product').click(function(event) {  
    if (from_php_id<=0) {myalert('error:'+gks_lang('Αποθηκεύστε πρώτα την κατηγορία')); return;}	
    datasend='';
    ...
    $.ajax({
			url: '/my/admin-product-categories-item-product-add.php',
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
  ...
  $('#add_ergasia').click(function(event) {  
    if (from_php_id<=0) {myalert('error:'+gks_lang('Αποθηκεύστε πρώτα την κατηγορία')); return;}	
    datasend='';
    datasend+='cat_id=' + from_php_id;    
    datasend+='&from=cat&&ergasia_id='  + encodeURI($("#ergasia_id").val().trim());    
    //console.log(datasend);
    ...
    $.ajax({
            ...
            $('.ergasies_tr_new').each(function() {
              $(this).removeClass('ergasies_tr_new').addClass('product_tr_exist');
            });
            var ergasies_aa=0;
  ...
  jqXHR = $('#myphoto_category_upload').fileupload({
      ...
      add: function (e, data) {
          ...
          if (from_php_id<=0) {
             uploadErrors.push(gks_lang('Αποθηκεύστε πρώτα την κατηγορία'));
          }
          ...
          $.each(data.result.files, function (index, file) {
            if (typeof file.error == 'undefined') {
              ...
              myhtmlimg+='  <a class="lightgallery_item_category" href="' + file.url + '" data-download-url="' + file.url + '">';
              ...
              myhtmlimg+='      <a href="" class="set_category_photo"   data-url="' + file.url_thumb + '" title="' + gks_lang('Ορισμός ως προεπιλεγμένη φωτογραφία') + '"><img src="/my/img/icons/photo.png" border="0" width="16"></a>';
              myhtmlimg+='      <a href="" class="delete_category_upload_photo" data-url="' + file.url_thumb + '" data-id="' + file.insert_id + '" title="' + gks_lang('Διαγραφή') + '"><img src="/my/img/0.png" border="0" width="16"></a>';
              myhtmlimg+='  </div>';
              myhtmlimg+='</div>';


              $('#imagelist_photo').append(myhtmlimg);
              $('#item_upload_photo_' + file.insert_id + ' .delete_category_upload_photo').click(delete_category_upload_photo_click);
              $('#item_upload_photo_' + file.insert_id + ' .set_category_photo').click(set_category_photo_click);
              
             
            
              $("#lightgallery_category").data('lightGallery').destroy(true);
              $("#lightgallery_category").lightGallery({
              	selector: '.lightgallery_item_category',
              	thumbnail:true,
              	hideBarsDelay:1000,
              }); 
              
              if ($('#form_product_category_photo').val() == '') {
                $('#form_product_category_photo').val(file.url_thumb);
                $('#form_product_category_photo_img').attr("src",file.url_thumb);  
                $('#reset_category_photo').show(); 
                need_save=true;         
              }
            }
          });
	...
	delete_category_upload_photo_click = function(event){	
    var uid=$(event.target.parentNode).attr('data-id');
    var data_url=$(event.target.parentNode).attr('data-url');
    
    
    $.ajax({
			url: '/my/admin-product-categories-item-photo-delete.php?id=' + uid,
			myuid: uid,
			type: 'POST',
			cache: false,
			dataType: 'json',
			...
			success: function(data) {
				...
				if (!data) {
					...
					if (data.success == true) {
  					...
  					if (this.mydata_url == $('#form_product_category_photo').val()) {
    					need_save=true;
    					if ($(".set_category_photo").length == 0) {
    					  
                $('#form_product_category_photo').val('');
                $('#form_product_category_photo_img').attr("src",'/my/img/product.png');
                $('#reset_category_photo').hide();
              } else {
                
                $(".set_category_photo").each(function( index ) {
                  var data_url=$(this).attr('data-url');
                  $('#form_product_category_photo').val(data_url);
                  $('#form_product_category_photo_img').attr("src",data_url);
                  $('#reset_category_photo').show();
                  return;
                });  					
      				}
  ...
  $('.delete_category_upload_photo').click(delete_category_upload_photo_click);

	set_category_photo_click = function(event){	
    if (from_php_id<=0) {myalert('error:'+gks_lang('Αποθηκεύστε πρώτα την κατηγορία')); return;}	  
    need_save=true;      
    var data_url=$(event.target.parentNode).attr('data-url');
    $('#form_product_category_photo').val(data_url);
  ...
  $('.set_category_photo').click(set_category_photo_click);

  $('#reset_category_photo').click(function() {
    if (from_php_id<=0) {myalert('error:'+gks_lang('Αποθηκεύστε πρώτα την κατηγορία')); return;}	  
    need_save=true;
    $('#form_product_category_photo').val('');
    $('#form_product_category_photo_img').attr("src",'/my/img/product.png');   
  ...
  function eshop_sync_click() {
    if (need_save) {
      myalert('error:'+gks_lang('Αποθηκεύστε πρώτα την κατηγορία'));
      return;
    }
    ...
    datasend='product_category_id=' + from_php_id + '&eshop_id=' + eshop_id;
    $.ajax({
			url: '/my/admin-product-categories-item-eshop-sync.php',
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
    ...
    buttons: [
      {
        ...
        click: function() {
          ...
          remote_category_id=parseInt($('#dialog_eshoplink_list').val());
          if (isNaN(remote_category_id)) remote_category_id=0;
          if (remote_category_id<=0) {
            myalert('error:'+gks_lang('Επιλέξτε μια κατηγορία'));
            return;
          }
          datasend='product_category_id=' + from_php_id + '&eshop_id=' + eshop_id + '&remote_category_id=' + remote_category_id;
          //console.log(datasend);
          
          $('body').addClass("myloading");
          $.ajax({
      			url: '/my/admin-product-categories-item-eshop-link-remote-category.php',
      			type: 'POST',
      			cache: false,
      			dataType: 'json',
      			data: datasend,
  ...
  $('#dialog_eshoplink_eshop').change(function() {
    ...
    $.ajax({
			url: '/my/admin-product-categories-item-eshop-get-categories.php',
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
