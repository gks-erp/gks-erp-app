...
$my_page_title=gks_lang('Αναφορά - Pivot Table - Συσκευές');
$nav_active_array=array('crm','crm_machine_pivot11');
...
$_gks_session['temp']['where_crm_machine_pivot11']=$sql;
gks_erp_cookie_save();
...
jQuery(document).ready(function($) {
  ...
  $('.filterselectbox').on('change', function() {
      ...
      if (v==-2) { //is_custom_date
        if (sname == 'fmydate_add' || gks_custom_filters_date_elems.includes(sname)) {
  ...
  var mydata={};
  mydata.cols=[gks_lang('Ημέρα')];
  mydata.rows=[gks_lang('Έτος'),gks_lang('Μήνας')];
  mydata.vals=[gks_lang('Συσκευές')];
  ...
  $.ajax({
		url: '/my/admin-reports-crm-machine-pivot11-json.php',
		type: 'POST',
		cache: false,
		dataType: 'json',
		data: '',
  ...
  $('#export_pivot_to_excel').click(function() {
    ...
    $.ajax({
			url: '/my/admin-reports-crm-machine-pivot11-export-excel.php',
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
