...
$my_page_title=gks_lang('Cardlink Log');
...
if ($id>0) {
  $sql="select id_eftpos_transaction,xxx_transaction_id,send_array,response_array from gks_eftpos_transaction where id_eftpos_transaction=".$id." and payment_acquirer_with_id=4";
  ...
  $sql="select id_eftpos_transaction,xxx_transaction_id,send_array,response_array from gks_eftpos_transaction where xxx_transaction_id=".$mtid." and payment_acquirer_with_id=4";
  $result = $db_link->query($sql);
  if (!$result) { debug_mail(false,'error sql',$sql);die('sql error');}  
  if ($result->num_rows==0) {
