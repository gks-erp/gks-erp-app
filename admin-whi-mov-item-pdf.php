...
$my_page_title=gks_lang('Εκτύπωση Δελτίου').' id: '.$id;
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_whi_mov','view',$id);
if ($perm_ret['success']==false) {$return = array('success' => false, 'message' => base64_encode($perm_ret['message']));echo json_encode($return); die();}


if ($id==-1) {
    debug_mail(false,'first save delivery note');
    $return = array('success' => false, 'message' => base64_encode(gks_lang('Αποθηκεύστε πρώτα το δελτίο αποστολής')));
    echo json_encode($return); die();}
...
$mov_state=trim_gks($row['mov_state']);



$save_folder='whi/mov/'.$id.'/print/';
$save_basename='MOV_'.$id.'_'.greeklish(getWhiMovStateDescr($mov_state)).'_'.showDate(time(), 'Y-m-d_H.i.s',1).'.'.rand(100,999);
$print_params=array(
  'table' => 'gks_whi_mov',
  'id' => $id,
...
if ($gks_print_send_gks_erp_app==1 and $preview==0) {
  $sql_send_erp_app="SELECT gks_whi_mov.id_whi_mov, gks_acc_seires.erp_app_id, gks_acc_seires.erp_app_dest, 
  ...
  AND gks_whi_mov.mov_whi_seira_id>0
...
$ret_print = gks_print_form('gks_whi_mov',$id,$form_id,$print_params);
...
if ($ret_print['success']==false) {
  debug_mail(false,'error create print',$ret_print['message']);
...
if ($preview==0) {
  $sql="update gks_whi_mov set 
    ...
    print_mov_state='".$db_link->escape_string($mov_state)."'
    where id_whi_mov=".$id;
  ...
  gks_lang('Κατάσταση').': <span class="whi_mov_state_'.$mov_state.'">'.getWhiMovStateDescr($mov_state).'</span><br>'.
...
if (isset($_POST['set_def']) and $_POST['set_def']=='1') {
  ...
  $user_settings['print']['form_id_whi']=  number_format($form_id,0,'','');
...
$return = array('success' => true, 
  ...
  'redirect'=> base64_encode('admin-whi-mov-item.php?id='.$id.'&scrollto=gks_print'),
