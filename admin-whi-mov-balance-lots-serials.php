...
$my_page_title=gks_lang('Υπόλοιπα Παρτίδων-Serial Numbers');
$nav_active_array=array('warehouse','warehouse_balance_lots_serials');
...
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_whi_mov_balance_lots_serials','view',0);
...
$filters[] = array(
    ...
    'vals' => array(
        ...
        array('value' => 3,  'text' => gks_lang('Όλες οι παρτίδες-serial numbers'),  'sql' => "3"),
        array('value' => 4,  'text' => gks_lang('Όλες οι αποθήκες και όλες παρτίδες-serial numbers'),  'sql' => "1=1"),
    ),
...
$filters[] = array(
  ...
  'has_custom_default' => 1,
//		'mywherepos'=>1,
  'vals' => gks_filter_date_vals(['field'=>'gks_eshop_product_lots.lot_date_expire','future'=>false,'today'=>$today, 'today_vardia'=>$today_vardia]),
...
$filters[] = array(
    ...
    'class' => 'filterselectbox',
    'style' => '',
    'title' => gks_lang('Ενεργή Παρτίδα'),//Serial Number
    'has_custom_default' => -1,
    'multiselect' => true,
...
$sortable = array(
  ...
  array('name' => 'sodescr2', 'field' => 'product_descr_p'),
...
if ($fskip==1 or $fskip==2) {
  ...
  $sql_filter_product=" and gks_eshop_product_lots.id_lot_product in (
    SELECT lot_product_id
    FROM gks_warehouse_balance_lots_serials
    WHERE balance<>0 AND warehouse_id>2
...
if ($sort_warehouseid>=0) { 
  ...
  $sql_FROM_balance_2="
  left join (
    select lot_product_id,balance from gks_warehouse_balance_lots_serials where warehouse_id=".$sort_warehouseid."
  ) as mybalance ON gks_eshop_product_lots.id_lot_product = mybalance.lot_product_id)
";
}
...
FROM ".$sql_FROM_cat_1." ".$sql_FROM_bra_1." ".$sql_FROM_balance_1."
...
if (empty($sorted['sql'])) {
	...
	gks_eshop_product_lots.lot_sortorder,gks_eshop_product_lots.lot_name
	";
} else {
...
<thead>
    <tr >	
        ...
        <th class="table-dark" scope="col" style="text-align: left   !important;" width="40%"><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sodescr2', gks_lang('Περιγραφή')); ?></th> 
...
foreach ($warehouses as $witem) {
  if (count($where3_array)==0 or in_array($witem['id'],$where3_array)) {
  //if (in_array($witem['id'],$where3_array)) {
...
<tbody>
    ...
    while ($row = $result->fetch_assoc()) {
      $row_array[]=$row;
      $ids[]=$row['id_lot_product'];
    }
    $bbb=[];
    if (count($ids)>0) {
      $sql="select * from gks_warehouse_balance_lots_serials where lot_product_id in (".implode(',',$ids).")";
      //echo '<pre>'.$sql;die();
      ...
      if (!$result) die('sql error');
      while ($row = $result->fetch_assoc()) {
        if (isset($bbb[$row['lot_product_id']])==false) {
          $bbb[$row['lot_product_id']]=array(
            'warehouses'=>array(),
          );
        }
        if (isset($bbb[$row['lot_product_id']]['warehouses'][$row['warehouse_id']])==false) {
          $bbb[$row['lot_product_id']]['warehouses'][$row['warehouse_id']]=0;
        }
        $bbb[$row['lot_product_id']]['warehouses'][$row['warehouse_id']]+=$row['balance'];
...
?>
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>">
    ...
    <td nowrap class="mytdcml"><span id="lot_title_<?php echo $row['id_lot_product'];?>"><?php echo $row['lot_name'];?></span></td>
    ...
    <td class="mytdcml"><a href="admin-products-item.php?id=<?php echo $row['id_product'];?>"><span id="product_title_<?php echo $row['id_product'];?>"><?php echo $row['product_descr_p'];?></span></a></td>
    <td nowrap class="mytdcm"><?php echo $row['monada_symbol'];?></td> 
    <td nowrap class="mytdcm mytdtotal" 
      data-product="<?php echo $row['id_product'];?>" 
      data-lot="<?php echo $row['id_lot_product'];?>"><?php 
      
      $this_bal=0;
      if (isset($bbb[$row['id_lot_product']]['warehouses'][0])) {
        $this_bal=$bbb[$row['id_lot_product']]['warehouses'][0];
      }      
...
foreach ($warehouses as $witem) {
  if (count($where3_array)==0 or in_array($witem['id'],$where3_array)) {
    $this_bal=0;
    if (isset($bbb[$row['id_lot_product']]['warehouses'][$witem['id']])) {
      $this_bal=$bbb[$row['id_lot_product']]['warehouses'][$witem['id']];
    }
        
    echo '<td nowrap class="mytdcm mytdypoloipo" data-warehouse="'.$witem['id'].'" '.
    'data-product="'.$row['id_product'].'" '.
    'data-lot="'.$row['id_lot_product'].'" '.
...
<div id="dialog_posotitaonhand" title="<?php echo $GKS_SITE_HUMAN_NAME;?>" style="display: none;">
  <div>
    <?php echo gks_lang('Ορίστε την ποσότητα που έχετε τώρα στο χέρι για την<br>παρτίδα/serial number');?>:<br>
    <b><span id="dialog_posotitaonhand_lot"></span></b><br>
    <?php echo gks_lang('του είδους');?>:<br>
...
var from_php_perm_ret_apografi_add=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_whi_mov_balance_lots_serials_apografi','add',0);?>;
...
<script src="js/admin-whi-mov-balance-lots-serials.js?v=<?php echo $gks_cache_version;?>"></script>
