...
$sortable = array(
		array('name' => 'soid', 'field' => 'id_aade_skopos_diakinisis'),
		array('name' => 'socode', 'field' => 'aade_skopos_diakinisis_code'),
		array('name' => 'sodescr', 'field' => 'aade_skopos_diakinisis_descr'),
...
if (empty($sorted['sql'])) {
	$sql .= " ORDER BY sortorder,aade_skopos_diakinisis_descr,id_aade_skopos_diakinisis";
} else {
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-aade-skopos-diakinisis-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέου σκοπού διακίνησης');?></a>
...
<table class="table table-sm table-responsive1 table-striped table-bordered gkstable <?php
  ...
  ?>" border="0" cellspacing="0" cellpadding="5" align="center" id="table_gks_aade_skopos_diakinisis">
<thead>
  <tr >	
...
<tbody>
	...
	$i++;
?>
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>" data-id="<?php echo $row['id_aade_skopos_diakinisis'];?>">
    <th class="mytdcm" scope="row" nowrap><?php echo ($i + $showFrom);?></th>
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-aade-skopos-diakinisis-item.php?id=<?php echo $row['id_aade_skopos_diakinisis'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_aade_skopos_diakinisis'];?></td>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_aade_skopos_diakinisis'];?>" data-model="gks_aade_skopos_diakinisis"></i></td>
        </tr>      
      </table>
    </td>

    <td class="mytdcml"><?php echo $row['aade_skopos_diakinisis_descr'];?></td>
    <td class="mytdcm"><?php echo $row['aade_skopos_diakinisis_code'];?></td>
<?php if ($perm_edit) {?>
...
jQuery(document).ready(function($) {
  ...
  $('#table_gks_aade_skopos_diakinisis > tbody').sortable({
    handle: '.sortorder_handle',
    update: function( event, ui ) {
      mylist = $(this).sortable('toArray', {attribute: 'data-id'});
      gks_sortorder_obj('gks_aade_skopos_diakinisis',mylist,'#table_gks_aade_skopos_diakinisis > tbody');
    }
  }); 
  <?php if ($perm_edit) {?>
