...
$my_page_title=gks_lang('Αναφορά - Pivot Table - Παραγγελίες με Είδη - Λήψη δεδομένων').'...';
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_orders_pivot3','view',0);
...
$gks_custom_orders_prepare = gks_custom_table_item_prepare('gks_orders',['from'=>'pivot']);
$gks_custom_eidi_prepare = gks_custom_table_item_prepare('gks_eshop_products',['from'=>'pivot']);

$gks_custom_orders_header_fields = gks_custom_table_pivot_header($gks_custom_orders_prepare);
$gks_custom_eidi_header_fields = gks_custom_table_pivot_header($gks_custom_eidi_prepare);

//echo '<pre>'; print_r($gks_custom_orders_header_fields);die();
//print $sql;
//die();
...
if (isset($_gks_session['temp']['wherepivot3']) and $_gks_session['temp']['wherepivot3']!='') {
  $sql=$_gks_session['temp']['wherepivot3'];
} else {
  die();  
}
...
$file_path='pivot3_'.$my_wp_user_id.'_'.rand(100000,999999).'.csv';
$mypath=$dir_path.$file_path;
if (file_exists($mypath)) unlink($mypath);

$myfile = fopen($mypath, "a");

$first_line='"'.gks_lang('Έτος').'","'.gks_lang('Μήνας').'","'.gks_lang('Ημερομηνία').'","'.gks_lang('Ημέρα').'","'.gks_lang('Ώρα').'","'.gks_lang('Κατάσταση').'","'.gks_lang('Εταιρεία').'","'.gks_lang('Υποκατάστημα').'","'.gks_lang('Ημερολόγιο').'","'.gks_lang('Σειρά').'","'.gks_lang('Φορολογική Θέση').'","'.gks_lang('Τιμοκατάλογος').'",'.
...
'"'.gks_lang('Είδος').'","'.gks_lang('Κωδικός').'","'.gks_lang('Αξία','part2').'","'.gks_lang('Ποσότητα').'",';
if ($GKS_ORDERS_SHEETS) $first_line.='"'.gks_lang('Σελίδες').'",';
...
if ($GKS_ORDERS_OCCASION) $first_line.='"'.gks_lang('Περίσταση').'",';

foreach ($gks_custom_orders_header_fields as $value) {
  $first_line.='"'.$value.'",';
}  
...
while ($row = $result->fetch_assoc()) {
  ...
  $line=
    ...
    '"'.gks_csv_txt(getWeekDayName(date('w', strtotime($row['order_date']) - GKS_ERP_START_VARDIA*60*60))).'",'.
    ...
    $row['product_price_final_all_net'].','.
    $row['product_quantity'].','
  ;
  
  
  if  ($GKS_ORDERS_SHEETS) $line.=$row['product_sheets'].',';
  ...
  if  ($GKS_ORDERS_OCCASION) $line.='"'.gks_csv_txt($row['occasion_type_descr']).'",';
    
  $line_extra_orders= gks_custom_table_pivot_rows($gks_custom_orders_prepare,$row);
  //print $line_extra_orders; die();
  $line.=$line_extra_orders;
