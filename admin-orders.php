...
$my_page_title=gks_lang('Παραγγελίες');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-orders-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέας παραγγελίας');?></a>
      <?php
      $sql_export_excel="SELECT id_export_excel, export_excel_descr FROM gks_export_excel WHERE export_excel_object='gks_orders' and export_excel_disable=0 ORDER BY export_excel_descr;";
      ...
      <div class="btn-group" >
        ...
        <div class="dropdown-menu" style="background-color: #fafcff;box-shadow: 0px 4px 24px 12px rgb(0 0 0 / 20%), 0px 6px 20px 0px rgb(0 0 0 / 19%);">
          ...
          foreach ($export_excel as $itemee) {
            echo '<a class="dropdown-item" href="admin-orders-export-excel.php?'.$_SERVER['QUERY_STRING'].'&id_export_excel='.$itemee['id_export_excel'].'" download>'.$itemee['export_excel_descr'].'</a>';
          }?>
        </div>  
      </div> 
      <?php } ?>
...
<?php }?>
        ...
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;" width="0%" ><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'soexec', gks_lang('Εκτέλεση')); ?></th>        
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;" width="0%" ><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sopay', gks_lang('Πληρωμή')); ?></th>        
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;" width="0%"><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'souser', gks_lang('Επαφή')); ?></th> 
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;" width="0%"  nowrap><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sopoli', gks_lang('Πόλη')); ?></th> 
<?php 
        gks_plugins_functions_run('admin_orders_th1',array());
?>
        ...
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;" width="0%" ><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'soddate', gks_lang('Παράδοση')); ?></th>        
...
<?php if ($GKS_ORDERS_OCCASION) {?>
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;max-width:150px;" width="10%" ><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'soocc', gks_lang('Περίσταση')); ?></th>  
<?php } ?>              
        <th nowrap class="table-dark" scope="col" style="text-align: left   !important;" width="45%" ><?php echo gks_lang('Ιδιότητες');?></th>        
        <th nowrap class="table-dark" scope="col" style="text-align: left   !important;" width="45%" ><?php echo gks_lang('Σχόλια');?></th>        
<?php if ($GKS_ORDERS_PRODUCTION) {?>
        <th nowrap class="table-dark" scope="col" style="text-align: center !important;" width="10%" ><?php 
          echo makeSortLink($sortable, $sortable_url, $_GET, 'sopososto', '<span class="tooltipster" title="'.gks_lang('Ποσοστό Παραγωγής').'">'.gks_lang('Ποσοστό').'</span>').'<br>'; 
          echo makeSortLink($sortable, $sortable_url, $_GET, 'sotime', gks_lang('Χρόνος')); 
          
          ?></th>
        <th class="table-dark" scope="col" style="text-align: left   !important;" width="0%"  nowrap><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sokostos', gks_lang('Κόστος')); ?></th>        
  ...
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>">
    ...
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-orders-item.php?id=<?php echo $row['id_order'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_order'];?></td>
          <?php if ($perm_orders_delete) {?>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_order'];?>" data-model="gks_orders"></i></td>
          <?php } ?>
        </tr>      
      </table>
      ...
      <?php 
      $ret_plugin='';
      gks_plugins_functions_run('admin_orders_td1',array(
        'row'=>&$row,
        'ret_plugin' => &$ret_plugin,
      ));
      
      if ($ret_plugin!='') {
        echo $ret_plugin;
      } else {
        if (!empty($row['order_ref_number'])) {?>
          <table cellpadding=0 cellspacing=0 class="ref_number_table">
            <tr class="ref_number_table_tr">
              <td class="ref_number_table_td">
                <span class="ref_number_table_td_span"><?php echo $row['order_ref_number']?></span>
              </td>
            </tr>  
          </table>
        <?php } ?>        
...
<?php if (count($user_companys)>1) {?>
    <td        class="mytdcm"><?php echo $row['company_title']; if (isset($row['company_sub_title'])) echo ' \ '.$row['company_sub_title']; else echo ' \ '.gks_lang('Κεντρικό');?></td> 
<?php } ?>
    <td        class="mytdcm"><?php echo $row['acc_journal_descr'];?></td>
    <td nowrap class="mytdcm"><?php echo $row['seira_code'];?></td>
    <td nowrap class="mytdcm"><?php if ($row['order_number_int']<>0) echo $row['order_number_int'];?></td>

    <td nowrap class="mytdcm"><?php if (isset($row['mdate_execute'])) echo showDate(strtotime($row['mdate_execute']), 'd/m/Y\<\b\r\>H:i:s', 1);?></td>   
    <td nowrap class="mytdcm"><?php if (isset($row['mdate_payment'])) echo showDate(strtotime($row['mdate_payment']), 'd/m/Y\<\b\r\>H:i:s', 1);?></td>   
    ...
    <?php
      $ret_plugin='';
      gks_plugins_functions_run('admin_orders_td2',array(
        'row'=>&$row,
        'ret_plugin' => &$ret_plugin,
      ));
      echo $ret_plugin;
    ...
    <td nowrap class="mytdcm" ><?php 
      if ($row['gks_price_net']!=0) echo '<b>'.myCurrencyFormat($row['gks_price_net']).'</b>';
        ...
        $ret_plugin='';
        gks_plugins_functions_run('admin_orders_td3',array(
          'row'=>&$row,
          'ret_plugin' => &$ret_plugin,
        ));
        echo $ret_plugin;
    ...
    ?></td>    
    <td nowrap class="mytdcm"><?php if (isset($row['ddate'])) echo showDate(strtotime($row['ddate']), 'd/m/Y', 1);?></td>   
    <td        class="mytdcm"><?php echo $row['delivery_method_name'];?></a></td>
    <td        class="mytdcm"><?php echo $row['payment_acquirer_name'];?></a></td>
      
<?php if ($GKS_ORDERS_OCCASION) {?>    
    <td        class="mytdcm" style="max-width:150px;word-wrap: break-word;">
      <?php echo $row['occasion_type_descr'];?><br>
      <?php echo $row['occasion_title'];?>
    </td>
<?php } ?>
...
<?php  if ($GKS_ORDERS_PRODUCTION) {?>    
    ...
    <td nowrap class="mytdcm" ><?php 
      if (isset($row['production_kostos']) and $row['production_kostos']!=0) 
        echo '<a href="admin-production-item.php?id='.$row['id_order'].'">'.myCurrencyFormat($row['production_kostos']).'</a>';
    ?></td>
<?php } ?>   
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_orders','edit',  0);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_orders','add',   0);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_orders','delete',0);?>;
