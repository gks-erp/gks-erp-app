...
$my_page_title=gks_lang('Αποθήκευση κίνησης απογραφής').' '.$warehouse_id.' '.$product_id.' '.$posotitaonhand;
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_whi_mov_balance_apografi','add',0);
...
$curr_balance=0;
$sql="select balance from gks_warehouse_balance_eidi
...
if ($result->num_rows>=1) {
  ...
  $sxolio=gks_lang('Προσθήκη είδους [1] με ποσότητα: [2]'); 
  $sxolio=str_replace('[1]',$product_descr,$sxolio);
  $sxolio=str_replace('[2]',$diafora,$sxolio);
  $sql="insert into gks_whi_mov_log (whi_mov_id, add_date,user_id,sxolio) values (
  ".$id_whi_mov.",now(),".$my_wp_user_id.",'".$db_link->escape_string($sxolio)."')";
  $result = $db_link->query($sql);        
  if (!$result) {
...
$total_balance=0;
$sql="select balance from gks_warehouse_balance_eidi 
where product_id=".$product_id." and warehouse_id=0";
$result = $db_link->query($sql);        
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode('sql error'));
