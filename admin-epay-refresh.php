...
//include_once('functions_epay.php');

//echo '<pre>';
// /my/admin-epay-refresh.php?today=1
// /my/admin-epay-refresh.php?days=0

if (isset($my_wp_user_id)==false) $my_wp_user_id=2;
if (isset($gkIP)==false) $gkIP='127.0.0.1';


$my_page_title=gks_lang('epay Refresh'); 
db_open();
stat_record();

$sql="SELECT id_company
FROM gks_company
WHERE epay_username<>'' AND epay_password<>'' AND epay_authorization_code<>'' and epay_x_api_key<>'' AND company_disable=0
...
if (isset($_GET['today'])) {
  ...
  foreach ($id_company_ids as $id_company) {
    for($i=0;$i<=1;$i++) {
      gks_epay_api_get_transactions($i,$id_company,'');
