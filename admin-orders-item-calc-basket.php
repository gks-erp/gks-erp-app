...
$my_page_title=gks_lang('Υπολογισμός dp Παραγγελίας').' id: '.$id;
db_open();
stat_record();
...
if (isset($mydata['gks_lock']) and $mydata['gks_lock']==true) {
  ...
  $mybasketarray['order_journal_id']=intval($row['order_journal_id']);
  $mybasketarray['order_seira_id']=intval($row['order_seira_id']);
  $mybasketarray['order_state']=trim_gks($row['order_state']);
  ...
  $cache_file='order_'.$id.'_'.date('Y-m-d_H-i-s').'_'.rand(10000,99999).'.json';
...
$mybasketarray['order_journal_id']=intval($mydata['order_journal_id']);
$mybasketarray['order_seira_id']=intval($mydata['order_seira_id']);
$mybasketarray['order_state']=trim_gks($mydata['order_state']);
$mybasketarray['order_date']='';
if ($mydata['order_date'] == '__/__/____ __:__') $mydata['order_date']='';
$mydata['order_date']=trim_gks(stripslashes(urldecode($mydata['order_date'])));
if ($mydata['order_date']!='') {
  $mybasketarray['order_date'] = mystrtodb($mydata['order_date']);
}
$mybasketarray['online_enable']=intval($mydata['online_enable'])==1;
...
$mybasketarray['parastatiko']=intval($mydata['parastatiko']);
  ...
  switch ($mycmd) {
    case 'couponadd':
      ...
      if (isset($mybasketarray['coupons'][$pricelist_item_coupon])) {
        debug_mail(false,'coupon is added:'. $mycoupon);
...
//  $product_price_ekptosi_pososto=0;
//  
//  if ($value['product_id']['product_price_start_all_net'] != $value['product_id']['product_price_final_all_net']) {
//    $ekptosi_poso_html=(myCurrencyFormat($value['product_id']['product_price_start_all_net'] - $value['product_id']['product_price_final_all_net'],false,true));
//    
//    if  ($value['product_id']['product_price_start_all_net']!=0) $product_price_ekptosi_pososto=round($product_price_ekptosi_net*100/$value['product_id']['product_price_start_all_net'],2);
//  }
  ...
  $eidi[] = array(
    ...
    'other_taxes_tooltip' => base64_encode($value['other_taxes_tooltip']),
