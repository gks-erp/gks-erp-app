...
$return = array(
  'success'     => false, 
  'message'     => gks_lang('Δεν έχει ορισθεί το').' ID.',
  'frpc_enable' => false,
  'frpc_ini'    => '',
  'port'        =>54678,
);
...
$return_ext = array(
  ...
  'can_pos'     =>false,
  'pos_list'    =>[],
  'pos_extra_data'=>array('viva_merchantKey'=>'','wordline_version'=>'','wordline_app_language_code'=>'','wordline_merchant_bank_id'=>'','wordline_solution_partner_id'=>''),
  'can_transfer' =>false,
  'user_token' => '',
  'latest_ver_msg'=>'',
  'latest_ver_download_link'=>'',
  'printers_rpp200'=> array(
    'RPP200',
  ),
  'printers_btprinter'=> array(
    'BlueTooth Printer',
  ),
);
...
if ($login_type=='token') {
  if ($token=='' or $semd5=='') {	
    debug_mail(false,'empty data 1','');
    $return_ext['message']=gks_lang('Λάθος δεδομένα');echo json_encode($return_ext); die();}
  ...
  if ($result->num_rows<>1) {	
    ...
    $return_ext['message']=gks_lang('Το κλειδί είναι λάθος');echo json_encode($return_ext); die();}
  ...
  $erp_app_mobile_can_transfer=intval($row_app['erp_app_mobile_can_transfer'])!=0;
...
if ($login_type=='user') {
  ...
  if ($result->num_rows==0) {
    debug_mail(false,'empty data 3',$login_username);
    $return_ext['message']=gks_lang('Το όνομα χρήστη ή/και ο κωδικός πρόσβασης είναι λάθος');echo json_encode($return_ext); die();}
  ...
  if (wp_check_password($login_userpass, $user_pass_db, $user_id)==false) {
    debug_mail(false,'empty data 4',$login_userpass);
    //debug_mail(false,'empty data',$login_userpass.'<br>'.$user_pass_db.'<br>'.$user_id);
    $return_ext['message']=gks_lang('Το όνομα χρήστη ή/και ο κωδικός πρόσβασης είναι λάθος');echo json_encode($return_ext); die();}
  ...
  if ($result->num_rows==0) {
    ...
    `erp_app_mobile_can_capture`,`erp_app_mobile_can_sms`,`erp_app_mobile_can_pos`,`erp_app_mobile_pos_list`,`erp_app_mobile_can_transfer`,
  ...
  if ($result->num_rows<>1) {	
    ...
    $return_ext['message']=gks_lang('Το gks ERP App Mobile είναι απενεργοποιημένο');echo json_encode($return_ext); die();}
  ...
  if ($erp_app_mobile_user_token=='') {
    $erp_app_mobile_user_token=rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999).rand(1000,9999);
    $sql="update gks_erp_app_mobile set
    erp_app_mobile_user_token='".$db_link->escape_string($erp_app_mobile_user_token)."'
    where erp_app_mobile_user_id=".$user_id."
    and erp_app_mobile_disabled=0"; 
    $result = $db_link->query($sql);  
    if (!$result) {
...
if ($login_type=='token') {
  ...
  # console or real logFile path like ./frpc.log
  log.to = "/data/user/0/com.gks_gr.gks_erp_app_mobile/files/frpc.log"
  # trace, debug, info, warn, error
  log.level = "debug"
  log.maxDays = 30
...
switch ($cmd) {   
  case 'test_conn':
    ...
    $return['frpc_ini']='';
    $return['port']=54678;
    $return['printers_rpp200']=$return_ext['printers_rpp200'];
    $return['printers_btprinter']=$return_ext['printers_btprinter'];
    ...
    $firebase_token=''; if (isset($input_data['firebase_token'])) $firebase_token=trim_gks($input_data['firebase_token']);
    
    $sql="insert into gks_erp_app_mobile_ping (
    erp_app_mobile_id,mydate,myip,
    appver,osver,rand1,ostime,lanips,phonenumber,
    screw,screh,firebase_token
    ) values (
    ...
    if ($firebase_token!='') {
      $sql.=",firebase_token='".$db_link->escape_string($firebase_token)."'";
    }
    $sql.=" where id_erp_app_mobile=".$id_erp_app_mobile;    
    $result = $db_link->query($sql);  
    if (!$result) {
      debug_mail(false,'error sql',$sql);
      $return['message']='sql error';echo json_encode($return); die();} 
    ...
    $return['can_transfer']=$erp_app_mobile_can_transfer;
    ...
    if ($erp_app_mobile_can_pos==1) {
      ...
      if ($login_type=='user' and $user_id>0) {
        gks_plugins_functions_run('api_gkserpapp_mobile_pos_list',array(
          'extra_where'=>&$extra_where,
          'id_erp_app_mobile' =>$id_erp_app_mobile,
      ...
      $sql="select id_pos,pos_name,def_tropos_pliromis_array from gks_pos 
      where pos_disable=0 ".$extra_where."
      order by pos_name";
      //debug_mail(false,'sql1111111111 a',$sql);
      $result = $db_link->query($sql);  
      if (!$result) {
        debug_mail(false,'error sql',$sql);
        $return['message']='sql error';echo json_encode($return); die();} 
      while ($row = $result->fetch_assoc()) {
        $id_payment_acquirer_ids=[];
        ...
        //debug_mail(false,'sql1111111111 c',print_r($id_payment_acquirer_ids,true));
        $has_viva=false;
        $has_worldline=false;
        if (count($id_payment_acquirer_ids)>0) {
          $sql_pway="SELECT payment_acquirer_with_id
          ...
          //debug_mail(false,'sql1111111111 b',$sql_pway);
          $result_pway = $db_link->query($sql_pway);        
          if (!$result_pway) {
            debug_mail(false,'error sql',$sql_pway);
            $return['message']='sql error';echo json_encode($return); die();} 
          
          while ($row_pway=$result_pway->fetch_assoc()) {
            if (intval($row_pway['payment_acquirer_with_id'])==1) $has_viva=true;
            if (intval($row_pway['payment_acquirer_with_id'])==6) $has_worldline=true;
          }
        }
        ...
        $pos_list[]=array(
          ...
          'has_viva' => $has_viva,
          'has_worldline' => $has_worldline,
        );
      }
    }
  
    $return['pos_list']=$pos_list;
    $return['pos_extra_data']=[];
    $return['pos_extra_data']['viva_merchantKey']='0000000000';
    $return['pos_extra_data']['wordline_version']='1.0.1';
    $return['pos_extra_data']['wordline_app_language_code']='en';
    $return['pos_extra_data']['wordline_merchant_bank_id']=GKS_WORLDLINE_COM_API_BANK_ID;
    $return['pos_extra_data']['wordline_solution_partner_id']=GKS_WORLDLINE_COM_API_PARTNER_ID;
    ...
    if ($appver!=$latest_ver) {
      $return['latest_ver_msg']=gks_lang('Έχετε την έκδοση').' '.$appver."\n".gks_lang('Κάντε αναβάθμιση στην έκδοση').' '.$latest_ver;
    ...
    $return['success']=true;$return['message']=gks_lang('Επιτυχής σύνδεση');echo json_encode($return); die();  
    break;  
  case 'scan':
    ...
    if ($content=='') {
      ...
      $return['message']=gks_lang('Κενό κείμενο. Ξαναδοκιμάστε');echo json_encode($return); die(); }
    ...
    if (!$result) {
      debug_mail(false,'error sql',$sql);
      $return['message']='sql error';echo json_encode($return); die();} 
    $return['success']=true;$return['message']=gks_lang('Επιτυχής αποστολή');echo json_encode($return); die();  
    break;
  case 'sms_receive':
    echo '<pre>error_sms_receive';die();
    ...
    if (count($sms_list)==0) {
      ...
      $return['message']=gks_lang('Δεν έχουν σταλεί τα δεδομένα των SMS που λήφθηκαν');echo json_encode($return); die();} 
    ...
    foreach ($sms_list as $mysms) {
      ...
      $my_status=1000;
      $my_status_name=gks_lang('Λήφθηκε');
    ...
    $return['success']=true;$return['message']=gks_lang('Επιτυχής καταχώρηση');echo json_encode($return); die(); 
  ...
  case 'msg_list':
    ...
    //if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/msg_list'.time().'.txt',print_r($msg_list,true));
    
    if (count($msg_list)==0) {
      debug_mail(false,'msg_list is empty','');
      $return['message']=gks_lang('Δεν έχουν σταλεί τα δεδομένα των SMS');echo json_encode($return); die();} 
    ...
    foreach ($msg_list as $mysms) {
      ...
      if ($folder=='inbox') {
        ...
        switch ($status) {   //TP-Status} value for the message, or -1 if no status has been received.
          ...
          case 64: //STATUS_FAILED
            $my_status=406;
            $my_status_name=gks_lang('Αποτυχία');
            break;
          
          
          default:
            //$my_status=  $status;
            //$my_status_name=gks_lang('Άγνωστο');
        }        
      }
      ...
      if ($id>0) {
        ...
        if ($my_call_from!='all' and $folder=='inbox' and strtotime($date_add) > (time()-24*60*60)) {
          ...
          $mysubject=gks_lang('Νέο SMS από').': '.$myfrom;
          $message_email=gks_lang('Νέο SMS από').': <a href="'.GKS_SITE_URL.'my/admin-sms-chat.php#number='.$myfrom.'">#'.$myfrom.' '.$display_name.'</a><br>'.$message;
          ...
          $message_notification=gks_lang('Νέο SMS από').': <a href="'.GKS_SITE_URL.'my/admin-sms-chat.php#number='.$myfrom.'">#'.$myfrom.'</a><br>'.$message;
  ...
  case 'gps_list':
    ...
    if (count($gps_list)==0) {
      ...
      $return['message']=gks_lang('Δεν έχουν σταλεί δεδομένα GPS');echo json_encode($return); die();} 
    ...
    $return['success']=true;
    $return['message']=gks_lang('Επιτυχής καταχώρηση');
    echo json_encode($return); die(); 
  ...
  case 'getvivaterminalid':
      $return['id_asset']=0;
      $return['asset_code']='';
      $return['asset_title']='';
      $return['viva_terminal_id']='';
      $return['id_company']=0;
      $return['company_eponimia']='';
    
    //print_r($input_data);die();
    $virtualid=''; if (isset($input_data['virtualid'])) $virtualid=trim_gks($input_data['virtualid']);
    if ($virtualid=='') {
      $return['success']=false;
      $return['message']=gks_lang('Δεν έχει ορισθεί το Virtual Terminal ID');
      echo json_encode($return); die();}
    
    $sql="SELECT gks_assets.*, gks_company.id_company, gks_company.company_eponimia, gks_assets.asset_disable
    FROM gks_assets 
    LEFT JOIN gks_company ON gks_assets.viva_company_id = gks_company.id_company
    where gks_assets.viva_terminal_code like '".$db_link->escape_string($virtualid)."'
    ...
    if ($result->num_rows==1) {	
      $row = $result->fetch_assoc();
      $return['id_asset']=intval($row['id_asset']);
      $return['asset_code']=trim_gks($row['asset_code']);
      $return['asset_title']=trim_gks($row['asset_title']);
      $return['viva_terminal_id']=trim_gks($row['viva_terminal_id']);
      $return['id_company']=intval($row['id_company']);
      $return['company_eponimia']=trim_gks($row['company_eponimia']);
      

      $return['message']=gks_lang('Βρέθηκε το Terminal ID και είναι το').' '."\r\n".$return['viva_terminal_id']."\r\n".
      gks_lang('Ως πάγιο η συσκευή είναι η').' '."\r\n".$return['asset_title']."\r\n".
      gks_lang('και ανήκει στην εταιρεία').' '."\r\n".$return['company_eponimia'];
      
      $asset_disable=intval($row['asset_disable']);
      if ($asset_disable==0) {
        $sql="update gks_assets set asset_disable=0 where id_asset=".$return['id_asset'];
        $result = $db_link->query($sql);  
        if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
      }
      ...
      $sql="SELECT *
      FROM gks_company
      WHERE viva_merchant_id<>'' 
      AND viva_api_key<>''
      AND viva_pos_client_id<>''
      AND viva_pos_client_secret<>''
      AND company_disable=0";
      $result = $db_link->query($sql);  
      if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
      if ($result->num_rows==0) {
        $return['success']=false;
        $return['message']=gks_lang('Καμία εταιρεία δεν έχει τις σχετικές ρυθμίσεις για την Viva');
        echo json_encode($return); die();}
      ...
      foreach ($viva_companys as $row_company) {
        ...
        if ($gks_curl_http_code!=200) {
          $return['success']=false;
          $return['message']=gks_lang('Δεν ήταν δυνατή η εποικωνία με το').' '.$url;
          echo json_encode($return); die();}
        ...
        if (!(is_array($response_array) and count($response_array)>0 and isset($response_array[0]['statusId']))) {
          $return['success']=false;
          $return['message']=gks_lang('Λάθος δεδομένα από').' '."\r\n".$url."\r\n\r\n".$response;
          echo json_encode($return); die();}
        
        foreach ($response_array as $myterminal) {
          if ($myterminal['virtualTerminalId']==$virtualid) {
            $return['viva_terminal_id']=$myterminal['terminalId'];
            
            $sql="SELECT gks_assets.*, gks_company.id_company, gks_company.company_eponimia, gks_assets.asset_disable
            FROM gks_assets 
            LEFT JOIN gks_company ON gks_assets.viva_company_id = gks_company.id_company
            where gks_assets.viva_terminal_id like '".$db_link->escape_string($return['viva_terminal_id'])."'
            order by id_asset desc limit 1";
            $result = $db_link->query($sql);  
            if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
            ...
            if ($result->num_rows==1) {	
              ...
              $return['asset_code']=trim_gks($row['asset_code']);
              $return['asset_title']=trim_gks($row['asset_title']);
              $return['id_company']=intval($row_company['id_company']);
              $return['company_eponimia']=trim_gks($row_company['company_eponimia']);
              
              $sql="update gks_assets set 
              viva_terminal_code='".$db_link->escape_string($myterminal['virtualTerminalId'])."',
              asset_disable=0,
              viva_company_id=".$row_company['id_company']."
              where id_asset=".$return['id_asset'];
              $result = $db_link->query($sql);  
              if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
              ...
              gks_lang('Ως πάγιο η συσκευή είναι η').' '."\r\n".$return['asset_title']."\r\n".
              gks_lang('και ανήκει στην εταιρεία').' '."\r\n".$return['company_eponimia']."\r\n".
              gks_lang('Έχει ενημερωθεί και το πάγιο'); 
            ...
            //den vrethike pagio
            if (isset($user_id)==false or $user_id==0) $user_id=2;
            $sql="insert into gks_assets (
              mydate_add,mydate_edit,user_id_add,user_id_edit,myip,
              asset_code,asset_title,asset_type,
              asset_date_activate,
              asset_sxolio,
              asset_last_company_id,
              viva_company_id,
              viva_terminal_id,
              viva_terminal_code
            ) values (
              now(),now(),".$user_id.",".$user_id.",'".$db_link->escape_string($gkIP)."',
              'viva','viva pos ',27,
              '".showDate(time(),'Y-m-d',1)."',
              '".$db_link->escape_string(gks_lang('Αυτόματη εισαγωγή μέσα από το gks ERP App Mobile ID').': '.$id_erp_app_mobile)."',
              ".$row_company['id_company'].",
              ".$row_company['id_company'].",
              '".$db_link->escape_string($return['viva_terminal_id'])."',
              '".$db_link->escape_string($myterminal['virtualTerminalId'])."'
            )";
            $result = $db_link->query($sql);  
            if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
            $id_asset = intval($db_link->insert_id); 
            
            $sql="update gks_assets set 
            asset_code='viva".$id_asset."',
            asset_title='viva ".$return['viva_terminal_id']."'
            where id_asset=".$id_asset;
            $result = $db_link->query($sql);  
            if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
            
            
            $sql="insert into gks_assets_log (
              asset_id,add_date, user_id, sxolio
            ) values (
              ".$id_asset.",
              now(),
              ".$user_id.",
            ...
            $return['id_asset']=intval($id_asset);
            $return['asset_code']='viva'.$id_asset;
            $return['asset_title']='viva '.$return['viva_terminal_id'];
            $return['id_company']=intval($row_company['id_company']);
            $return['company_eponimia']=trim_gks($row_company['company_eponimia']);

            $return['message']=gks_lang('Βρέθηκε το Terminal ID και είναι το').' '."\r\n".$return['viva_terminal_id']."\r\n".
            gks_lang('Ως πάγιο η συσκευή προστέθηκε και είναι η').' '."\r\n".$return['asset_title']."\r\n".
            gks_lang('και ανήκει στην εταιρεία').' '."\r\n".$return['company_eponimia']."\r\n".
            gks_lang('Ενημερώστε το πάγιο και με άλλες λεπτομέρεις π.χ. φωτογραφία, μέσω του gks ERP'); 
            $return['success']=true;
            echo json_encode($return); die();
      ...
      $return['message']='OK111';
      
    }
      
    if ($return['message']=='OK111') {
      $return['success']=false;
      $return['message']=gks_lang('Είναι σίγουρα το viva στην εταιρεία που πρέπει ;');
  ...
  case 'getworldlineterminalid':
      $return['id_asset']=0;
      $return['asset_code']='';
      $return['asset_title']='';
      $return['worldline_terminal_id']='';
      $return['id_company']=0;
      $return['company_eponimia']='';
    ...
    if ($virtualid=='') {
      $return['success']=false;
      $return['message']=gks_lang('Δεν έχει ορισθεί το Device ID');
      echo json_encode($return); die();}
    
    $terminalid=''; if (isset($input_data['terminalid'])) $terminalid=trim_gks($input_data['terminalid']);
    if ($terminalid=='') {
      $return['success']=false;
      $return['message']=gks_lang('Δεν έχει ορισθεί το Terminal ID');
      echo json_encode($return); die();}
    $return['worldline_terminal_id']=$terminalid;
    
    $merchantid=''; if (isset($input_data['merchantid'])) $merchantid=trim_gks($input_data['merchantid']);
    if ($merchantid=='') {
      $return['success']=false;
      $return['message']=gks_lang('Δεν έχει ορισθεί το Merchant ID');
      echo json_encode($return); die();}

    $sql="SELECT *
    FROM gks_company
    WHERE worldline_mid='".$db_link->escape_string($merchantid)."' 
    AND worldline_username<>''
    AND company_disable=0";
    $result = $db_link->query($sql);  
    if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
    if ($result->num_rows==0) {
      $return['success']=false;
      $return['message']=gks_lang('Δεν βρέθηκε ενεργή εταιρεία με Merchant ID').': '.$merchantid;
      echo json_encode($return); die();}
    
    $row_company = $result->fetch_assoc();
    $id_company=intval($row_company['id_company']);
    $company_title=trim($row_company['company_title']);
    if (isset($user_id)==false or $user_id==0) $user_id=2;
    
    $sql="SELECT gks_assets.*, gks_company.id_company, gks_company.company_eponimia, gks_assets.asset_disable,
    gks_company.worldline_mid
    FROM gks_assets 
    LEFT JOIN gks_company ON gks_assets.worldline_company_id = gks_company.id_company
    where gks_assets.worldline_id like '".$db_link->escape_string($virtualid)."'
    order by id_asset desc limit 1";
    $result = $db_link->query($sql);  
    if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
    ...
    if ($result->num_rows==1) {	
      $row = $result->fetch_assoc();
      $worldline_mid=trim_gks($row['worldline_mid']);
      
      $return['id_asset']=intval($row['id_asset']);
      $return['asset_code']=trim_gks($row['asset_code']);
      $return['asset_title']=trim_gks($row['asset_title']);
      $return['worldline_terminal_id']=trim_gks($row['worldline_terminal_id']);
      $return['id_company']=intval($row['id_company']);
      $return['company_eponimia']=trim_gks($row['company_eponimia']);
      $asset_last_company_id=intval($row['asset_last_company_id']);
      ...
      if (($worldline_mid!='' and $merchantid!=$worldline_mid) or 
          ($id_company>0 and $id_company!=$return['id_company']) or 
          ($id_company>0 and $id_company!=$asset_last_company_id)) {
        $sql="update gks_assets set 
        asset_last_company_id=".$id_company.",
        worldline_company_id=".$id_company."
        where id_asset=".$return['id_asset'];
        $result = $db_link->query($sql);  
        if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
        
        $return['message'].="\r\n".gks_lang('Τώρα έχει μεταφερθεί στην εταιρεία').' '.$company_title;
        
        $tmpmsg=gks_lang('Μεταφορά στην εταρεία [1] μέσα από το gks ERP App Mobile ID: [2]');
        $tmpmsg=str_replace('[1]',$company_title,$tmpmsg);
        $tmpmsg=str_replace('[2]',$id_erp_app_mobile,$tmpmsg);
        ...
        $sql="insert into gks_assets_log (
          ...
          now(),
          ".$user_id.",
          '".$db_link->escape_string($tmpmsg)."'
        )";
        $result = $db_link->query($sql);  
        if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }  

        $sql="insert into gks_assets_moves (
        asset_id,company_id,mydate,user_id_add,action_myip
        ) values (
        ".$return['id_asset'].",".$id_company.",now(),".$user_id.",'".$db_link->escape_string($gkIP)."'
        )";
        $result = $db_link->query($sql);  
        if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }  
      ...
      $sql="insert into gks_assets (
        ...
        asset_sxolio,
        asset_last_company_id,
        worldline_company_id,
        worldline_id,
        worldline_terminal_id
      ) values (
        now(),now(),".$user_id.",".$user_id.",'".$db_link->escape_string($gkIP)."',
        'worldline','worldline pos ',27,
        '".showDate(time(),'Y-m-d',1)."',
        '".$db_link->escape_string(gks_lang('Αυτόματη εισαγωγή μέσα από το gks ERP App Mobile ID').': '.$id_erp_app_mobile)."',
        ".$row_company['id_company'].",
        ".$row_company['id_company'].",
        '".$db_link->escape_string($virtualid)."',
        '".$db_link->escape_string($terminalid)."'
      )";
      $result = $db_link->query($sql);  
      if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
      $id_asset = intval($db_link->insert_id); 
      ...
      $sql="update gks_assets set 
      asset_code='worldline".$id_asset."',
      asset_title='worldline ".$terminalid."'
      where id_asset=".$id_asset;
      $result = $db_link->query($sql);  
      if (!$result) {debug_mail(false,'error sql',$sql);echo 'sql error'; die(); }     
      ...
      $return['id_asset']=intval($id_asset);
      $return['asset_code']='worldline'.$id_asset;
      $return['asset_title']='worldline '.$terminalid;
      $return['id_company']=intval($row_company['id_company']);
      $return['company_eponimia']=trim_gks($row_company['company_eponimia']);
      ...
      gks_lang('Ως πάγιο η συσκευή προστέθηκε και είναι η').' '."\r\n".$return['asset_title']."\r\n".
      gks_lang('και ανήκει στην εταιρεία').' '."\r\n".$return['company_eponimia']."\r\n".
      gks_lang('Ενημερώστε το πάγιο και με άλλες λεπτομέρεις π.χ. φωτογραφία, μέσω το gks ERP'); 
      $return['success']=true;
      echo json_encode($return); die();
...
//    if ($return['message']=='OK111') {
//      $return['success']=false;
//      $return['message']=gks_lang('Είναι σίγουρα το worldline στην εταιρεία που πρέπει ;');
//    } else {
      $return['success']=true;
//    }
  ...
  case 'worldlineapp2appres':
    $mytoast=''; if (isset($input_data['mytoast'])) $mytoast=trim_gks($input_data['mytoast']);
    $myresultCode=''; if (isset($input_data['myresultCode'])) $myresultCode=trim_gks($input_data['myresultCode']);
    $mydata=''; if (isset($input_data['mydata'])) $mydata=trim_gks($input_data['mydata']);
    $res_intent_result=''; if (isset($input_data['res_intent_result'])) $res_intent_result=trim_gks($input_data['res_intent_result']);
    $res_trn_receipt=''; if (isset($input_data['res_trn_receipt'])) $res_trn_receipt=trim_gks($input_data['res_trn_receipt']);
    $res_app_version=''; if (isset($input_data['res_app_version'])) $res_app_version=trim_gks($input_data['res_app_version']);
    $res_token=''; if (isset($input_data['res_token'])) $res_token=trim_gks($input_data['res_token']);
    $res_merchant_device_id=''; if (isset($input_data['res_merchant_device_id'])) $res_merchant_device_id=trim_gks($input_data['res_merchant_device_id']);
    $res_server_message=''; if (isset($input_data['res_server_message'])) $res_server_message=trim_gks($input_data['res_server_message']);
    $res_merchant_device_status_code=''; if (isset($input_data['res_merchant_device_status_code'])) $res_merchant_device_status_code=trim_gks($input_data['res_merchant_device_status_code']);
    $res_protocol_version=''; if (isset($input_data['res_protocol_version'])) $res_protocol_version=trim_gks($input_data['res_protocol_version']);
    $res_terminal_status=''; if (isset($input_data['res_terminal_status'])) $res_terminal_status=trim_gks($input_data['res_terminal_status']);


    if (isset($user_id)==false or $user_id==0) $user_id=2;

    $sql="insert into gks_worldline_transaction_app2app_res (
    mydate_add,mydate_edit,user_id_add,user_id_edit,myip,
    erp_app_mobile_id,user_id,
    mytoast,
    myresultCode,
    mydata,
    res_intent_result,
    ...
    res_app_version,
    res_token,
    res_merchant_device_id,
    res_server_message,
    res_merchant_device_status_code,
    res_protocol_version,
    res_terminal_status
    ) values (
    now(),now(),".$user_id.",".$user_id.",'".$db_link->escape_string($gkIP)."',
    ".$id_erp_app_mobile.",".$user_id.",
    '".$db_link->escape_string($mytoast)."',
    '".$db_link->escape_string($myresultCode)."',
    '".$db_link->escape_string($mydata)."',
    '".$db_link->escape_string($res_intent_result)."',
    '".$db_link->escape_string($res_trn_receipt)."',
    '".$db_link->escape_string($res_app_version)."',
    '".$db_link->escape_string($res_token)."',
    '".$db_link->escape_string($res_merchant_device_id)."',
    '".$db_link->escape_string($res_server_message)."',
    '".$db_link->escape_string($res_merchant_device_status_code)."',
    '".$db_link->escape_string($res_protocol_version)."',
    '".$db_link->escape_string($res_terminal_status)."'
    )";    
    $result = $db_link->query($sql);  
    if (!$result) {
      debug_mail(false,'error sql',$sql);
    ...
    if ($res_intent_result=='INTENT-0') {   
      //ola kala, min kaneis kati  
    } else if (substr($res_intent_result, 0,7)=='INTENT-') {
      if ($res_token!='') {
        $sql="select * from gks_worldline_transaction 
        where intentId='".$res_token."'
        and myStatus=1 and myResult=5";
        ...
        if ($result->num_rows==1) {	
          ...
          $row = $result->fetch_assoc();        
          $id_worldline_transaction=$row['id_worldline_transaction'];
          $eftpos_transaction_id=intval($row['eftpos_transaction_id']);
          
          $myerror=gks_eftpos_transaction_result_text_worldline_app2app($res_intent_result);;
          $myResult=6;
          if ($res_intent_result=='INTENT-968') {
            $myerror=gks_lang('Ακυρώθηκε από τον χρήστη'); //Cardholder rejected transaction
            $myResult=103;
          }
          ...
          where intentId='".$res_token."'
          and myStatus=1 and myResult=5
          and id_worldline_transaction=".$id_worldline_transaction;
          ...
          if ($eftpos_transaction_id>0) {
            $sql="select * from gks_eftpos_transaction 
            where id_eftpos_transaction=".$eftpos_transaction_id."
            and payment_acquirer_with_id=6";
            ...
            if ($result->num_rows==1) {	
              ...
              AND id_eftpos_transaction=".$eftpos_transaction_id;
              //debug_mail(false,'sql4',$sql);
              $result = $db_link->query($sql);        
              if (!$result) {
                debug_mail(false,'error sql',$sql);
                $return['message']='sql error';echo json_encode($return); die();} 
              ...
              if (in_array($transaction_type,['sale','saleerp'])) {
                ...
                AND gks_eftpos_transaction.id_eftpos_transaction=".$eftpos_transaction_id;
                //debug_mail(false,'sql5',$sql);
                $result = $db_link->query($sql);        
                if (!$result) {
                  debug_mail(false,'error sql',$sql);
                  $return['message']='sql error';echo json_encode($return); die();} 
    ...
    $return['success']=true;$return['message']='OK';echo json_encode($return); die();  
    break;
      
  default:
    debug_mail(false,'cmd error',$cmd);
    $return['message']=gks_lang('Λάθος εντολή');echo json_encode($return); die();  
    break;      
}
