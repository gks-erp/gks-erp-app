...
$my_page_title=gks_lang('Παρακρατούμενοι Φόροι');
...
$sortable = array(
		array('name' => 'soid', 'field' => 'id_aade_katigoria_parakratoumemenon_foron'),
		array('name' => 'socode', 'field' => 'aade_katigoria_parakratoumemenon_foron_code'),
		array('name' => 'sodescr', 'field' => 'aade_katigoria_parakratoumemenon_foron_descr'),
		array('name' => 'sotype', 'field' => 'aade_katigoria_parakratoumemenon_foron_type'),
		array('name' => 'sopososto', 'field' => 'aade_katigoria_parakratoumemenon_foron_pososto'),
		array('name' => 'sofn', 'field' => 'aade_katigoria_parakratoumemenon_foron_poso_fn'),
		array('name' => 'sofix', 'field' => 'aade_katigoria_parakratoumemenon_foron_poso_fix'),
		array('name' => 'sosort', 'field' => 'sortorder'),
		array('name' => 'sodisable', 'field' => 'aade_disable'),
		array('name' => 'sopeppol', 'field' => 'parakrat_peppol_code'),   
);



$search_fields = array(
'aade_katigoria_parakratoumemenon_foron_descr',
'parakrat_peppol_code',
);
...
if (empty($sorted['sql'])) {
	$sql .= " ORDER BY sortorder,aade_katigoria_parakratoumemenon_foron_descr,id_aade_katigoria_parakratoumemenon_foron";
} else {
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-aade-katigoria-parakratoumemenon-foron-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέου παρακρατούμενου φόρου');?></a>
...
<table class="table table-sm table-responsive1 table-striped table-bordered gkstable <?php
  ...
  ?>" border="0" cellspacing="0" cellpadding="5" align="center" id="table_gks_aade_katigoria_parakratoumemenon_foron">
<thead>
  <tr >	
...
<tbody>
	...
	$i++;
?>
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>" data-id="<?php echo $row['id_aade_katigoria_parakratoumemenon_foron'];?>">
    <th class="mytdcm" scope="row" nowrap><?php echo ($i + $showFrom);?></th>
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-aade-katigoria-parakratoumemenon-foron-item.php?id=<?php echo $row['id_aade_katigoria_parakratoumemenon_foron'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_aade_katigoria_parakratoumemenon_foron'];?></td>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_aade_katigoria_parakratoumemenon_foron'];?>" data-model="gks_aade_katigoria_parakratoumemenon_foron"></i></td>
        </tr>      
      </table>
    </td>    
    <td class="mytdcml"><?php echo $row['aade_katigoria_parakratoumemenon_foron_descr'];?></td>
    <td class="mytdcm"><?php echo $row['aade_katigoria_parakratoumemenon_foron_code'];?></td>
    <td class="mytdcm"><?php echo $row['aade_katigoria_parakratoumemenon_foron_type'];?></td>
    <td class="mytdcm"><?php echo $row['aade_katigoria_parakratoumemenon_foron_pososto'];?></td>
    <td class="mytdcml"><?php echo $row['aade_katigoria_parakratoumemenon_foron_poso_fn'];?></td>
    <td class="mytdcm"><?php echo $row['aade_katigoria_parakratoumemenon_foron_poso_fix'];?></td>
<?php if ($perm_edit) {?>
...
<?php } ?>
    ...
    <td class="mytdcml div_peppol" data-id="<?php echo $row['id_aade_katigoria_parakratoumemenon_foron'];?>"><?php echo $row['parakrat_peppol_code'];?></td>
  ...
  <div class="row align-items-center">
    <div class="col-sm-12" style="text-align:center">
      <p><?php echo gks_lang('Για τους κωδικούς Peppol δείτε τον παρακάτω σύνδεσμο');?>:<br>
        <a href="https://docs.peppol.eu/poacc/billing/3.0/codelist/UNCL5189/" target="_blank"
          >https://docs.peppol.eu/poacc/billing/3.0/codelist/UNCL5189/</a>
      </p>
    </div>
  </div>
</div>
...
jQuery(document).ready(function($) {
  ...
  $('#table_gks_aade_katigoria_parakratoumemenon_foron > tbody').sortable({
    handle: '.sortorder_handle',
    update: function( event, ui ) {
      mylist = $(this).sortable('toArray', {attribute: 'data-id'});
      gks_sortorder_obj('gks_aade_katigoria_parakratoumemenon_foron',mylist,'#table_gks_aade_katigoria_parakratoumemenon_foron > tbody');
    }
  }); 
  <?php if ($perm_edit) {?>
  ...
  function save_peppol_click() {
    ...
    gks_field_from_table_save('gks_aade_katigoria_parakratoumemenon_foron',myid,'parakrat_peppol_code',mytext, false);
  }
  window.gks_field_from_table_save_after=function(mydata) {
    if (mydata.table_name!='gks_aade_katigoria_parakratoumemenon_foron') return;
    if (mydata.table_id<=0) return;
    if (mydata.field_name!='parakrat_peppol_code') return;
