...
$my_page_title=gks_lang('Εντολή Πρότυπου SMS');
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_sms_viber_template','autocomplete',$id);
...
$sql="select * from gks_sms_viber_template where id_sms_viber_template=".$id." limit 1";
...
if ($cmd=='getparams') {
  $sms_params='<table class="table table-sm table-responsive table-striped table-bordered" border="0" style="width:100%" cellspacing="0" cellpadding="5" align="center">'.
  '<thead>'.
    '<tr>'.
...
//    '</tr>';
  ...
  foreach ($params as $vparam) {
    ...
    if ($vparam['type']=='text') {
      $sms_params.='<input id="'.$vparam['id'].'" type="'.$vparam['type'].'" class="form-control form-control-sm" data-jqs="'.base64_encode($jquery_selector).'" value="';
      if (isset($vparam['value'])) $sms_params.=$vparam['value'];
      $sms_params.='">';
    } else if ($vparam['type']=='textarea') { 
      $sms_params.='<textarea style="height:200px; width:100%" id="'.$vparam['id'].'" type="text" class="form-control form-control-sm" data-jqs="'.base64_encode($jquery_selector).'">';
      if (isset($vparam['value'])) $sms_params.=$vparam['value'];
      $sms_params.='</textarea>';
    }
    $sms_params.='</td>'. 
...
/*
$ddd=[];
$ddd[]=array('def_check'=>true,'basefolder'=>'erpfi','relative_path'=>'base/users/31413/NIPT TRF Request Consent Form_ ENG FINAL.pdf','name_for_sms'=>'');
$ddd[]=array('def_check'=>true,'basefolder'=>'erpfi','relative_path'=>'base/users/31413/Prequel_NIPT Request Consent Form_ for laboratory ENG FINAL.pdf','name_for_sms'=>'');
$ddd[]=array('def_check'=>true,'basefolder'=>'erpfi','relative_path'=>'base/users/31413/prequel_report_eng_SHRO.pdf','name_for_sms'=>'');
$ddd[]=array('def_check'=>true,'basefolder'=>'erpfi','relative_path'=>'base/users/31413/Prequel Flyer_2025.pdf','name_for_sms'=>'');
$ddd[]=array('def_check'=>false,'basefolder'=>'erpfi','relative_path'=>'base/users/31413/Prequel_Payment instructions.pdf','name_for_sms'=>'neo onoma');
$ddd[]=array('def_check'=>true,'basefolder'=>'erpfi','relative_path'=>'base/users/31413/nikon_d7000.jpg','name_for_sms'=>'');
...
//            //if ($value['name_for_sms']!='') $atta_name=$value['name_for_sms'];
//
//            $url='';
//            $img_thump='';
//            $url_thump='';
...
//            }
//            $sms_attachments.=
//            '<tr class="gks_sms_attachments_item">'.
//              '<td style="width:50px;text-align: center;vertical-align: middle;">'.
//                '<input '.($value['def_check'] ? 'checked' : '').' data-basefolder="'.$value['basefolder'].'" type="checkbox" class="dialog_item_message_sms_attachments_checkbox" data-path="'.$value['relative_path'].'">'.
//              '</td>'.
...
//  }
  ...
  $return = array(
    ...
    'sms_params'=>base64_encode($sms_params),
    'sms_subject' => '', //base64_encode($row['sms_subject']),
    'sms_message' => base64_encode($row['sms_viber_template_text']),
    'sms_attachments' => base64_encode($sms_attachments),
  );
  echo json_encode($return); die();  
