...
$is_from_cron=false;
if (isset($_GET['cron']) and $_GET['cron']=='1') $is_from_cron=true; 
if ($is_from_cron==false) {
...
if ($id<=0) {
  debug_mail(false,'admin-orders-item-link-action_start.php the id is not set','');
  die();}


$my_page_title=gks_lang('Έναρξη λήψης αρχείου παραγγελίας').' '.$id;
db_open();
stat_record();
...
$sql="select * from gks_orders_links where id_order_links=".$id;
...
if ($result->num_rows <= 0) {  
  debug_mail(false,'admin-orders-item-link-action_start.php record not found',$sql);
  die();}
...
if ($row['download_status']!=0) {
  if ($row['download_status']!=2) { // einai idi OK
    $sql="update gks_orders_links set download_end=now(), 
    download_message='".$db_link->escape_string(gks_lang('Η κατάσταση του συνδέσμου δεν είναι η σωστή'))."'
    where id_order_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();} 
  } 
  debug_mail(false,'admin-orders-item-link-action_start.php download_status='.$row['download_status'].' not 0', $sql);
  die();}
...
if ($url=='' or filter_var($url, FILTER_VALIDATE_URL) === FALSE) {
  ...
  $result = $db_link->query($sql);
  if (!$result) {debug_mail(false,'error sql',$sql);die();}  
  debug_mail(false,'admin-orders-item-link-action_start.php url is valid',$url);
  die();} 


$sql="update gks_orders_links set 
download_status=1, 
download_start=now(),
download_end=null, 
...
if (file_exists(GKS_FileServerShare) == false) {
  $sql="update gks_orders_links set html_tds=null,download_end=now(), 
...
$upload_dir = GKS_FileServerShare.'order/'.$order_id.'/';

if (file_exists($upload_dir) == false) {
  if (@mkdir($upload_dir , 0777, true) == false ) {
    $sql="update gks_orders_links set html_tds=null,download_end=now(), 
    download_message='".$db_link->escape_string(gks_lang('Δεν μπορεί να δημιουργηθεί ο φάκελος').' ../@orders/'.$order_id)."' 
    where id_order_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();}  
    debug_mail(false,'admin-orders-item-link-action_start.php can not create dir: ',$upload_dir);
    die(); 
  }
}
//echo '<pre>';print time();die();


//http://www.mediafire.com/file/q26nf2wcec7tzxd/HelBaptPanag.rar/file
//https://drive.google.com/uc?export=download&id=15axd7wJkJ43jw6GPzCnwwrPo9SdSuIC7
// OK https://mega.nz/folder/tvQB2QSD#uS27gGakXUb1rsauX_Wm7w
//http://plisitis.quickconnect.to/d/f/539384346075773450
//https://app.box.com/s/08agutb5u5xbqrkvjx1fo3hb668bmh62
//https://boards.wetransfer.com/board/sfk6w33qekerlgire20191008154900/latest?token=51977419-63c3-435e-bb68-c3cebf673e11
//https://fil.email/bpKbnyYl
// can not https://mail.google.com/mail/u/0/#inbox/162488e5a38b3ccc
// can not https://mail.yahoo.com/d/folders/1/messages/6639?action=showLetter&umid=2_0_0_1_458002_AJ8JDNkAAeC3Vyi48A5%208GHHnpY&box=Inbox&src=hp
// OK https://photographystudioiosifina.wetransfer.com/downloads/3f6cb3cae18e0e39b1e3d808964ae5ea20190128095557/181723e2166c38b31698e63e3b12a23520190128095557/2b7a4a
//https://send-anywhere.com/web/downloads/803ILSCP
...
if (startwith(strtolower($url),'https://mega.nz/') and strlen($url)>=17) {
  ...
  if (@mkdir($upload_dir2 , 0777, true) == false ) {
    ...
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();}  
    debug_mail(false,'admin-orders-item-link-action_start.php can not create dir: ',$upload_dir2);
    die(); 
  }
...
//  debug_mail(false,'exec4',print_r($return_var2,true));
  ...
  //debug_mail(false,'upload_dir2',$upload_dir2);
  ...
  if (copy (GKS_SITE_PATH.GKS_SITE_HTTPDOCS.'/my/utils/transferwee.py', $upload_dir2.'transferwee.py') == false) {
    ...
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();}  
    debug_mail(false,'admin-orders-item-link-action_start.php can not copy file transferwee.py in dir: ',$upload_dir2);
    die();    
  }
  $relative_path=substr($upload_dir2, strlen($upload_dir));
  $sql="update gks_orders_links set 
...
//  die();
  ...
  //debug_mail(false,'extract_path',$extract_path);
  //debug_mail(false,'upload_dir2',$upload_dir2.'<br>'.$targetFile_final);
...
function progressCallback($resource, $download_size, $downloaded_size, $upload_size, $uploaded_size ) {
  ...
  if ( $progress > $previousProgress) {
    ...
    $sql="select download_status from gks_orders_links where id_order_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();} 
    if ($result->num_rows==0) {
