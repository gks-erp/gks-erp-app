...
$sql="select * from gks_acc_pay_links where acc_pay_id=".$id." and (download_status=1 or (download_status in (2,3) and download_end > date_sub(now(), interval 1 minute)))";
...
while ($row = $result->fetch_assoc()) {
  ...
  if ($download_size_until_now==0 and $relative_path!= '') {
    $path_scan=GKS_FileServerShare.'acc/pay/'.$id.'/'.$relative_path;
  ...
  $data[] =array(
    'id' => intval($row['id_acc_pay_links']),
