...
//include_once('functions_viva.php');

//echo '<pre>';
// /my/admin-viva-refresh.php?today=1
// /my/admin-viva-refresh.php?days=0


$my_page_title=gks_lang('Viva Refresh'); 
db_open();
stat_record();

$sql="SELECT id_company
FROM gks_company
WHERE viva_merchant_id<>'' AND viva_api_key<>'' AND viva_verify_webhook_page_key<>'' AND company_disable=0
...
if (isset($_GET['today'])) {
  ...
  foreach ($id_company_ids as $id_company) {
    for($i=0;$i<=1;$i++) {
      gks_viva_api_get_transactions($i,$id_company,'');
