...
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_crm_leads','view',$id);
if ($perm_ret['success']==false) {$return = array('success' => false, 'message' => base64_encode($perm_ret['message']));echo json_encode($return); die();}



$sql="select * from gks_crm_leads_links where crm_lead_id=".$id." and (download_status=1 or (download_status in (2,3) and download_end > date_sub(now(), interval 1 minute)))";
...
while ($row = $result->fetch_assoc()) {
  ...
  if ($download_size_until_now==0 and $relative_path!= '') {
    $path_scan=GKS_FileServerShare.'crm/lead/'.$id.'/'.$relative_path;
  ...
  $data[] =array(
    'id' => intval($row['id_crm_leads_links']),
