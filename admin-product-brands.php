...
$my_page_title=gks_lang('Μάρκες Ειδών');
$nav_active_array=array('manage','manage_menu_product','manage_product_brands');


db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_eshop_products_brands','view',0);
...
$gks_custom_prepare = gks_custom_table_item_prepare('gks_eshop_products_brands',['from'=>'list']);
...
$filters[] = array(
    ...
    'vals' => array(
        ...
        array('value' => 1, 'text' => gks_lang('Ενεργή'),     'sql' => "gks_eshop_products_brands.brand_disable = 0"),
        array('value' => 2, 'text' => gks_lang('Μη ενεργή'),  'sql' => "gks_eshop_products_brands.brand_disable <> 0"),
    ),
);
$filters=array_merge($filters,$gks_custom_prepare['sql_filters']);

$sortable = array(
  						array('name' => 'soid', 'field' => 'gks_eshop_products_brands.id_product_brand'),
  						array('name' => 'sodescr', 'field' => 'gks_eshop_products_brands.product_brand_descr'),
  						array('name' => 'socount', 'field' => 'ccc'),
  						array('name' => 'soadd', 'field' => 'gks_eshop_products_brands.mydate_add'),
  						array('name' => 'sodisable', 'field' => 'gks_eshop_products_brands.brand_disable'),
  						array('name' => 'sophoto', 'field' => 'gks_eshop_products_brands.brand_photo'),
            );
...
$sql = "SELECT SQL_CALC_FOUND_ROWS gks_eshop_products_brands.*, ccproducts.ccc,
...
FROM ".$gks_custom_prepare['sql_all_list_from']." (((((((((gks_eshop_products_brands
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-product-brands-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέας μάρκας ειδών');?></a>
...
<thead>
    <tr >	
        ...
        <th nowrap class="table-dark" scope="col" style="text-align: left   !important;" width="20%"><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sodescr', gks_lang('Μάρκα')); ?></th>        
  ...
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>">
    ...
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-product-brands-item.php?id=<?php echo $row['id_product_brand'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_product_brand'];?></td>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_product_brand'];?>" data-model="gks_eshop_products_brands"></i></td>
        </tr>      
      </table>
    </td>

    <td class="tdimg"><?php 
    $myimgurl=trim_gks($row['brand_photo'].'');
    if ($myimgurl == '') {
    ...
    <td class="mytdcml"><?php echo $row['product_brand_descr'];?></td>
    ...
    <td nowrap class="mytdcm"><?php
      ...
      $sql="SELECT ug1.id_product_brand AS gid1, 
                   ...
                   ug5.id_product_brand AS gid6,
                   ug5.id_product_brand AS gid7,
                   ug5.id_product_brand AS gid8,
                   ug5.id_product_brand AS gid9,
                   ug5.id_product_brand AS gid10
      ...
      where ug1.id_product_brand=".$row['id_product_brand'];
      ...
      if (strlen($gu_in)>0) {
        $sql="SELECT count(Distinct product_id) as ccc2 FROM gks_eshop_products_brands_products WHERE product_brand_id In (".$gu_in.")";
        $result_gu = $db_link->query($sql);        
        if (!$result_gu) {
          debug_mail(false,'error sql',$sql);
          die('sql error');
    ...
    <td class="mytdcml"><?php echo nl2br_gks($row['brand_comments']);?></td>
    <td nowrap class="mytdcm"><?php echo showDate(strtotime($row['mydate_add']), 'd/m/Y\<\b\r\> H:i:s', 1);?></td>   
    
    <td class="mytdcm"><?php echo myimg010r($row['brand_disable']);?></td>
<?php
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_eshop_products_brands','edit',  0);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_eshop_products_brands','add',   0);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_eshop_products_brands','delete',0);?>;
