...
$my_page_title=gks_lang('Αναφορά - Pivot Table - Δελτία - Λήψη δεδομένων').'...';
db_open();
stat_record();
...
$gks_custom_prepare = gks_custom_table_item_prepare('gks_whi_mov',['from'=>'pivot']);
$gks_custom_header_fields = gks_custom_table_pivot_header($gks_custom_prepare);



if (isset($_gks_session['temp']['wherepivot8']) and $_gks_session['temp']['wherepivot8']!='') {
  $sql=$_gks_session['temp']['wherepivot8'];
} else {
  die();  
}
...
$file_path='pivot8_'.$my_wp_user_id.'_'.rand(100000,999999).'.csv';
...
'"'.gks_lang('Επαφή').'","'.gks_lang('Πόλη').'","'.gks_lang('Νομός').'","'.gks_lang('Χώρα').'","'.gks_lang('Δελτίο').'",'.
'"'.gks_lang('Τρόπος Αποστολής').'",'.
'"'.gks_lang('Ποσότητα').'","'.gks_lang('Ανάθεση').'",';
