...
$my_page_title=gks_lang('epay Log');
...
if ($id>0) {
  $sql="select id_eftpos_transaction,xxx_transaction_id,send_array,response_array from gks_eftpos_transaction where id_eftpos_transaction=".$id." and payment_acquirer_with_id=5";
  ...
  $sql="select id_eftpos_transaction,xxx_transaction_id,send_array,response_array from gks_eftpos_transaction where xxx_transaction_id=".$mtid." and payment_acquirer_with_id=5";
  //echo $sql;die();
  $result = $db_link->query($sql);
  if (!$result) { debug_mail(false,'error sql',$sql);die('sql error');}  
  if ($result->num_rows==0) {
...
if ($xxx_transaction_id>0) {
  $sql="select myjson from gks_epay_transaction where id_epay_transaction=".$xxx_transaction_id;
  $result = $db_link->query($sql);
  if (!$result) { debug_mail(false,'error sql',$sql);die('sql error');}  
  if ($result->num_rows==0) {
