...
$my_page_title=gks_lang('Αφαίρεση φωτογραφίας από είδος');
db_open();
stat_record();

$sql="select * from gks_eshop_products_photo where id_product_photo=".$id;
...
$sql="delete from gks_eshop_products_photo where id_product_photo=".$id;
$result = $db_link->query($sql);        
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Σφάλμα SQL').'<br>'.gks_lang('Παρακαλώ δοκιμάστε αργότερα')));
