...
$sortable = array(
		array('name' => 'soid', 'field' => 'id_aade_katigoria_fpa_ejeresi'),
		array('name' => 'socode', 'field' => 'aade_katigoria_fpa_ejeresi_code'),
		array('name' => 'sodescr', 'field' => 'aade_katigoria_fpa_ejeresi_descr'),
		array('name' => 'sosort', 'field' => 'sortorder'),
		array('name' => 'sodisable', 'field' => 'aade_disable'),
		array('name' => 'sopeppol', 'field' => 'fpa_ejeresi_peppol_code'),   
);



$search_fields = array(
'aade_katigoria_fpa_ejeresi_descr',
'fpa_ejeresi_peppol_code',
);
...
if (empty($sorted['sql'])) {
	$sql .= " ORDER BY sortorder,aade_katigoria_fpa_ejeresi_descr,id_aade_katigoria_fpa_ejeresi";
} else {
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-aade-katigoria-fpa-ejeresi-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέας αιτίας εξαίρεσης ΦΠΑ');?></a>
...
<table class="table table-sm table-responsive1 table-striped table-bordered gkstable <?php
  ...
  ?>" border="0" cellspacing="0" cellpadding="5" align="center" id="table_gks_aade_katigoria_fpa_ejeresi">
<thead>
  <tr >	
...
<tbody>
	...
	$i++;
?>
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>" data-id="<?php echo $row['id_aade_katigoria_fpa_ejeresi'];?>">
    <th class="mytdcm" scope="row" nowrap><?php echo ($i + $showFrom);?></th>
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-aade-katigoria-fpa-ejeresi-item.php?id=<?php echo $row['id_aade_katigoria_fpa_ejeresi'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_aade_katigoria_fpa_ejeresi'];?></td>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_aade_katigoria_fpa_ejeresi'];?>" data-model="gks_aade_katigoria_fpa_ejeresi"></i></td>
        </tr>      
      </table>
    </td>
    <td class="mytdcml"><?php echo $row['aade_katigoria_fpa_ejeresi_descr'];?></td>
    <td class="mytdcm"><?php echo $row['aade_katigoria_fpa_ejeresi_code'];?></td>
<?php if ($perm_edit) {?>
...
<?php } ?>
    ...
    <td class="mytdcml div_peppol" data-id="<?php echo $row['id_aade_katigoria_fpa_ejeresi'];?>"><?php echo $row['fpa_ejeresi_peppol_code'];?></td>
  ...
  <div class="row align-items-center">
    <div class="col-sm-12" style="text-align:center">
      <p><?php echo gks_lang('Για τους κωδικούς Peppol δείτε τον παρακάτω σύνδεσμο');?>:<br>
        <a href="https://docs.peppol.eu/poacc/billing/3.0/codelist/vatex/" target="_blank"
          >https://docs.peppol.eu/poacc/billing/3.0/codelist/vatex/</a>
      </p>
    </div>
  </div>
</div>
...
jQuery(document).ready(function($) {
  ...
  $('#table_gks_aade_katigoria_fpa_ejeresi > tbody').sortable({
    handle: '.sortorder_handle',
    update: function( event, ui ) {
      mylist = $(this).sortable('toArray', {attribute: 'data-id'});
      gks_sortorder_obj('gks_aade_katigoria_fpa_ejeresi',mylist,'#table_gks_aade_katigoria_fpa_ejeresi > tbody');
    }
  }); 
  <?php if ($perm_edit) {?>
  ...
  function save_peppol_click() {
    ...
    gks_field_from_table_save('gks_aade_katigoria_fpa_ejeresi',myid,'fpa_ejeresi_peppol_code',mytext, false);
  }
  window.gks_field_from_table_save_after=function(mydata) {
    if (mydata.table_name!='gks_aade_katigoria_fpa_ejeresi') return;
    if (mydata.table_id<=0) return;
    if (mydata.field_name!='fpa_ejeresi_peppol_code') return;
