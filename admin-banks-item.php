...
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_banks',($id==-1 ? 'add' : 'view'),$id);
...
if ($id==-1) {
  ...
  $row['bank_descr'] ='';
  $row['bank_disable']=0;
  $row['bank_sortorder']=1000;

  $my_page_title=gks_lang('Νέα Τράπεζα');


} else {
 $sql ="SELECT gks_banks.*,
  ".GKS_WP_TABLE_PREFIX."users_add.gks_nickname as gks_nickname_add, ".GKS_WP_TABLE_PREFIX."users_edit.gks_nickname as gks_nickname_edit
  FROM gks_banks
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_add on gks_banks.user_id_add = ".GKS_WP_TABLE_PREFIX."users_add.ID
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_edit on gks_banks.user_id_edit = ".GKS_WP_TABLE_PREFIX."users_edit.ID
  where id_bank = ".$id;
  ...
  $my_page_title=gks_lang('Τράπεζα').': '.$row['bank_descr'];
  $object_title=$row['bank_descr'];
}

stat_record();
$nav_active_array=array('manage','manage_banks');
...
$lang_data_obj=gks_lang_data_obj_prepare('gks_banks','default');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    <div class="col-sm-12" style="text-align:center">
      <?php if ($id > 0) {?>
        <h3><?php echo gks_lang('Τράπεζα');?>: <span class="gks_object_badge_secondary">#<?php echo $id;?></span> <?php echo $object_title;?></h3>
      <?php } else { ?>
        <h3><?php echo gks_lang('Τράπεζα');?>: <span class="gks_object_badge_secondary">#<?php echo gks_lang('Νέα');?></span></h3>
      <?php }?>
    </div>
  </div>
</div>
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>  
          ...
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="bank_descr"><?php echo gks_lang('Τράπεζα');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="bank_descr"  value="<?php echo htmlspecialchars_gks($row['bank_descr']);?>">
            </div>
          </div>
          
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('bank_descr'));
          ?>
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="bank_bic"><?php echo gks_lang('BIC / SWIFT');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="bank_bic"  value="<?php echo htmlspecialchars_gks($row['bank_bic']);?>">
            </div>
          </div>
          <div class="form-group row">
            <label for="bank_sortorder" class="col-sm-4 col-form-label form-control-sm text-sm-right"><?php echo gks_lang('Σειρά Ταξινόμησης');?>:</label>
            <div class="col-sm-8">
              <input id="bank_sortorder" type="number" class="form-control form-control-sm myneedsave" value="<?php echo htmlspecialchars_gks($row['bank_sortorder']);?>" min="1" strep="1">
            </div>
          </div>
                    
          <div class="form-group row">
            <label for="bank_disable" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Ενεργός');?>:</label>
            <div class="col-md-8">
              <input type="checkbox" id="bank_disable" value="1" <?php if ($row['bank_disable']==0) echo ' checked '; ?> class="switchery1_this">
            </div>
          </div>  
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>       
          ...
          <div class="row">
            ...
            <div class="col-sm-8"><span class="form-control-plaintext form-control-sm"><?php if ($row['id_bank']>0) echo $row['id_bank'];?></span></div>
          </div>
          <div class="row">
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_banks','edit',  $id);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_banks','add',   $id);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_banks','delete',$id);?>;
...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    ...
    datasend+='&bank_descr='  +  encodeURIComponent($.base64.encode($("#mypostform #bank_descr").val().trim()));
    datasend+='&bank_bic='  +  encodeURIComponent($.base64.encode($("#mypostform #bank_bic").val().trim()));
    datasend+='&bank_sortorder='  + encodeURIComponent(($("#mypostform #bank_sortorder").val().trim()));
    datasend+='&bank_disable=' + (($('#mypostform #bank_disable').is(':checked')) ? '0':'1');
    ...
    $.ajax({
			url: '/my/admin-banks-item-exec.php?id=' + <?php echo $id;?>,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
