...
$product_brand_id=0;
if (isset($_POST['product_brand_id'])) $product_brand_id=intval($_POST['product_brand_id']);
...
$my_page_title=gks_lang('Προσθήκη μάρκας σε στοιχείο τιμοκαταλόγου');
db_open();
stat_record();
...
$sql="SELECT id_product_brand FROM gks_eshop_products_brands where id_product_brand = ".$product_brand_id;
$result = $db_link->query($sql);
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode('sql error'));
...
if ($result->num_rows == 0) {
  debug_mail(false,'empty',gks_lang('Δεν βρέθηκε η μάρκα'));
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Δεν βρέθηκε η μάρκα')));
  echo json_encode($return); die();}  



$sql="SELECT * FROM gks_eshop_pricelist_items_brands where pricelist_item_id = ".$id." and product_brand_id=".$product_brand_id;
$result = $db_link->query($sql);
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode('sql error'));
...
if ($result->num_rows != 0) {
  debug_mail(false,'empty',gks_lang('Το στοιχείο τιμοκαταλόγου - μάρκα υπάρχει ήδη'));
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Το προϊόν - μάρκα υπάρχει ήδη')));
  echo json_encode($return); die();}  


$sql="insert into gks_eshop_pricelist_items_brands (product_brand_id,pricelist_item_id,is_include,
...
$id_pricelist_item_brand = $db_link->insert_id; 



$row_html='';
if (1==2 and isset($_POST['from']) and $_POST['from']=='cat') {
$sql_list = "SELECT gks_eshop_pricelist_items_brands.*, gks_eshop_products.product_descr,product_photo,product_code
FROM gks_eshop_pricelist_items_brands 
LEFT JOIN gks_eshop_products ON gks_eshop_pricelist_items_brands.pricelist_item_id = gks_eshop_products.id_product
WHERE id_pricelist_item_brand=".$id_pricelist_item_brand;
...
$row_html=
'<tr class="product_tr_new" data-id="'.$row_list['id_pricelist_item_brand'].'">'.
  '<th scope="row" nowrap align="right" class="product_aa">*</td>'.
  '<td nowrap align="center">'.
    '<img src="img/delete.png" border="0" width="16" class="deleterow" data-deleteafter="gks_fnc_product_delete_after|'.$row_list['id_pricelist_item_brand'].'" data-id="'.$row_list['id_pricelist_item_brand'].'" data-model="gks_eshop_pricelist_items_brands">'.
  '</td>'.
...
$row_html=
'<tr class="brands_tr_new" data-id="'.$row_list['id_pricelist_item_brand'].'">'.
  '<th scope="row" nowrap class="mytdcm brands_aa">*</td>'.
  '<td class="mytdcm">'.
    '<img src="img/delete.png" border="0" width="16" class="deleterow" data-deleteafter="gks_fnc_brands_delete_after|'.$row_list['id_pricelist_item_brand'].'" data-id="'.$row_list['id_pricelist_item_brand'].'" data-model="gks_eshop_pricelist_items_brands">'.
  '</td>'.
  '<td class="mytdcm" nowrap align="center"><a href="admin-product-brands-item.php?id='.$row_list['product_brand_id'].'"><i class="enterrow fas fa-pen" title="'.gks_lang('Προβολή').'"></i></a></td>'.
  '<td class="mytdcm" nowrap>'.getBrandPhoto($row_list['product_brand_id'],$row_list['brand_photo'],32).'</td>'.
