...
$my_page_title=gks_lang('Αφαίρεση προσωπικής φωτογραφίας σε προφίλ');
db_open();
stat_record();

$sql="select * from gks_users_photo where id_user_photo=".$id." and user_id=".$my_wp_user_id;
...
$sql="delete from gks_users_photo where id_user_photo=".$id." and user_id=".$my_wp_user_id;
$result = $db_link->query($sql);        
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Σφάλμα SQL').'<br>'.gks_lang('Παρακαλώ δοκιμάστε αργότερα')));
