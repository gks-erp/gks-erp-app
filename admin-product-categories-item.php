...
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_eshop_products_categories',($id==-1 ? 'add' : 'view'),$id);
...
$gks_custom_prepare = gks_custom_table_item_prepare('gks_eshop_products_categories',['from'=>'item']);
...
if ($id==-1) {
  ...
  $row['myip'] ='';
  $row['id_product_category']=-1;
  $row['product_category_parent_id']=0;
  $row['product_category_descr']='';
  $row['category_disable']=0;
...
//if ($id==-1) {
//  $sql="insert into gks_eshop_products_categories (product_category_descr,product_category_parent_id,
...
//if ($id <= 0) {header('Location: /my'); die(); }
  ...
  $my_page_title=gks_lang('Νέα Κατηγορία');
} else {


  
  $sql ="SELECT gks_eshop_products_categories.*,
  ".GKS_WP_TABLE_PREFIX."users_add.gks_nickname as gks_nickname_add, ".GKS_WP_TABLE_PREFIX."users_edit.gks_nickname as gks_nickname_edit
  FROM gks_eshop_products_categories 
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_add on gks_eshop_products_categories.user_id_add = ".GKS_WP_TABLE_PREFIX."users_add.ID
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_edit on gks_eshop_products_categories.user_id_edit = ".GKS_WP_TABLE_PREFIX."users_edit.ID
  ...
  $my_page_title=gks_lang('Κατηγορία').': '.$row['product_category_descr'];
  $object_title=$row['product_category_descr'];
}
...
$lang_data_obj=gks_lang_data_obj_prepare('gks_eshop_products_categories','default');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    <div class="col-md-12" style="text-align:center">
      <?php if ($id > 0) {?>
        <h3><?php echo gks_lang('Κατηγορία');?>: <span class="gks_object_badge_secondary">#<?php echo $id;?></span> <?php echo $object_title;?></h3>
      <?php } else { ?>
        <h3><?php echo gks_lang('Κατηγορία');?>: <span class="gks_object_badge_secondary">#<?php echo gks_lang('Νέα');?></span></h3>
      <?php }?>
    </div>
  </div>
</div>
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>         
          ...
          <div class="form-group row">
            <label for="product_category_descr" class="col-sm-4 col-form-label form-control-sm text-sm-right"><?php echo gks_lang('Κατηγορία');?>:</label>
            <div class="col-sm-8">
              <input id="product_category_descr" type="text" class="form-control form-control-sm myneedsave" value="<?php echo htmlspecialchars_gks($row['product_category_descr']);?>">
            </div>
          </div>
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('product_category_descr'));
          ?>
                    
          <div class="form-group row">
            <label for="product_category_parent_id" class="col-sm-4 col-form-label form-control-sm text-sm-right"><?php echo gks_lang('Γονική Κατηγορία');?>:</label>
            <div class="col-sm-8">
              <select name="product_category_parent_id" id="product_category_parent_id"  class="form-control form-control-sm myneedsave">
              ...
              where gks_eshop_products_categories.id_product_category<>".$id."
              ORDER BY descr";
              ...
              while ($row_select = $result_select->fetch_assoc()) {
                ...
                if ($row_select['id']==$row['product_category_parent_id']) echo ' selected ';
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-12">
              <textarea id="category_comments" type="text" class="gks_tinymce form-control form-control-sm myneedsave" style="height:200px;"><?php echo htmlspecialchars_gks($row['category_comments']); ?></textarea>
            </div>
          </div>        
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('category_comments'));
          ?>

          <div class="form-group row">
            <label for="category_disable" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Ενεργή');?>:</label>
            <div class="col-md-8">
              <input type="checkbox" name="category_disable"  id="category_disable" value="1" <?php if ($row['category_disable']==0) echo ' checked '; ?> class="switchery1_this">
            </div>
          </div>                    
        </div>
      </div>
    ...
    <div class="col-md-6">
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('photo');?>>       
          <div class="row">
            <div class="col-md-12" style="text-align:center;"><?php echo gks_lang('Η προεπιλεγμένη φωτογραφία της κατηγορίας');?></div>
            
            <div class="col-md-12" style="text-align:center;">
              <?php
              $product_category_photo_value="";
              $myimgurl = $row['category_photo']; //get_user_meta($my_wp_user_id, 'wsl_current_user_image', true);
              //echo $myimgurl;
              if ($myimgurl.'' == '') {
                $myimgurl="/my/img/product.png";
              } else {
                $product_category_photo_value = $myimgurl;
              }
              ?>
              <img src="<?php echo $myimgurl;?>" border="0" style="max-width:96px;max-height:96px;" id="form_product_category_photo_img"/><br>
              
              <a href="" id="reset_category_photo" title="<?php echo gks_lang('Διαγραφή');?>" <?php 
                if ($product_category_photo_value == '') {
                  echo ' style="display:none" ';
                }
                ?> ><img src="/my/img/0.png" border="0" width="16" ></a>
              <br><input type="hidden" id="form_product_category_photo" name="form_product_category_photo" value="<?php echo $product_category_photo_value;?>" />
            </div>                     
          </div>
          <div class="row">
            <div class="col-md-12" style="text-align:center; padding-top: 24px;"><?php echo gks_lang('Φωτογραφίες της κατηγορίας');?></div>
            
            <form role="form" method="post" action="admin-product-categories-item-photo-upload.php" id="myphoto_category_upload" enctype="multipart/form-data" style="width: 100%;">
              <input type="hidden" name="product_category_id" id="product_category_id" value="<?php echo $id;?>">
              <div id="lightgallery_category">
                <div class="form-group" id="imagelist_photo">
                <?php   
                  $sql="select * from gks_eshop_products_categories_photo where product_category_id =".$id." and filesobjectlist=0 order by id_eshop_products_categories_photo";
                  $result_select = $db_link->query($sql);        
                  if (!$result_select) {
                    debug_mail(false,'error sql',$sql);
                    die('sql error');
                  ...
                  while ($row_select = $result_select->fetch_assoc()) {
                    ...
                    ?>
                    <div id="item_upload_photo_<?php echo $row_select['id_eshop_products_categories_photo'];?>" style="float: left;width:100px;height:130px;border: 0px solid #ddd;padding:2px;margin:2px;text-align: center;overflow: hidden;">
                      <a class="lightgallery_item_category" href="<?php echo $photo_url;?>" data-download-url="<?php echo $photo_url;?>">
                        <img style="position: relative; top: 5px; left: 0px;max-width:96px;max-height:96px;" id="myimg" src="<?php echo $photo_url_thumb;?>">
                      </a>
                      <br>
                      <div style="padding-top:4px">
                        <a href="" class="set_category_photo"   data-url="<?php echo $photo_url_thumb;?>" title="<?php echo gks_lang('Ορισμός ως προεπιλεγμένη φωτογραφία');?>"><img src="/my/img/icons/photo.png" border="0" width="16"></a>
                        <a href="" class="delete_category_upload_photo" data-url="<?php echo $photo_url_thumb;?>" data-id="<?php echo $row_select['id_eshop_products_categories_photo'];?>" title="<?php echo gks_lang('Διαγραφή');?>"><img src="/my/img/0.png" border="0" width="16"></a>
                      </div>
                    </div>
                  <?php }?>
                </div>
              </div>
              <?php gks_f_button_add_files_photo_html('gks_eshop_products_categories',$id);?>
            </form>                      
...
<div class="container-fluid" id="gks_rsrv_f">
  <div class="form-group1 row">
    <div class="col-md-12 text-center mt-2">
      ...
      <button type="button" class="btn btn-danger deleterowbtn" data-id="<?php echo $row['id_product_category'];?>" data-model="gks_eshop_products_categories" data-backurl="admin-product-categories.php" <?php if ($id<=0) echo 'disabled';?>><?php echo gks_lang('Διαγραφή');?></button>
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('ergas');?>>        

          <?php
          $query = "SELECT gks_production_ergasies_eidoscat.*, gks_production_ergasies.production_ergasia_descr
          ...
          WHERE gks_production_ergasies_eidoscat.cateidos_id=".$id."
          ...
          while ($row_list = $result_list->fetch_assoc()) {
            ...
            $i++;
            ?>
            <tr class="ergasies_tr_exist" data-id="<?php echo $row_list['id_production_ergasies_eidoscat'];?>">
              <th scope="row" nowrap align="right" class="ergasies_aa"><?php echo ($i);?></td>       
              <td nowrap align="center">
                <img src="img/delete.png" border="0" width="16" class="deleterow" data-deleteafter="gks_fnc_ergasies_delete_after|<?php echo $row_list['id_production_ergasies_eidoscat'];?>" data-id="<?php echo $row_list['id_production_ergasies_eidoscat'];?>" data-model="gks_production_ergasies_eidoscat">            
              </td>
      ...
      <div class="card gks_card_expand">
        <div class="card-header" style="text-align:center">
          <span style="vertical-align: middle;"><?php echo gks_lang('Σύνδεση με κατηγορία eshop');?></span>
        ...
        <div class="card-body" <?php echo gks_card_body('eshops');?>>
          ...
          $query = "SELECT gks_woo_categories.*, gks_eshops.eshop_name, gks_eshops.eshop_url, ".GKS_WP_TABLE_PREFIX."users.gks_nickname
          FROM (gks_woo_categories 
          LEFT JOIN gks_eshops ON gks_woo_categories.eshop_id = gks_eshops.id_eshop) 
          LEFT JOIN ".GKS_WP_TABLE_PREFIX."users ON gks_woo_categories.last_update_user_id = ".GKS_WP_TABLE_PREFIX."users.ID
          WHERE gks_woo_categories.product_category_id=".$id."
          ORDER BY gks_eshops.eshop_sortorder, gks_woo_categories.id_woo_category;";
          ...
          <thead>
            <tr>	
              ...
              <th class="table-dark" scope="col" style="text-align: center !important;" width="20%"  nowrap>ID eshop</th>        
          ...
          while ($row_list = $result_list->fetch_assoc()) {
            ...
            $i++;
            ?>
            <tr class="eshoplink_tr_exist" data-id="<?php echo $row_list['id_woo_category'];?>">
              <th scope="row" nowrap class="mytdcm eshoplink_aa"><?php echo ($i);?></td>       
              <td nowrap class="mytdcm">
                <img src="img/delete.png" border="0" width="16" class="deleterow" data-deleteafter="gks_fnc_eshoplink_delete_after|<?php echo $row_list['id_woo_category'];?>" data-id="<?php echo $row_list['id_woo_category'];?>" data-model="gks_woo_categories">
              </td>
              ...
              <td class="mytdcm" nowrap><a href="<?php echo $row_list['eshop_url'];?>/wp-admin/term.php?taxonomy=product_cat&post_type=product&tag_ID=<?php echo $row_list['remote_category_id'];?>" target="_blank"><?php echo $row_list['remote_category_id'];?></a></td>  
              ...
              <td class="mytdcm" nowrap ><span class="tooltipster" title="<?php 
                echo gks_lang('Από').': '.$row_list['gks_nickname'];
                ?>"><?php if (isset($row_list['last_update_date'])) echo showDate(strtotime($row_list['last_update_date']), 'd/m/Y\<\b\r\>H:i:s', 1);?></span></td>
            </tr>
          <?php } ?>
      ...
      <?php 
      echo getObjectRels('gks_eshop_products_categories',$id);
      echo getActivityObjectTable('gks_eshop_products_categories',$id);
      $obj_fileslist= gks_FilesObjectList(array('objname'=>'gks_eshop_products_categories','id'=>$id));
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>   
          ...
          <div class="row">
            ...
            <div class="col-sm-8"><input id="id_product_category" type="text" readonly class="form-control-plaintext form-control-sm" value="<?php echo $row['id_product_category'];?>"></div>
          </div>
          <div class="row">
    ...
    <div class="col-md-6">
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('eidi');?>>        
          ...
          <?php
          $query = "SELECT gks_eshop_products_categories_products.*, gks_eshop_products.product_descr,product_photo,product_code
          ...
          WHERE gks_eshop_products_categories_products.product_category_id=".$id."
          ...
          while ($row_list = $result_list->fetch_assoc()) {
            ...
            $i++;
            ?>
            <tr class="product_tr_exist" data-id="<?php echo $row_list['id_eshop_products_categories_products'];?>">
              <th scope="row" nowrap class="mytdcm product_aa"><?php echo ($i);?></td>       
              <td nowrap class="mytdcm">
                <img src="img/delete.png" border="0" width="16" class="deleterow" data-deleteafter="gks_fnc_product_delete_after|<?php echo $row_list['id_eshop_products_categories_products'];?>" data-id="<?php echo $row_list['id_eshop_products_categories_products'];?>" data-model="gks_eshop_products_categories_products">            
              </td>
      ...
      <div class="card gks_card_expand">
        <div class="card-header" style="text-align:center">
          <?php echo gks_lang('Όλοι τα Είδη, και από τις υποκατηγορίες');?>
        </div>
        <div class="card-body" <?php echo gks_card_body('alleidi');?>>        
          ...
          where ug1.id_product_category=".$id;
          //echo $sql;
          ...
          $sql="SELECT DISTINCT gks_eshop_products_categories_products.product_id, gks_eshop_products.product_descr,product_photo,product_code
          FROM gks_eshop_products_categories_products LEFT JOIN gks_eshop_products ON gks_eshop_products_categories_products.product_id = gks_eshop_products.id_product
          WHERE gks_eshop_products_categories_products.product_category_id In (".$gu_in.")
          ORDER BY product_code,gks_eshop_products.product_descr";
...
<div id="dialog_eshoplink" title="<?php echo $GKS_SITE_HUMAN_NAME;?>" style="display: none;">
  <div class="container-fluid" style="" >
    <div class="row" style="margin-bottom: 10px;">
      <div style="font-size: 120%;font-weight: bold;text-align:center;width: 100%;"><?php echo gks_lang('Σύνδεση κατηγορίας eshop');?></div>
    </div>
    <div class="row" style="margin-bottom: 10px;">
    ...
    <div class="row" style="margin-bottom: 10px;">
      <label for="dialog_eshoplink_list" class="col-md-12 col-form-label form-control-sm text-sm-center"><?php echo gks_lang('Κατηγορία');?>:</label>
      <div class="col-md-12 text-sm-center">
...
var from_php_dialog_object_rel_curr='gks_eshop_products_categories';
var from_php_activity_model='gks_eshop_products_categories';
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_eshop_products_categories','edit',  $id);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_eshop_products_categories','add',   $id);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_eshop_products_categories','delete',$id);?>;
...
<script src="js/admin-product-categories-item.js?v=<?php echo $gks_cache_version;?>"></script>
