...
function gks_worldline_api_get_transactions($mydaydif, $company_id, $in_transaction_id) {
  global $db_link;
  global $gkIP;
  ...
  if ($in_transaction_id=='') {
    ...
    if ($result->num_rows==0) {
      ...
      if ($worldline_username=='') {debug_mail(false,'worldline_username is not set for company '.$company_id,$sql);die('worldline_username is not set for company '.$company_id);}
      if ($worldline_password=='') {debug_mail(false,'worldline_password is not set for company '.$company_id,$sql);die('worldline_password is not set for company '.$company_id);}
      if ($worldline_authorization_code=='') {debug_mail(false,'worldline_authorization_code is not set for company '.$company_id,$sql);die('worldline_authorization_code is not set for company '.$company_id);}
      if ($worldline_x_api_key=='') {debug_mail(false,'worldline_x_api_key is not set for company '.$company_id,$sql);die('worldline_x_api_key is not set for company '.$company_id);}
      
      //echo '<pre>ssssss';die();
    }
  } else {
    $sql="select MID from gks_worldline_transaction where Id='".$db_link->escape_string($in_transaction_id)."'";
    $result = $db_link->query($sql);        
    if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
    if ($result->num_rows==0) {
    ...
    $sql="select * from gks_company where worldline_mid like '".$db_link->escape_string($MerchantId)."'";
    $result = $db_link->query($sql);        
    if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
    if ($result->num_rows==0) {
      debug_mail(false,'company not found',$sql);die('company not found');
  ...
  $headers = array(
    ...
    'X-Api-Key: '.$worldline_x_api_key,
  );

  $url=GKS_WORLDLINE_COM_API.'/api/v2.1/transactionintent/?Timestamp_min='.$today_vardia_min.'&Timestamp_max='.$today_vardia_max;
  //echo '<pre>'.$url;die();
  ...
  $url=GKS_WORLDLINE_COM_API.'/api/v2.1/transaction/?Timestamp_min='.$today_vardia_min.'&Timestamp_max='.$today_vardia_max;;
  do {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_HTTPGET, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  ...
  foreach ($mydata as &$mytra) {
    ...
    if ($intentId!='') {
      $sql="select id_worldline_transaction from gks_worldline_transaction 
      where intentId='".$db_link->escape_string($intentId)."'";
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
      if ($result->num_rows>0) { 
        $row = $result->fetch_assoc();
        $mytra['id_worldline_transaction']=$row['id_worldline_transaction'];
      }
  ...
  foreach ($mydata as &$mytra) {
    $intentId=trim_gks($mytra['intentId']);
    $sql="select id_worldline_transaction from gks_worldline_transaction where ";
    ...
    if ($result->num_rows==0) { 
      ...
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
      $mytra['id_worldline_transaction']=$db_link->insert_id; 
      
      if ($mytra['id_eftpos_transaction']>0) {
        $sql="update gks_eftpos_transaction set 
        xxx_transaction_id=".$mytra['id_worldline_transaction'].",
      ...
      $mytra['id_worldline_transaction']=$id_worldline_transaction;
  ...
  foreach ($mydata as &$mytra) {
    $intentId=trim_gks($mytra['intentId']);
    $id_worldline_transaction=$mytra['id_worldline_transaction'];
    if ($mytra['gks_transaction']>0 and $id_worldline_transaction>0 and count($mytra['gks_trans'])==1) {
      ...
      //if ($id_worldline_transaction==10049) die();
      ...
      $sql.=" where id_worldline_transaction=".$id_worldline_transaction;
      
      
      
      //if ($id_worldline_transaction==10024) {echo '<pre>'.$sql;die();}
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
