...
$my_page_title=gks_lang('Εκτύπωση Παραγγελίας').' id: '.$id;
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_orders','view',$id);
...
if ($id==-1) {
    debug_mail(false,'error sql','save order');
...
$save_folder='order/'.$id.'/print/';
$save_basename='ORDER_'.$id.'_'.greeklish(getOrderStateDescr($order_state)).'_'.showDate(time(), 'Y-m-d_H.i.s',1).'.'.rand(100,999);
$print_params=array(
  'table' => 'gks_orders',
  'id' => $id,
...
if ($gks_print_send_gks_erp_app==1 and $preview==0) {
  $sql_send_erp_app="SELECT gks_orders.id_order, gks_acc_seires.erp_app_id, gks_acc_seires.erp_app_dest, 
  ...
  AND gks_orders.order_seira_id>0
...
$ret_print = gks_print_form('gks_orders',$id,$form_id,$print_params);
...
if ($ret_print['success']==false) {
  debug_mail(false,'error creaate print',$ret_print['message']);
...
if ($preview==0) {
  $sql="update gks_orders set 
    ...
    print_order_state='".$db_link->escape_string($order_state)."'
    where id_order=".$id;
  ...
  gks_lang('Κατάσταση').': <span class="order_state_'.$order_state.'">'.getOrderStateDescr($order_state).'</span><br>'.
...
if (isset($_POST['set_def']) and $_POST['set_def']=='1') {
  ...
  $user_settings['print']['form_id_order']=  number_format($form_id,0,'','');
...
$return = array('success' => true, 
  ...
  'redirect'=> base64_encode('admin-orders-item.php?id='.$id.'&scrollto=gks_print'),
