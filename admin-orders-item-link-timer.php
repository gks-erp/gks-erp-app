...
$my_page_title=gks_lang('Λήψη κατάστασης των λήψεων σε παραγγελία');
db_open();
stat_record();
...
$sql="select * from gks_orders_links where order_id=".$id." and (download_status=1 or (download_status in (2,3) and download_end > date_sub(now(), interval 1 minute)))";
...
while ($row = $result->fetch_assoc()) {
  ...
  if ($download_size_until_now==0 and $relative_path!= '') {
    $path_scan=GKS_FileServerShare.'order/'.$id.'/'.$relative_path;
