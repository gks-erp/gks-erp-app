...
$my_page_title=gks_lang('Αυτόματη συμπλήρωση προτύπου HTML');
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_template_html','autocomplete',0);
...
$sql="SELECT id_template_html,template_html_descr FROM gks_template_html
where 
".($anddisabled==1 ? '' : "is_disable=0 and ")." 
".($notid<=0 ? '' : "id_template_html<>".$notid." and ")." 
...
foreach ($term_array as $value) {
  ...
  $mywhere.=" (template_html_descr like '%".$db_link->escape_string($value)."%' or template_html_descr like '%".$db_link->escape_string($value_en)."%') and ";
} 
if (strlen($mywhere)>5) $mywhere=substr($mywhere, 0, strlen($mywhere)-5);
$sql.=$mywhere.")  
order by gks_template_html.sortorder,gks_template_html.template_html_descr
limit 1000";  
...
while ($row = $result->fetch_assoc()) {
  $fount_count++;
  $out[] = array('id' => $row['id_template_html'], 'value' => $row['template_html_descr']);
}
