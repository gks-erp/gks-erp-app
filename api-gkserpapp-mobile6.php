...
switch ($cmd) {   
  ...
  case 'getvivaterminalid':
    ...
    if ($virtualid=='') {
      ...
      $return['message']='Δεν έχει ορισθεί το Virtual Terminal ID';
      echo json_encode($return); die();}
    ...
    if ($result->num_rows==1) {	
      ...
      $return['message']='Βρέθηκε το Terminal ID και είναι το '."\r\n".$return['viva_terminal_id']."\r\n".
      'Ως πάγιο η συσκευή είναι η '."\r\n".$return['asset_title']."\r\n".
      'και ανήκει στην εταιρεία '."\r\n".$return['company_eponimia'];
      ...
      if ($result->num_rows==0) {
        $return['success']=false;
        $return['message']='Καμία εταιρεία δεν έχει τις σχετικές ρυθμίσεις για την Viva';
        echo json_encode($return); die();}
      ...
      foreach ($viva_companys as $row_company) {
        ...
        if ($gks_curl_http_code!=200) {
          $return['success']=false;
          $return['message']='Δεν ήταν δυνατή η εποικωνία με το '.$url;
          echo json_encode($return); die();}
        ...
        if (!(is_array($response_array) and count($response_array)>0 and isset($response_array[0]['statusId']))) {
          $return['success']=false;
          $return['message']='Λάθος δεδομένα από '."\r\n".$url."\r\n\r\n".$response;
          echo json_encode($return); die();}
        ...
        foreach ($response_array as $myterminal) {
          if ($myterminal['virtualTerminalId']==$virtualid) {
            ...
            $sql="insert into gks_assets (
              ...
              'Αυτόματη εισαγωγή μέσα από το gks ERP App Mobile ID: ".$id_erp_app_mobile."',
            ...
            $return['asset_title']='viva pos '.$id_asset;
            $return['id_company']=intval($row_company['id_company']);
            $return['company_eponimia']=trim_gks($row_company['company_eponimia']);

            $return['message']='Βρέθηκε το Terminal ID και είναι το '."\r\n".$return['viva_terminal_id']."\r\n".
            'Ως πάγιο η συσκευή προστέθηκε και είναι η '."\r\n".$return['asset_title']."\r\n".
            'και ανήκει στην εταιρεία '."\r\n".$return['company_eponimia']."\r\n".
            'Ενημερώστε το πάγιο και με άλλες λεπτομέρεις π.χ. φωτογραφία, μέσω το gks ERP'; 
            $return['success']=true;
            echo json_encode($return); die();
    ...
    if ($return['message']=='OK111') {
      $return['success']=false;
      $return['message']='Είναι σίγουρα το viva στην εταιρεία που πρέπει ;';
