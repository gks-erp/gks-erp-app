...
$my_page_title=gks_lang('Επεξεργασία καταχώρησης ημερολογίου').' - '.gks_lang('Δραστηριότητα');
db_open();
stat_record();
...
if (isset($row_old) and is_array($row_old) and 
   ...
   $row_old['activity_status']=='050new' and 
   $start!='' and strtotime($start)>=time() and 
   $start!=$row_old['activity_duedate']) {
  $sql_activity_notification_send_at='activity_notification_send_at=null,';  
}  


$sql="update gks_crm_activity set
activity_duedate ='".$start."',
".$sql_activity_notification_send_at."
mydate_edit=now(),
