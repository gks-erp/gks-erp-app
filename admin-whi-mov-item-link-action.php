...
$my_page_title=gks_lang('Εντολή λήψης αρχείου δελτίου');
db_open();
stat_record();
...
if ($action == 'start') {
  ...
  gks_curl_post_async(GKS_SITE_URL.'my/exec_whi_mov_link_start.php',array('id' =>$id));

  $return = array('success' => true, 'message' => base64_encode('running...'),'id' => $id, 'action' => $action);
  echo json_encode($return); die();
} else if ($action == 'stop') {
  $sql="update gks_whi_mov_links set html_tds=null,relative_path='',download_status=3,download_start=null,
  download_end=null,download_pososto=0,download_size_until_now=0,download_size_total=0,
  ...
  $sql="update gks_whi_mov_links set html_tds=null,relative_path='',download_status=0,download_start=null,download_end=null,download_pososto=0,download_size_until_now=0,download_size_total=0,
  download_message='' 
  where id_whi_mov_links=".$id;
  $result = $db_link->query($sql);
  if (!$result) {
