...
www.gks.gr
*/
function gks_eftpos_has_transaction_status_mellon($id) {
  if ($id===null) return '';
  $id=intval($id);
  switch ($id) {
...
function gks_eftpos_has_transaction_result_mellon($id) {
  if ($id===null) return '';
  $id=intval($id);
  switch ($id) {
...
function gks_eftpos_has_transaction_type_mellon($id) {
  if ($id===null) return '';
  $id=intval($id);
  switch ($id) {
    case 0: return 'Sale';
...
function gks_eftpos_has_credentials_mellon($row_company) {
  $return=array('success' => false, 'message' => 'generic error');
  if (trim_gks($row_company['mellon_username'])=='' or trim_gks($row_company['mellon_password'])=='' or trim_gks($row_company['mellon_authorization_code'])=='' or trim_gks($row_company['mellon_x_api_key'])=='') {
    $return['message']=gks_lang('Δεν έχουν ορισθεί τα Διαπιστευτήρια πρόσβασης για την Mellon Technologies στην εταιρεία').' '.$row_company['company_title'];
    debug_mail(false,$return['message'],print_r($row_company,true));return $return;
  }
  
  return array('success' => true, 'message' => 'OK', 'data' => array(
    'mellon_username' => $row_company['mellon_username'],
    'mellon_password' => $row_company['mellon_password'],
    'mellon_authorization_code' => $row_company['mellon_authorization_code'],
    'mellon_x_api_key' => $row_company['mellon_x_api_key'],
  ));  
}

function gks_eftpos_get_token_mellon($row_company) {
  global $db_link;
  ...
  if ($need_only_refresh) {
    $url=GKS_MELLONGROUP_COM_API.'/api/token/refresh/';
    $mypost=array();
    $mypost['refresh']=$pc_refresh_token_id;
  } else { 
    $url=GKS_MELLONGROUP_COM_API.'/api/token/';
    $mypost=array();
    $mypost['username']=$row_company['mellon_username'];
    $mypost['password']=$row_company['mellon_password'];
  }
  //echo $url;die();
  ...
  if($need_only_refresh) {
    ...
    $pc_token_expiration=date('Y-m-d H:i:s',time() + 8*60*60);
    ...
    $pc_refresh_token_expiration=date('Y-m-d H:i:s',time() + 30*24*60*60);
  }
...
function gks_eftpos_sales_request_mellon($data) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  $return=array('success' => false, 'message' => 'generic error');
  ...
  $url=GKS_MELLONGROUP_COM_API.'/api/v2.2/terminal/'.$data['row_asset']['mellon_id'].'/txninit/';
  //echo '<pre>'.$url;die();
  ...
  $mypost['CashbackAmount']=0; //The cashback amount in integer form. Ex. 1.00 eur is 100
  ...
  if (in_array($transaction_type,['refund','refunderp','preauthcompletion','preauthcompletionerp'])) {
    ...
    $sql="select * from gks_mellon_transaction 
    where id_mellon_transaction>0
    and myStatus=3 and myResult=1
    and id_mellon_transaction=".$xxx_transaction_id;
    $result = $db_link->query($sql);        
    if (!$result) {
    ...
    $row_prev_mellon = $result->fetch_assoc();
    
    $mypost['InitialTransaction']=$row_prev_mellon['Id'];
  }
  ...
  $headers = array(
    ...
    'X-Api-Key: '.$data['mellon_x_api_key'],
  );
    
  $mypostdata=json_encode($mypost);
  //echo '<pre>postargs '."\n";print_r($headers);print_r($mypost);print_r($data);die();
  
  $sql="insert into gks_mellon_transaction (
  user_id_add,user_id_edit,mydate_add,mydate_edit,myip,
  ...
  ".$my_wp_user_id.",".$my_wp_user_id.",now(),now(),'".$db_link->escape_string($gkIP)."',
  ".$my_wp_user_id.",'gks ERP App','Mellon Api',
  ...
  if (!$result) {
    debug_mail(false,'error sql',$sql);
    return array('success' => false, 'message' => 'sql error', 'status' => 'sql error');}
  $id_mellon_transaction = $db_link->insert_id; 
  ...
  xxx_transaction_id=".$id_mellon_transaction."
  ...
  //debug_mail(false,'response mellon',$response);
  
  $gks_curl_http_code=(isset($gks_curl_info['http_code']) ? intval($gks_curl_info['http_code']) : 0);

  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/mellon_sales_request_send_'.time().'.json',json_encode(json_decode($mypostdata,true),JSON_PRETTY_PRINT));
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/mellon_sales_request_response_'.time().'.json','gks_curl_http_code:'.$gks_curl_http_code."\n".$response);
  ...
  $sql="update gks_mellon_transaction set
  ...
  where id_mellon_transaction=".$id_mellon_transaction;
  $result = $db_link->query($sql);        
  if (!$result) {
    debug_mail(false,'error sql',$sql);
    return array('success' => false, 'message' => 'sql error', 'status' => 'sql error');}
  ...
  if ($gks_curl_http_code!=200) {
    ...
    debug_mail(false,'mellon error',$response);return $return;}
...
function gks_eftpos_sales_request_get_status_mellon($data) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  ...
  if (in_array($transaction_type,['fullvoid','fullvoiderp'])) {
    $sql="SELECT gks_eftpos_transaction_thisisfor.my_for, gks_mellon_transaction.TransactionId_org
    ...
    AND gks_mellon_transaction.TransactionId_org<>''";
    $result = $db_link->query($sql);        
    if (!$result) {
      debug_mail(false,'error sql',$sql);
      return array('success' => false, 'message' => 'sql error', 'status' => 'sql error');}
    ...
    $url=GKS_MELLONGROUP_COM_API.'/api/v2.2/transactionintent/?TransactionId='.rawurlencode($prev_TransactionId_org);
  } else {
    $url=GKS_MELLONGROUP_COM_API.'/api/v2.2/transactionintent/?CustomerReference='.$data['sessionId'];
  }
  
//  $url=GKS_MELLONGROUP_COM_API.'/api/v2.2/transactionintent/'.
//  '?TID='.
...
//  date('Y-m-d\TH:i:s\Z', strtotime($data['row_tra']['mydate_add'])-10*60); //2024-07-01T15:19:48Z
  ...
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/mellon_status_send_s1_'.time().'.json',$url);
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/mellon_status_response_s1_'.time().'.json','gks_curl_http_code:'.$gks_curl_http_code."\n".$response);
  ...
  if ($remote_id!='') {
    foreach ($response_array['results'] as $res_val) {
      if (isset($res_val['Id']) and intval($res_val['Id'])==$remote_id) {
        $myresult=$res_val;
        break;
      }
    } 
...
*/
  ...
  if (in_array($r_Result,[2,3,4,5,6,7])) {//ola ektos APPROVED
    ...
    if (in_array($transaction_type,['sale','saleerp'])) {  
      ...
      WHERE gks_eftpos_transaction.payment_acquirer_with_id=3
  ...
  $url=GKS_MELLONGROUP_COM_API.'/api/v2.2/transaction/'.
  '?TID='.
  $data['row_tra']['terminalId'].
  '&Timestamp_min='.
  date('Y-m-d\TH:i:s\Z', strtotime($data['row_tra']['mydate_add'])-10*60); //2024-07-01T15:19:48Z
  ...
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/mellon_status_send_s2_'.time().'.json',$url);
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/mellon_status_response_s2_'.time().'.json','gks_curl_http_code:'.$gks_curl_http_code."\n".$response);
...
//          }
          ...
          $company_id=0;
          $mellon_mid=''; if (isset($resitem['MID'])) $mellon_mid=trim_gks($resitem['MID']);
          if ($mellon_mid!='') {
            $sql="select id_company from gks_company where company_disable=0 and mellon_mid='".$db_link->escape_string($mellon_mid)."'";
          ...
          if (count($new_fileds)>0)  {
            debug_mail(false,'new fields in mellon.php',print_r($new_fileds,true)."\n".print_r($resitem,true));
          }
...
function gks_eftpos_get_transaction_extra_html_mellon($input) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  ...
  if ($result->num_rows==1) {
    ...
    if (!empty($row['Approved']) and $row['Approved']==1) {
      $html[]='Approved: <span class="gks_mellon_transaction_approved">Approved</span>';
    } else {
      $html[]='Approved: <span class="gks_mellon_transaction_not_approved">Not Approved</span>';
    }
    if (!empty($row['Voided']) and $row['Voided']==1) {
      $html[]='Voided: <span class="gks_mellon_transaction_voided">Voided</span>';
    } 
    
    
    //if (!empty($row['TxnType'])) {
      $html[]=gks_lang('Τύπος').': <b>'. gks_eftpos_has_transaction_type_mellon($row['TxnType']).'</b>';
    //} 
    ...
    $html[]=gks_lang('Τύπος Κάρτας').': <b class="mellon_tra_PaymentTypeCardType mellon_tra_PaymentType_'.$row['PaymentType'].' mellon_tra_CardType_'.str_replace(' ','_',$row['CardType']).'">'.$temp.'</b>';
    ...
    $html[]='<a href="admin-mellon-transaction-raw.php?id='.$input['id_eftpos_transaction'].'" target="_blank" title="Raw Data"><i class="fas fa-database gks_payment_rawdata"></i></a>'.
...
function gks_eftpos_fullvoid_request_mellon($data) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  $return=array('success' => false, 'message' => 'generic error');
  ...
  $url=GKS_MELLONGROUP_COM_API.'/api/v2.2/terminal/'.$data['row_asset']['mellon_id'].'/txnvoid/';
  //echo '<pre>'.$url;die();
