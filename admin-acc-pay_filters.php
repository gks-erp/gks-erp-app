...
$gks_custom_prepare = gks_custom_table_item_prepare('gks_acc_pay',['from'=>'list']);
...
$filters[] = array(
  ...
  'multiselect' => true,
  'field'  => "gks_acc_pay.pay_state = '%V%'",
  'vals' => array(
    array('value' => 111, 'text' => getAccPayStateDescr('010draft'),      'sql' => "gks_acc_pay.pay_state='010draft'"),
    array('value' => 100, 'text' => getAccPayStateDescr('040cancelled'),  'sql' => "gks_acc_pay.pay_state='040cancelled'"),
    //array('value' => 101, 'text' => getAccPayStateDescr('050proinvoice'), 'sql' => "gks_acc_pay.pay_state='050proinvoice'"),
    array('value' => 102, 'text' => getAccPayStateDescr('080listing'),    'sql' => "gks_acc_pay.pay_state='080listing'"),
    array('value' => 103, 'text' => getAccPayStateDescr('090ekdosi'),     'sql' => "gks_acc_pay.pay_state='090ekdosi'"),
    //array('value' => 109, 'text' => getAccPayStateDescr('100payment'),    'sql' => "gks_acc_pay.pay_state='100payment'"),
  ),
);
...
$filters[] = array(
  ...
  'field' => 'gks_acc_pay.pay_date', 
  'has_custom_default' => 6,
  //		'mywherepos'=>1,
  'vals' => gks_filter_date_vals(['field'=>'gks_acc_pay.pay_date','future'=>false,'today'=>$today, 'today_vardia'=>$today_vardia]),
);
...
if (count($user_companys)>1) {
  ...
  foreach ($user_companys as $key=>$value) {
    $vals[]=array('value' => $key, 'text' => $value['descr'],      'sql' => "gks_acc_pay.company_id=".$value['id_company']." and gks_acc_pay.company_sub_id=".$value['id_company_sub']);
  } 
  $filters[] = array(
    'name' => 'fcompany_id',
    'class' => 'filterselectbox',
...
$filters[] = array(
  ...
  'has_custom_default' => -1,
  'multiselect' => true,
  'field'  => "gks_acc_pay.pay_acc_journal_id = %V%",
  'vals' => array(
...
FROM gks_acc_pay LEFT JOIN gks_acc_journal ON gks_acc_pay.pay_acc_journal_id = gks_acc_journal.id_acc_journal
WHERE (((gks_acc_journal.id_acc_journal) Is Not Null))
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
".(count($perm_id_acc_seira_ids)>0 ? " and gks_acc_pay.pay_acc_seira_id in (".implode(',',$perm_id_acc_seira_ids).")" : "")."
...
$filters[] = array(
  ...
  'has_custom_default' => -1,
  'multiselect' => true,
  'field'  => "gks_acc_pay.pay_acc_seira_id = %V%",
  'vals' => array(
      //array('value' => -100, 'text' => gks_lang('Δεν έχει ορισθεί'),      'sql' => "gks_acc_pay.user_id=0"),
      //array('value' => -101, 'text' => gks_lang('Έχει ορισθεί'),          'sql' => "gks_acc_pay.user_id<>0"),
  ),
  'sql' => "SELECT gks_acc_seires.id_acc_seira AS id, gks_acc_seires.seira_descr AS descr
FROM gks_acc_pay LEFT JOIN gks_acc_seires ON gks_acc_pay.pay_acc_seira_id = gks_acc_seires.id_acc_seira
...
$filters[] = array(
  ...
  'has_custom_date' => true,
  'field' => 'gks_acc_pay.print_date', 
  'has_custom_default' => 1,
  //		'mywherepos'=>1,
  'vals' => gks_filter_date_vals(['field'=>'gks_acc_pay.print_date','future'=>false,'today'=>$today, 'today_vardia'=>$today_vardia,'extra10' =>
    array(
    	array('value' => 102,
    				'text' => gks_lang('Δεν έχει ορισθεί'),
    				'sql' => "gks_acc_pay.print_date is null"),
    	array('value' => 103,
    				'text' => gks_lang('Έχει ορισθεί'),
    				'sql' => "gks_acc_pay.print_date is not null"),
    ),
  ]),
...
$filters[] = array(
  ...
  'has_custom_date' => true,
  'field' => 'gks_acc_pay.aade_send_date', 
  'has_custom_default' => 1,
  //		'mywherepos'=>1,
  'vals' => gks_filter_date_vals(['field'=>'gks_acc_pay.aade_send_date','future'=>false,'today'=>$today, 'today_vardia'=>$today_vardia,'extra10' =>
    array(
    	array('value' => 102,
    				'text' => gks_lang('Δεν έχει ορισθεί'),
    				'sql' => "gks_acc_pay.aade_send_date is null"),
    	array('value' => 103,
    				'text' => gks_lang('Έχει ορισθεί'),
    				'sql' => "gks_acc_pay.aade_send_date is not null"),
    ),
  ]),
);
...
$filters[] = array(
  ...
  'title' => gks_lang('ΜΑΡΚ'),
  'has_custom_date' => true,
  'field' => 'gks_acc_pay.aade_invoicemark', 
  'has_custom_default' => -1,
  'multiselect' => true,
  'vals' => array(
      array('value' => -100, 'text' => gks_lang('Δεν έχει ορισθεί'),      'sql' => "(gks_acc_pay.aade_invoicemark is null or gks_acc_pay.aade_invoicemark='')"),
      array('value' => -101, 'text' => gks_lang('Έχει ορισθεί'),          'sql' => "(gks_acc_pay.aade_invoicemark <>'')"),
  ),
);
...
$filters[] = array(
  ...
  'has_custom_default' => -1,
  'multiselect' => true,
  'field'  => "gks_acc_pay.user_id = %V%",
  'vals' => array(
      array('value' => -100, 'text' => gks_lang('Δεν έχει ορισθεί'),      'sql' => "gks_acc_pay.user_id=0"),
      array('value' => -101, 'text' => gks_lang('Έχει ορισθεί'),          'sql' => "gks_acc_pay.user_id<>0"),
  ),
  'sql' => "SELECT gks_acc_pay.user_id AS id, ".GKS_WP_TABLE_PREFIX."users.gks_nickname AS descr
FROM gks_acc_pay LEFT JOIN ".GKS_WP_TABLE_PREFIX."users ON gks_acc_pay.user_id = ".GKS_WP_TABLE_PREFIX."users.ID
WHERE (((".GKS_WP_TABLE_PREFIX."users.ID) Is Not Null))
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
".(count($perm_id_acc_seira_ids)>0 ? " and gks_acc_pay.pay_acc_seira_id in (".implode(',',$perm_id_acc_seira_ids).")" : "")."
GROUP BY gks_acc_pay.user_id, ".GKS_WP_TABLE_PREFIX."users.gks_nickname
...
$filters[] = array(
    ...
    'sql' => "SELECT ma_poli as id, ma_poli as descr
    FROM gks_acc_pay LEFT JOIN gks_users ON gks_acc_pay.user_id = gks_users.user_id
    where ma_poli<>''
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
...
$filters[] = array(
    ...
    'sql' => "SELECT gks_users.ma_nomos_id AS id, gks_nomoi.nomos_descr AS descr
    FROM (gks_acc_pay LEFT JOIN gks_users ON gks_acc_pay.user_id = gks_users.user_id) LEFT JOIN gks_nomoi ON gks_users.ma_nomos_id = gks_nomoi.id_nomos
    WHERE gks_nomoi.id_nomos is not null
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
".(count($perm_id_acc_seira_ids)>0 ? " and gks_acc_pay.pay_acc_seira_id in (".implode(',',$perm_id_acc_seira_ids).")" : "")."
    GROUP BY gks_users.ma_nomos_id, gks_nomoi.nomos_descr
...
$filters[] = array(
    ...
    'sql' => "SELECT gks_users.ma_country_id AS id, gks_country.country_name AS descr
    FROM (gks_acc_pay LEFT JOIN gks_users ON gks_acc_pay.user_id = gks_users.user_id) LEFT JOIN gks_country ON gks_users.ma_country_id = gks_country.id_country
    WHERE (((gks_country.id_country) Is Not Null))
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
".(count($perm_id_acc_seira_ids)>0 ? " and gks_acc_pay.pay_acc_seira_id in (".implode(',',$perm_id_acc_seira_ids).")" : "")."
    GROUP BY gks_users.ma_country_id, gks_country.country_name
...
$filters[] = array(
    ...
    'vals' => array(
        array('value' => -100, 'text' => gks_lang('Έχει'),      'sql' => "pay_state in ('080listing','090ekdosi','100payment') and affect_balance=1 and affect_balance_poso<>0"),
        array('value' => -101, 'text' => gks_lang('Δεν έχει'),  'sql' => "not (pay_state in ('080listing','090ekdosi','100payment') and affect_balance=1 and affect_balance_poso<>0)"),
    ),
);
...
$filters[] = array(
    ...
    'has_custom_default' => -1,
    'multiselect' => true,
    'field'  => "gks_acc_pay.assigned_id=%V%",
    'vals' => array(
        //array('value' => -1, 'text' => gks_lang('Όλα'),          'sql' => "1=1"),
    ),
    'sql' => "SELECT ".GKS_WP_TABLE_PREFIX."users.ID as id, ".GKS_WP_TABLE_PREFIX."users.gks_nickname as descr
    FROM gks_acc_pay LEFT JOIN ".GKS_WP_TABLE_PREFIX."users ON gks_acc_pay.assigned_id = ".GKS_WP_TABLE_PREFIX."users.ID
    WHERE (((".GKS_WP_TABLE_PREFIX."users.ID) Is Not Null))
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
...
if ($GKS_CRM_ENABLE) {
  $filters[] = array(
      ...
      'has_custom_default' => -1,
      'multiselect' => true,
      'field'  => "gks_acc_pay.crm_channel_id=%V%",
      'vals' => array(
          //array('value' => -1, 'text' => gks_lang('Όλα'),          'sql' => "1=1"),
      ),
      'sql' => "SELECT gks_crm_channel_sale.id_crm_channel_sale AS id, gks_crm_channel_sale.crm_channel_sale_descr AS descr
      FROM gks_acc_pay LEFT JOIN gks_crm_channel_sale ON gks_acc_pay.crm_channel_id = gks_crm_channel_sale.id_crm_channel_sale
...
".(count($perm_id_acc_seira_ids)>0 ? " and gks_acc_pay.pay_acc_seira_id in (".implode(',',$perm_id_acc_seira_ids).")" : "")."
  ...
  $filters[] = array(
      ...
      'has_custom_default' => -1,
      'multiselect' => true,
      'field'  => "gks_acc_pay.crm_channel_contact_id=%V%",
      'vals' => array(
          //array('value' => -1, 'text' => gks_lang('Όλα'),          'sql' => "1=1"),
      ),
      'sql' => "SELECT ".GKS_WP_TABLE_PREFIX."users.ID as id, ".GKS_WP_TABLE_PREFIX."users.gks_nickname as descr
      FROM gks_acc_pay LEFT JOIN ".GKS_WP_TABLE_PREFIX."users ON gks_acc_pay.crm_channel_contact_id = ".GKS_WP_TABLE_PREFIX."users.ID
      WHERE (((".GKS_WP_TABLE_PREFIX."users.ID) Is Not Null))
".(count($perm_id_company_ids)>0 ? " and gks_acc_pay.company_id in (".implode(',',$perm_id_company_ids).")" : "")."
".(count($perm_id_company_sub_ids)>0 ? " and gks_acc_pay.company_sub_id in (".implode(',',$perm_id_company_sub_ids).")" : "")."
".(count($perm_id_acc_journal_ids)>0 ? " and gks_acc_pay.pay_acc_journal_id in (".implode(',',$perm_id_acc_journal_ids).")" : "")."
".(count($perm_id_acc_seira_ids)>0 ? " and gks_acc_pay.pay_acc_seira_id in (".implode(',',$perm_id_acc_seira_ids).")" : "")."
  ...
  $filters[] = array(
      ...
      'has_custom_default' => -1,
      'multiselect' => true,
      'field'  => "gks_acc_pay.crm_channel_campain_id=%V%",
      'vals' => array(
          //array('value' => -1, 'text' => gks_lang('Όλα'),          'sql' => "1=1"),
      ),
...
$sortable = array(
  array('name' => 'soid', 'field' => 'gks_acc_pay.id_acc_pay'),
  array('name' => 'sood', 'field' => 'gks_acc_pay.pay_date'),
  ...
  array('name' => 'sonumber', 'field' => 'gks_acc_pay.pay_acc_number_int'),
  
  
  
  array('name' => 'soprint_date', 'field' => 'gks_acc_pay.print_date'),
  array('name' => 'soaade_send_date', 'field' => 'gks_acc_pay.aade_send_date'),
  array('name' => 'sostate', 'field' => 'gks_acc_pay.pay_state'),
  array('name' => 'souser', 'field' => GKS_WP_TABLE_PREFIX.'users.gks_nickname'),
  array('name' => 'soprice', 'field' => 'gks_acc_pay.gks_price_total'),
  ...
  array('name' => 'sopoli', 'field' => 'gks_users.ma_poli'),
	...
	array('name' => 'socrmcode', 'field' => 'gks_acc_pay.crm_channel_code'),
);
...
'gks_acc_pay.note_logistirio',
'gks_acc_pay.note_doc',
...
//$sql = "SELECT SQL_CALC_FOUND_ROWS gks_acc_pay.*, 
...
//".GKS_WP_TABLE_PREFIX."users_aade.gks_nickname AS gks_nickname_aade,".GKS_WP_TABLE_PREFIX."users_print.gks_nickname AS gks_nickname_print,
...
//FROM ".$gks_custom_prepare['sql_all_list_from']." (((((((((((((((((((((gks_acc_pay
...
//LEFT JOIN gks_eshop_fiscal_position ON gks_acc_pay.fiscal_position_id = gks_eshop_fiscal_position.id_fiscal_position) 
//LEFT JOIN gks_eshop_pricelist ON gks_acc_pay.pricelist_id = gks_eshop_pricelist.id_pricelist)
//LEFT JOIN gks_country ON gks_acc_pay.ma_country_id = gks_country.id_country)
//LEFT JOIN gks_nomoi ON gks_acc_pay.ma_nomos_id = gks_nomoi.id_nomos)
//LEFT JOIN gks_country as gks_country_dest ON gks_acc_pay.destination_data_country_id = gks_country_dest.id_country) 
//LEFT JOIN gks_nomoi as gks_nomoi_dest ON gks_acc_pay.destination_data_nomos_id = gks_nomoi_dest.id_nomos)
//LEFT JOIN gks_lang ON gks_acc_pay.user_lang = gks_lang.id_lang)
...
if (isset($_GET['fparoxos'])) {
  $sql.= " and gks_acc_pay.aade_paroxos_id=".intval($_GET['fparoxos'])." ";
}
if (isset($_GET['paroxos_tf1_url_has'])) {
  $sql.= " and gks_acc_pay.paroxos_tf1_url_has=".intval($_GET['paroxos_tf1_url_has'])." ";
}
...
if (empty($sorted['sql'])) {
	$sql .= " ORDER BY gks_acc_pay.pay_date desc, gks_acc_pay.id_acc_pay desc";
} else {
