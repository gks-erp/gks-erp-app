...
if ($id==-1) {
  ...
  $row['myip'] ='';
  $row['id_fiscal_position']=-1;
  $row['fiscal_position_descr'] ='';
  $row['fiscal_position_sortorder'] =1000;
  $row['fiscal_position_disable'] =0;

  $my_page_title=gks_lang('Φορολογική Θέση');

} else {
 $sql ="SELECT gks_eshop_fiscal_position.*,
 ".GKS_WP_TABLE_PREFIX."users_add.gks_nickname as gks_nickname_add, ".GKS_WP_TABLE_PREFIX."users_edit.gks_nickname as gks_nickname_edit
  FROM gks_eshop_fiscal_position
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_add on gks_eshop_fiscal_position.user_id_add = ".GKS_WP_TABLE_PREFIX."users_add.ID
  LEFT JOIN ".GKS_WP_TABLE_PREFIX."users as ".GKS_WP_TABLE_PREFIX."users_edit on gks_eshop_fiscal_position.user_id_edit = ".GKS_WP_TABLE_PREFIX."users_edit.ID
  where id_fiscal_position = ".$id;
  $result = $db_link->query($sql);        
  if (!$result) debug_mail(false,'error sql',$sql);
  if (!$result) die('sql error');
  if ($result->num_rows!=1) die('record not found'); 
  $row = $result->fetch_assoc();
  $my_page_title=gks_lang('Φορολογική Θέση').': '.$row['fiscal_position_descr'];
  $object_title=$row['fiscal_position_descr'];
}

stat_record();
$nav_active_array=array('manage','manage_aade','manage_aade_fiscal_position');



$lang_data_obj=gks_lang_data_obj_prepare('gks_eshop_fiscal_position','default');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    <div class="col-sm-12" style="text-align:center">
      <?php if ($id > 0) {?>
        <h3><?php echo gks_lang('Σκοπός Διακίνησης');?>: <span class="gks_object_badge_secondary">#<?php echo $id;?></span> <?php echo $object_title;?></h3>
      <?php } else { ?>
        <h3><?php echo gks_lang('Σκοπός Διακίνησης');?>: <span class="gks_object_badge_secondary">#<?php echo gks_lang('Νέος');?></span></h3>
      <?php }?>
    </div>
  </div>
</div>
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>  
          <div class="form-group row">
            <label class="col-sm-4 col-form-label form-control-sm text-sm-right" for="fiscal_position_descr"><?php echo gks_lang('Περιγραφή');?>:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control form-control-sm myneedsave" id="fiscal_position_descr"  value="<?php echo htmlspecialchars_gks($row['fiscal_position_descr']);?>">
            </div>
          </div>

          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('fiscal_position_descr'));
          ?>


          <div class="form-group row">
            <label for="fiscal_position_sortorder" class="col-sm-4 col-form-label form-control-sm text-sm-right"><?php echo gks_lang('Σειρά Ταξινόμησης');?>:</label>
            <div class="col-sm-8">
              <input id="fiscal_position_sortorder" type="number" class="form-control form-control-sm myneedsave" value="<?php echo htmlspecialchars_gks($row['fiscal_position_sortorder']);?>" min="1" strep="1">
            </div>
          </div>
                    
          <div class="form-group row">
            <label for="fiscal_position_disable" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Ενεργός');?>:</label>
            <div class="col-md-8">
              <input type="checkbox" id="fiscal_position_disable" value="1" <?php if ($row['fiscal_position_disable']==0) echo ' checked '; ?> class="switchery1_this">
            </div>
          </div> 
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>       
          ...
          <div class="row">
            ...
            <div class="col-sm-8"><span class="form-control-plaintext form-control-sm"><?php if ($row['id_fiscal_position']>0) echo $row['id_fiscal_position'];?></span></div>
          </div>
          <div class="row">
...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    ...
    datasend='';
    datasend+='&fiscal_position_descr='  +  encodeURIComponent($.base64.encode($("#mypostform #fiscal_position_descr").val().trim()));
    datasend+='&fiscal_position_sortorder='  +  encodeURIComponent($("#mypostform #fiscal_position_sortorder").val().trim());
    datasend+='&fiscal_position_disable='  +  (($('#mypostform #fiscal_position_disable').is(':checked')) ? '0':'1');
    ...
    $.ajax({
			url: '/my/admin-eshop-fiscal-position-item-exec.php?id=' + <?php echo $id;?>,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
