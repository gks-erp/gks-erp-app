...
$my_page_title=gks_lang('Αυτόματη συμπλήρωση ευκαιρίας');
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_crm_leads','autocomplete',0);
if ($perm_ret['success']==false) {$return = array('success' => false, 'message' => base64_encode($perm_ret['message']));echo json_encode($return); die();}



$sql="SELECT gks_crm_leads.*
FROM gks_crm_leads
where 
...
if (isset($_GET['notids'])) {
  $notids = trim_gks(base64_decode(rawurldecode($_GET['notids'])));
	...
	if (!($notids === null && json_last_error() !== JSON_ERROR_NONE)) {
      ...
      if (count($fix)>0) {
	      $sql.=" and id_crm_lead not in (".implode(',',$fix).") ";
	    }
	  }
	}
}
...
while ($row = $result->fetch_assoc()) {
  $fount_count++;
  $out[] = array('id' => $row['id_crm_lead'], 'value' => $row['subject']);
}
