...
$my_page_title=gks_lang('Εκτύπωση Πληρωμής').' id: '.$id;
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_acc_pay','view',$id);
...
$pay_state=trim_gks($row['pay_state']);



$save_folder='acc/pay/'.$id.'/print/';
$save_basename='PAY_'.$id.'_'.greeklish(getAccPayStateDescr($pay_state)).'_'.showDate(time(), 'Y-m-d_H.i.s',1).'.'.rand(100,999);
$print_params=array(
  'table' => 'gks_acc_pay',
  'id' => $id,
...
if ($gks_paroxos_send_pdf==1 and $preview==0) {
  ...
  $sql_paroxos_send_pdf="select id_acc_pay,company_id,company_sub_id,aade_paroxos_id,paroxos_processId,
  print_date,print_file_name,print_file_url,print_user_id,print_pay_state
  from gks_acc_pay
  where paroxos_status=1
  and aade_paroxos_id>0
  and pay_state in ('090ekdosi','100payment')
  ...
  if ($result_paroxos_send_pdf->num_rows==0) {
    ...
    gks_lang('ή το παρασταικό (xml) δεν έχει αποσταλεί στον πάροχο').'<br>'.
    gks_lang('ή έχει σταλεί ήδη η εκτύπωση').'<br>'.
    gks_lang('ή η κατάσταση δεν είναι').' <b>'.getAccPayStateDescr('090ekdosi').
    '</b>-<b>'.getAccPayStateDescr('100payment').'</b>,<br>'.
...
if ($gks_print_send_gks_erp_app==1 and $preview==0) {
  $sql_send_erp_app="SELECT gks_acc_pay.id_acc_pay, gks_acc_seires.erp_app_id, gks_acc_seires.erp_app_dest, 
  ...
  AND gks_acc_pay.pay_acc_seira_id>0
...
$ret_print = gks_print_form('gks_acc_pay',$id,$form_id,$print_params);
...
if ($ret_print['success']==false) {
  debug_mail(false,'error print',$ret_print['message']);
...
if ($preview==0) {
  $sql="update gks_acc_pay set 
    ...
    print_pay_state='".$db_link->escape_string($pay_state)."'
    where id_acc_pay=".$id;
  ...
  gks_lang('Κατάσταση').': <span class="acc_pay_state_'.$pay_state.'">'.getAccPayStateDescr($pay_state).'</span><br>'.
  ...
  $save_but_message=gks_lang('Επιτυχής δημιουργία του αρχείου εκτύπωσης').'<br>'.
  gks_lang('Τι θέλετε να κάνετε ;').'<br>'.
...
if (isset($_POST['set_def']) and $_POST['set_def']=='1') {
  ...
  $user_settings['print']['form_id_pay']=  number_format($form_id,0,'','');
...
$return = array('success' => true, 
  ...
  'redirect'=> base64_encode('admin-acc-pay-item.php?id='.$id.'&scrollto=gks_print'),
