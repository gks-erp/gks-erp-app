...
function gks_epay_api_get_transactions($mydaydif, $company_id, $in_transaction_id) {
  global $db_link;
  global $gkIP;
  ...
  if ($in_transaction_id=='') {
    ...
    if ($result->num_rows==0) {
      ...
      if ($epay_username=='') {debug_mail(false,'epay_username is not set for company '.$company_id,$sql);die('epay_username is not set for company '.$company_id);}
      if ($epay_password=='') {debug_mail(false,'epay_password is not set for company '.$company_id,$sql);die('epay_password is not set for company '.$company_id);}
      if ($epay_authorization_code=='') {debug_mail(false,'epay_authorization_code is not set for company '.$company_id,$sql);die('epay_authorization_code is not set for company '.$company_id);}
      if ($epay_x_api_key=='') {debug_mail(false,'epay_x_api_key is not set for company '.$company_id,$sql);die('epay_x_api_key is not set for company '.$company_id);}
      
      //echo '<pre>ssssss';die();
    }
  } else {
    $sql="select MID from gks_epay_transaction where Id='".$db_link->escape_string($in_transaction_id)."'";
    $result = $db_link->query($sql);        
    if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
    if ($result->num_rows==0) {
    ...
    $sql="select * from gks_company where epay_mid like '".$db_link->escape_string($MerchantId)."'";
    $result = $db_link->query($sql);        
    if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
    if ($result->num_rows==0) {
      debug_mail(false,'company not found',$sql);die('company not found');
  ...
  $headers = array(
    ...
    'X-Api-Key: '.$epay_x_api_key,
  );

  $url=GKS_EPAY_COM_API.'/api/v2.2/transactionintent/?Timestamp_min='.$today_vardia_min.'&Timestamp_max='.$today_vardia_max;
  //echo '<pre>'.$url;die();
  ...
  $url=GKS_EPAY_COM_API.'/api/v2.2/transaction/?Timestamp_min='.$today_vardia_min.'&Timestamp_max='.$today_vardia_max;;
  do {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_HTTPGET, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  ...
  foreach ($mydata as &$mytra) {
    ...
    if ($intentId!='') {
      $sql="select id_epay_transaction from gks_epay_transaction 
      where intentId='".$db_link->escape_string($intentId)."'";
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
      if ($result->num_rows>0) { 
        $row = $result->fetch_assoc();
        $mytra['id_epay_transaction']=$row['id_epay_transaction'];
      }
  ...
  foreach ($mydata as &$mytra) {
    $intentId=trim_gks($mytra['intentId']);
    $sql="select id_epay_transaction from gks_epay_transaction where ";
    ...
    if ($result->num_rows==0) { 
      ...
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
      $mytra['id_epay_transaction']=$db_link->insert_id; 
      
      if ($mytra['id_eftpos_transaction']>0) {
        $sql="update gks_eftpos_transaction set 
        xxx_transaction_id=".$mytra['id_epay_transaction'].",
      ...
      $row = $result->fetch_assoc(); 
      $id_epay_transaction=$row['id_epay_transaction'];
      $mytra['id_epay_transaction']=$id_epay_transaction;
  ...
  foreach ($mydata as &$mytra) {
    $intentId=trim_gks($mytra['intentId']);
    $id_epay_transaction=$mytra['id_epay_transaction'];
    if ($mytra['gks_transaction']>0 and $id_epay_transaction>0 and count($mytra['gks_trans'])==1) {
      ...
      //if ($id_epay_transaction==10049) die();
      ...
      $sql.=" where id_epay_transaction=".$id_epay_transaction;
      
      
      
      //if ($id_epay_transaction==10024) {echo '<pre>'.$sql;die();}
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
