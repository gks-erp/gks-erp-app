...
www.gks.gr
*/
function gks_nexi_hex_2_base64($hex) {
...
function gks_eftpos_has_transaction_status_nexi($id) {
  if ($id===null) return '';
  $id=intval($id);
  switch ($id) {
...
function gks_eftpos_has_transaction_result_nexi($id) {
  if ($id===null) return '';
  $id=intval($id);
  switch ($id) {
...
function gks_eftpos_has_transaction_type_nexi($id) {
  if ($id===null) return '';
  $id=intval($id);
  switch ($id) {
    case 0: return 'Sale';
...
function gks_eftpos_has_credentials_nexi($row_company) {
  $return=array('success' => false, 'message' => 'generic error');
  if (trim_gks($row_company['nexi_username'])=='' or trim_gks($row_company['nexi_password'])=='' or trim_gks($row_company['nexi_authorization_code'])=='' or trim_gks($row_company['nexi_x_api_key'])=='') {
    $return['message']=gks_lang('Δεν έχουν ορισθεί τα Διαπιστευτήρια πρόσβασης για την NEXI στην εταιρεία').' '.$row_company['company_title'];
    debug_mail(false,$return['message'],print_r($row_company,true));return $return;
  }
  
  return array('success' => true, 'message' => 'OK', 'data' => array(
    'nexi_username' => $row_company['nexi_username'],
...
function gks_eftpos_get_token_nexi($row_company) {
  global $db_link;
  ...
  where payment_acquirer_with_id=7
  ...
  if ($need_only_refresh) {
    $url=GKS_NEXI_COM_API.'/token/refresh/';
    $mypost=array();
    $mypost['refresh']=$pc_refresh_token_id;
  } else { 
    $url=GKS_NEXI_COM_API.'/token/';
    $mypost=array();
    $mypost['username']=$row_company['nexi_username'];
    $mypost['password']=$row_company['nexi_password'];
  }
  //echo $url;die();
  ...
  if($need_only_refresh) {
    ...
    $pc_refresh_token_expiration=date('Y-m-d H:i:s',time() + 3000); //exei parametro: expires_in:3600
  }
...
function gks_eftpos_sales_request_nexi($data) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  $return=array('success' => false, 'message' => 'generic error');
  ...
  $url=GKS_NEXI_COM_API.'/terminal/'.$data['row_asset']['nexi_id'].'/txninit/';
  //echo '<pre>'.$url;die();
  ...
  if (in_array($transaction_type,['refund','refunderp','preauthcompletion','preauthcompletionerp'])) {
    ...
    $sql="select * from gks_nexi_transaction 
    where id_nexi_transaction>0
    and myStatus=3 and myResult=1
    and id_nexi_transaction=".$xxx_transaction_id;
    $result = $db_link->query($sql);        
    if (!$result) {
    ...
    $row_prev_nexi = $result->fetch_assoc();
    
    $mypost['InitialTransaction']=$row_prev_nexi['Id'];
  }
  ...
  if ($data['seira_need_signature']) { 
    ...
    $mypost['ProviderData']=array(
      ...
      'ProviderId' => intval($data['aadeProviderId']),
      'Signature' => gks_nexi_hex_2_base64($data['aadeProviderSignature']),
    );
  }
  ...
  ".$my_wp_user_id.",".$my_wp_user_id.",now(),now(),'".$db_link->escape_string($gkIP)."',
  ".$my_wp_user_id.",'gks ERP App','nexi Api',
  ...
  if (!$result) {
    debug_mail(false,'error sql',$sql);
    return array('success' => false, 'message' => 'sql error', 'status' => 'sql error');}
  $id_nexi_transaction = $db_link->insert_id; 
  ...
  xxx_transaction_id=".$id_nexi_transaction."
  ...
  //debug_mail(false,'response nexi',$response);
  
  $gks_curl_http_code=(isset($gks_curl_info['http_code']) ? intval($gks_curl_info['http_code']) : 0);

  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/nexi_sales_request_send_'.time().'.json',$url."\n".json_encode(json_decode($mypostdata,true),JSON_PRETTY_PRINT));
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/nexi_sales_request_response_'.time().'.json','gks_curl_http_code:'.$gks_curl_http_code."\n".$response);
  ...
  if ($gks_curl_http_code!=200) {
    ...
    debug_mail(false,'nexi error',$response);return $return;}
...
function gks_eftpos_sales_request_get_status_nexi($data) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  ...
  if (in_array($transaction_type,['fullvoid','fullvoiderp'])) {
    $sql="SELECT gks_eftpos_transaction_thisisfor.my_for, gks_nexi_transaction.TransactionId_org
    ...
    LEFT JOIN gks_nexi_transaction ON gks_eftpos_transaction.xxx_transaction_id = gks_nexi_transaction.id_nexi_transaction
    ...
    AND gks_nexi_transaction.TransactionId_org<>''";
    $result = $db_link->query($sql);        
    if (!$result) {
      debug_mail(false,'error sql',$sql);
      return array('success' => false, 'message' => 'sql error', 'status' => 'sql error');}
    ...
    $url=GKS_NEXI_COM_API.'/transactionintent/?TransactionId='.rawurlencode($prev_TransactionId_org);
  } else {
    $url=GKS_NEXI_COM_API.'/transactionintent/?CustomerReference='.$data['sessionId'];
  }
  
//  $url=GKS_NEXI_COM_API.'/transactionintent/'.
//  '?TID='.
...
//  date('Y-m-d\TH:i:s\Z', strtotime($data['row_tra']['mydate_add'])-10*60); //2024-07-01T15:19:48Z
  ...
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/nexi_status_send_s1_'.time().'.json',$url);
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/nexi_status_response_s1_'.time().'.json','gks_curl_http_code:'.$gks_curl_http_code."\n".$response);
  ...
  if ($remote_id!='') {
    foreach ($response_array['results'] as $res_val) {
      if ((isset($res_val['Id']) and trim_gks($res_val['Id'])==$remote_id) or 
          (isset($res_val['id']) and trim_gks($res_val['id'])==$remote_id)) {
        $myresult=$res_val;
        break;
      }
    } 
  ...
  if (isset($myresult['Id'])==false and isset($myresult['id'])==true) {
    $myresult['Id']=$myresult['id'];
  }
...
*/
  ...
  if (in_array($r_Result,[2,3,4,5,6,7])) {//ola ektos APPROVED
    ...
    if (in_array($transaction_type,['sale','saleerp'])) {  
      ...
      WHERE gks_eftpos_transaction.payment_acquirer_with_id=7
  ...
  $url=GKS_NEXI_COM_API.'/transaction/'.
  '?TID='.
  $data['row_tra']['terminalId'].
  '&Timestamp_min='.
  date('Y-m-d\TH:i:s\Z', strtotime($data['row_tra']['mydate_add'])-10*60); //2024-07-01T15:19:48Z
  ...
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/nexi_status_send_s2_'.time().'.json',$url);
  if (GKS_DEBUG) file_put_contents(GKS_SITE_PATH.'tmp/nexi_status_response_s2_'.time().'.json','gks_curl_http_code:'.$gks_curl_http_code."\n".$response);
  ...
  if ($gks_curl_http_code==200) {
    ...
    if (is_array($response_array) and 
      ...
      foreach ($response_array['results'] as $index => $resitem) {
        if (isset($resitem['Id'])==false and isset($resitem['id'])) {
...
//          }
          ...
          $company_id=0;
          $nexi_mid=''; if (isset($resitem['MID'])) $nexi_mid=trim_gks($resitem['MID']);
          if ($nexi_mid!='') {
            $sql="select id_company from gks_company where company_disable=0 and nexi_mid='".$db_link->escape_string($nexi_mid)."'";
...
function gks_eftpos_get_transaction_extra_html_nexi($input) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  ...
  if ($result->num_rows==1) {
    ...
    if (!empty($row['Approved']) and $row['Approved']==1) {
      $html[]='Approved: <span class="gks_nexi_transaction_approved">Approved</span>';
    } else {
      $html[]='Approved: <span class="gks_nexi_transaction_not_approved">Not Approved</span>';
    }
    if (!empty($row['Voided']) and $row['Voided']==1) {
      $html[]='Voided: <span class="gks_nexi_transaction_voided">Voided</span>';
    } 
    
    
    //if (!empty($row['TxnType'])) {
      $html[]=gks_lang('Τύπος').': <b>'. gks_eftpos_has_transaction_type_nexi($row['TxnType']).'</b>';
    //} 
    ...
    $html[]='<a href="admin-nexi-transaction-raw.php?id='.$input['id_eftpos_transaction'].'" target="_blank" title="Raw Data"><i class="fas fa-database gks_payment_rawdata"></i></a>'.
...
function gks_eftpos_fullvoid_request_nexi($data) {
  global $db_link;
  global $my_wp_user_id;
  global $gkIP;
  $return=array('success' => false, 'message' => 'generic error');
  ...
  $url=GKS_NEXI_COM_API.'/terminal/'.$data['row_asset']['nexi_id'].'/txnvoid/';
  //echo '<pre>'.$url;die();
