...
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_orders_pivot2','view',0);
...
$gks_custom_prepare = gks_custom_table_item_prepare('gks_orders',['from'=>'pivot']);
$gks_custom_header_fields = gks_custom_table_pivot_header($gks_custom_prepare);

//echo '<pre>'; print_r($gks_custom_header_fields);die();
//print $sql;
//die();
...
if (isset($_gks_session['temp']['wherepivot2']) and $_gks_session['temp']['wherepivot2']!='') {
  $sql=$_gks_session['temp']['wherepivot2'];
} else {
  die();  
}
...
$file_path='pivot2_'.$my_wp_user_id.'_'.rand(100000,999999).'.csv';
...
'"'.gks_lang('Επαφή').'","'.gks_lang('Πόλη').'","'.gks_lang('Νομός').'","'.gks_lang('Χώρα').'","'.gks_lang('Παραγγελία').'",'.
'"'.gks_lang('Τρόπος Αποστολής').'","'.gks_lang('Τρόπος Πληρωμής').'",'.
'"'.gks_lang('Αξία','part2').'","'.gks_lang('Ανάθεση').'",';
