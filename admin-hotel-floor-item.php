...
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_hotel_floor',($id==-1 ? 'add' : 'view'),$id);
...
$gks_custom_prepare = gks_custom_table_item_prepare('gks_hotel_floor',['from'=>'item']);
...
if ($id==-1) {
  ...
  $my_page_title=gks_lang('Νέος όροφος');
} else {

  $sql ="SELECT gks_hotel_floor.*,
  ...
  $my_page_title=gks_lang('Όροφος').': '.$row['floor_descr'];
  $object_title=$row['floor_descr'];
}
...
$lang_data_obj=gks_lang_data_obj_prepare('gks_hotel_floor','default');
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    <div class="col-md-12" style="text-align:center">
      <?php if ($id > 0) {?>
        <h3><?php echo gks_lang('Όροφος');?>: <span class="gks_object_badge_secondary">#<?php echo $id;?></span> <?php echo $object_title;?></h3>
      <?php } else { ?>
        <h3><?php echo gks_lang('Όροφος');?>: <span class="gks_object_badge_secondary">#<?php echo gks_lang('Νέος');?></span></h3>
      <?php }?>
    </div>
  </div>
</div>
    ...
    <div class="col-md-6">
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('bas');?>>         
          ...
          <div class="form-group row">
            <label for="floor_descr" class="col-md-4 col-form-label form-control-sm text-md-right"><?php echo gks_lang('Όροφος');?>:</label>
            <div class="col-md-8">
              <input id="floor_descr" type="text" class="form-control form-control-sm" value="<?php echo htmlspecialchars_gks($row['floor_descr']);?>">
            </div>
          </div>
          <?php 
          echo gks_lang_data_obj_render_html($lang_data_obj,$row,array('floor_descr'));
...
<div class="container-fluid" id="gks_rsrv_f">
  <div class="form-group1 row">
    <div class="col-md-12 text-center mt-2">
      ...
      <button type="button" class="btn btn-danger deleterowbtn" data-id="<?php echo $row['id_hotel_floor'];?>" data-model="gks_hotel_floor" data-backurl="admin-hotel-floor.php"><?php echo gks_lang('Διαγραφή');?></button>
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-6">
      <?php 
      echo getObjectRels('gks_hotel_floor',$id);
      echo getActivityObjectTable('gks_hotel_floor',$id);
      $obj_fileslist= gks_FilesObjectList(array('objname'=>'gks_hotel_floor','id'=>$id));
      ...
      <div class="card gks_card_expand">
        ...
        <div class="card-body" <?php echo gks_card_body('kat');?>>       
          ...
          <div class="row">
            ...
            <div class="col-sm-8"><span class="form-control-plaintext form-control-sm"><?php if ($row['id_hotel_floor']>0) echo $row['id_hotel_floor'];?></span></div>
          </div>
          <div class="row">
...
var from_php_dialog_object_rel_curr='gks_hotel_floor';
var from_php_activity_model='gks_hotel_floor';
...
var from_php_perm_ret_edit  =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_hotel_floor','edit',  $id);?>;
var from_php_perm_ret_add   =<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_hotel_floor','add',   $id);?>;
var from_php_perm_ret_delete=<?php echo gks_permission_user_can_action_javascript($my_wp_user_id, 'gks_hotel_floor','delete',$id);?>;
...
jQuery(document).ready(function($) {
  ...
  function mysubmit() {
    ...
    datasend+='&floor_descr='  + encodeURIComponent($.base64.encode($("#mypostform #floor_descr").val().trim()));
    ...
    $.ajax({
			url: '/my/admin-hotel-floor-item-exec.php?id=' + <?php echo $id;?>,
			type: 'POST',
			cache: false,
			dataType: 'json',
			data: datasend,
