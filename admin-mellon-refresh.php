...
//include_once('functions_mellon.php');

//echo '<pre>';
// /my/admin-mellon-refresh.php?today=1
// /my/admin-mellon-refresh.php?days=0

if (isset($my_wp_user_id)==false) $my_wp_user_id=2;
if (isset($gkIP)==false) $gkIP='127.0.0.1';


$my_page_title=gks_lang('Mellon Refresh'); 
db_open();
stat_record();

$sql="SELECT id_company
FROM gks_company
WHERE mellon_username<>'' AND mellon_password<>'' AND mellon_authorization_code<>'' and mellon_x_api_key<>'' AND company_disable=0
...
if (isset($_GET['today'])) {
  ...
  foreach ($id_company_ids as $id_company) {
    for($i=0;$i<=1;$i++) {
      gks_mellon_api_get_transactions($i,$id_company,'');
