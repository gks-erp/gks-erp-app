...
$my_page_title=gks_lang('Είδη Παραστατικών');
...
$sortable = array(
		array('name' => 'soid', 'field' => 'id_acc_eidos_parastatikou'),
		array('name' => 'socode', 'field' => 'eidos_parastatikou_aade_code'),
		array('name' => 'sodescr', 'field' => 'eidos_parastatikou_descr'),
		array('name' => 'sosort', 'field' => 'sortorder'),
		array('name' => 'sodisable', 'field' => 'aade_disable'),
		array('name' => 'sopeppol', 'field' => 'peppol_code'),   
);



$search_fields = array(
'eidos_parastatikou_descr',
'peppol_code',
);
...
$sql = "SELECT SQL_CALC_FOUND_ROWS gks_acc_eidi_parastatikon.*,
...
if (empty($sorted['sql'])) {
	$sql .= " ORDER BY gks_acc_eidi_parastatikon.sortorder,fullpath";
} else {
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-aade-eidi-parastatikon-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέου είδους παραστατικών');?></a>
...
<table class="table table-sm table-responsive1 table-striped table-bordered gkstable <?php
  ...
  ?>" border="0" cellspacing="0" cellpadding="5" align="center" id="table_gks_acc_eidi_parastatikon">
<thead>
  <tr >	
    ...
    <th class="table-dark" scope="col" style="text-align: left   !important;" width="70%"><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sodescr', gks_lang('Περιγραφή')); ?></th>        
    <th class="table-dark" scope="col" style="text-align: left   !important;" width="0%"><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'socode', gks_lang('Κωδικός ΑΑΔΕ')); ?></th>        
    ...
    <th class="table-dark" scope="col" style="text-align: left   !important;" width="30%"><?php echo makeSortLink($sortable, $sortable_url, $_GET, 'sopeppol', gks_lang('Peppol')); ?></th>        
...
<tbody>
    ...
    $mypad='<div class="gks_pad1">'; 
    if (!empty($row['gt2'])) $mypad='<div class="gks_pad2">';
    if (!empty($row['gt3'])) $mypad='<div class="gks_pad3">';
    //$mypad='';
    
    if ($row['is_selectable']==0) {
...
?>
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>" data-id="<?php echo $row['id_acc_eidos_parastatikou'];?>">
    <th class="mytdcm" scope="row" nowrap><?php echo ($i + $showFrom);?></th>
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-aade-eidi-parastatikon-item.php?id=<?php echo $row['id_acc_eidos_parastatikou'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_acc_eidos_parastatikou'];?></td>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_acc_eidos_parastatikou'];?>" data-model="gks_acc_eidi_parastatikon"></i></td>
        </tr>      
      </table>
    </td> 
    <td class="mytdcml"><?php echo $mypad.$row['eidos_parastatikou_descr'].'</div>';?></td>
    <td class="mytdcm"><?php echo $row['eidos_parastatikou_aade_code'];?></td>
<?php if ($perm_edit) {?>
...
<?php } ?>
    ...
    <td class="mytdcml div_peppol" data-id="<?php echo $row['id_acc_eidos_parastatikou'];?>"><?php echo $row['peppol_code'];?></td>
  </tr>
<?php    
    }
?>
  ...
  <div class="row align-items-center">
    <div class="col-sm-12" style="text-align:center">
      <p><?php echo gks_lang('Για τους κωδικούς Peppol δείτε τον παρακάτω σύνδεσμο');?>:<br>
        <a href="https://docs.peppol.eu/poacc/billing/3.0/codelist/UNCL1001-inv/" target="_blank"
          >https://docs.peppol.eu/poacc/billing/3.0/codelist/UNCL1001-inv/</a>
      </p>
    </div>
  </div>
</div>
...
jQuery(document).ready(function($) {
  ...
  $('#table_gks_acc_eidi_parastatikon > tbody').sortable({
    handle: '.sortorder_handle',
    update: function( event, ui ) {
      mylist = $(this).sortable('toArray', {attribute: 'data-id'});
      gks_sortorder_obj('gks_acc_eidi_parastatikon',mylist,'#table_gks_acc_eidi_parastatikon > tbody');
    }
  }); 
  <?php if ($perm_edit) {?>
  ...
  function save_peppol_click() {
    ...
    gks_field_from_table_save('gks_acc_eidi_parastatikon',myid,'peppol_code',mytext, false);
  }
  window.gks_field_from_table_save_after=function(mydata) {
    if (mydata.table_name!='gks_acc_eidi_parastatikon') return;
    if (mydata.table_id<=0) return;
    if (mydata.field_name!='peppol_code') return;
