...
function gks_mellon_api_get_transactions($mydaydif, $company_id, $in_transaction_id) {
  global $db_link;
  global $gkIP;
  ...
  if ($in_transaction_id=='') {
    ...
    if ($result->num_rows==0) {
      ...
      if ($mellon_username=='') {debug_mail(false,'mellon_username is not set for company '.$company_id,$sql);die('mellon_username is not set for company '.$company_id);}
      if ($mellon_password=='') {debug_mail(false,'mellon_password is not set for company '.$company_id,$sql);die('mellon_password is not set for company '.$company_id);}
      if ($mellon_authorization_code=='') {debug_mail(false,'mellon_authorization_code is not set for company '.$company_id,$sql);die('mellon_authorization_code is not set for company '.$company_id);}
      if ($mellon_x_api_key=='') {debug_mail(false,'mellon_x_api_key is not set for company '.$company_id,$sql);die('mellon_x_api_key is not set for company '.$company_id);}
      
      //echo '<pre>ssssss';die();
    }
  } else {
    $sql="select MID from gks_mellon_transaction where Id='".$db_link->escape_string($in_transaction_id)."'";
    $result = $db_link->query($sql);        
    if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
    if ($result->num_rows==0) {
    ...
    $sql="select * from gks_company where mellon_mid like '".$db_link->escape_string($MerchantId)."'";
    $result = $db_link->query($sql);        
    if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
    if ($result->num_rows==0) {
      debug_mail(false,'company not found',$sql);die('company not found');
  ...
  $headers = array(
    ...
    'X-Api-Key: '.$mellon_x_api_key,
  );

  $url=GKS_MELLONGROUP_COM_API.'/api/v2.1/transactionintent/?Timestamp_min='.$today_vardia_min.'&Timestamp_max='.$today_vardia_max;
  //echo '<pre>'.$url;die();
  ...
  $url=GKS_MELLONGROUP_COM_API.'/api/v2.1/transaction/?Timestamp_min='.$today_vardia_min.'&Timestamp_max='.$today_vardia_max;;
  do {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_HTTPGET, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  ...
  foreach ($mydata as &$mytra) {
    ...
    if ($intentId!='') {
      $sql="select id_mellon_transaction from gks_mellon_transaction 
      where intentId='".$db_link->escape_string($intentId)."'";
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
      if ($result->num_rows>0) { 
        $row = $result->fetch_assoc();
        $mytra['id_mellon_transaction']=$row['id_mellon_transaction'];
      }
  ...
  foreach ($mydata as &$mytra) {
    $intentId=trim_gks($mytra['intentId']);
    $sql="select id_mellon_transaction from gks_mellon_transaction where ";
    ...
    if ($result->num_rows==0) { 
      ...
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
      $mytra['id_mellon_transaction']=$db_link->insert_id; 
      
      if ($mytra['id_eftpos_transaction']>0) {
        $sql="update gks_eftpos_transaction set 
        xxx_transaction_id=".$mytra['id_mellon_transaction'].",
      ...
      $row = $result->fetch_assoc(); 
      $id_mellon_transaction=$row['id_mellon_transaction'];
      $mytra['id_mellon_transaction']=$id_mellon_transaction;
  ...
  foreach ($mydata as &$mytra) {
    $intentId=trim_gks($mytra['intentId']);
    $id_mellon_transaction=$mytra['id_mellon_transaction'];
    if ($mytra['gks_transaction']>0 and $id_mellon_transaction>0 and count($mytra['gks_trans'])==1) {
      ...
      //if ($id_mellon_transaction==10049) die();
      ...
      $sql.=" where id_mellon_transaction=".$id_mellon_transaction;
      
      
      
      //if ($id_mellon_transaction==10024) {echo '<pre>'.$sql;die();}
      $result = $db_link->query($sql);        
      if (!$result) {debug_mail(false,'error sql',$sql);die('sql error');}
