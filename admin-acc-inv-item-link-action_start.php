...
if ($id<=0) {
  debug_mail(false,'admin-acc-inv-item-link-action_start.php the id is not set','');
  die();}


$my_page_title=gks_lang('Έναρξη λήψης αρχείου παραστατικού').' '.$id;
db_open();
stat_record();
...
$sql="select * from gks_acc_inv_links where id_acc_inv_links=".$id;
...
if ($result->num_rows <= 0) {  
  debug_mail(false,'admin-acc-inv-item-link-action_start.php record not found',$sql);
  die();}

$row=$result->fetch_assoc();
$acc_inv_id=$row['acc_inv_id'];
...
if ($row['download_status']!=0) {
  if ($row['download_status']!=2) { // einai idi OK
    $sql="update gks_acc_inv_links set download_end=now(), 
    download_message='".$db_link->escape_string(gks_lang('Η κατάσταση του συνδέσμου δεν είναι η σωστή'))."' 
    where id_acc_inv_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();} 
  } 
  debug_mail(false,'admin-acc-inv-item-link-action_start.php download_status='.$row['download_status'].' not 0', $sql);
  die();}
...
if ($url=='' or filter_var($url, FILTER_VALIDATE_URL) === FALSE) {
  ...
  $result = $db_link->query($sql);
  if (!$result) {debug_mail(false,'error sql',$sql);die();}  
  debug_mail(false,'admin-acc-inv-item-link-action_start.php url is valid',$url);
  die();} 


$sql="update gks_acc_inv_links set 
download_status=1, 
download_start=now(),
download_end=null, 
...
if (file_exists(GKS_FileServerShare) == false) {
  $sql="update gks_acc_inv_links set html_tds=null,download_end=now(), 
...
$upload_dir = GKS_FileServerShare.'acc/inv/'.$acc_inv_id.'/';

if (file_exists($upload_dir) == false) {
  if (@mkdir($upload_dir , 0777, true) == false ) {
    $sql="update gks_acc_inv_links set html_tds=null,download_end=now(), 
    download_message='". $db_link->escape_string(gks_lang('Δεν μπορεί να δημιουργηθεί ο φάκελος').' ../@acc/inv/'.$acc_inv_id)."' 
    where id_acc_inv_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();}  
    debug_mail(false,'admin-acc-inv-item-link-action_start.php can not create dir: ',$upload_dir);
    die(); 
  }
}
...
if (startwith(strtolower($url),'https://mega.nz/') and strlen($url)>=17) {
  ...
  if (@mkdir($upload_dir2 , 0777, true) == false ) {
    ...
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();}  
    debug_mail(false,'admin-acc-inv-item-link-action_start.php can not create dir: ',$upload_dir2);
    die(); 
  }
...
//  debug_mail(false,'exec4',print_r($return_var2,true));
  ...
  if (copy (GKS_SITE_PATH.GKS_SITE_HTTPDOCS.'/my/utils/transferwee.py', $upload_dir2.'transferwee.py') == false) {
    $sql="update gks_acc_inv_links set html_tds=null,download_end=now(), 
    download_message='". $db_link->escape_string('Δεν μπορεί να αντιγραφεί το αρχείο /my/utils/transferwee.py '.gks_lang('στον φάκελο').' '.$upload_dir2)."' 
    where id_acc_inv_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();}  
    debug_mail(false,'admin-acc-inv-item-link-action_start.php can not copy file transferwee.py in dir: ',$upload_dir2);
    die();    
  }
  $relative_path=substr($upload_dir2, strlen($upload_dir));
  $sql="update gks_acc_inv_links set 
...
function progressCallback($resource, $download_size, $downloaded_size, $upload_size, $uploaded_size ) {
  ...
  if ( $progress > $previousProgress) {
    ...
    $sql="select download_status from gks_acc_inv_links where id_acc_inv_links=".$id;
    $result = $db_link->query($sql);
    if (!$result) {debug_mail(false,'error sql',$sql);die();} 
    if ($result->num_rows==0) {
