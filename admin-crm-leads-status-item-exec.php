...
$my_page_title=gks_lang('Αποθήκευση Κατάσταση Ευκαιριών').': '.$id;
db_open();
stat_record();
$perm_ret=gks_permission_user_can_action($my_wp_user_id, 'gks_crm_leads_status',($id==-1 ? 'add':'edit'),$id);
...
if ($id>0) {
  $sql ="SELECT * FROM gks_crm_leads_status where id_crm_lead_status = ".$id;
...
if ($result->num_rows>=1) {
  $row = $result->fetch_assoc();
  $message=gks_lang('Η κατάσταση με περιγραφή <b>[1]</b> υπάρχει ήδη:<br><a href="admin-crm-leads-status-item.php?id=[2]" class="gks_link">Προβολή</a>');
  $message=str_replace('[1]',$lead_status_descr,$message);
  $message=str_replace('[2]',$row['id_crm_lead_status'],$message);
...
$redirect='';
if ($id==-1) {
  $sql="insert into gks_crm_leads_status (mydate_add,user_id_add,myip) values (now(),".$my_wp_user_id.",'".$db_link->escape_string($gkIP)."');";
  $result = $db_link->query($sql);  
  if (!$result) {
    debug_mail(false,'error sql',$sql);
    $return = array('success' => false, 'message' => base64_encode('sql error'));
  ...
  $redirect=base64_encode('admin-crm-leads-status-item.php?id='.$id); 
}
...
where id_crm_lead_status = ".$id." limit 1";
$result = $db_link->query($sql);  
if (!$result) {
  debug_mail(false,'error sql',$sql);
  $return = array('success' => false, 'message' => base64_encode('sql error'));
