...
$filters[] = array(
  ...
  'vals' => array(
      ...
      array('value' => 1, 'text' => gks_lang('Ενεργό'),     'sql' => "fiscal_position_disable = 0"),
      array('value' => 2, 'text' => gks_lang('Μη ενεργό'),  'sql' => "fiscal_position_disable <> 0"),
  ),
);
...
$sortable = array(
		array('name' => 'soid', 'field' => 'id_fiscal_position'),
		array('name' => 'sodescr', 'field' => 'fiscal_position_descr'),
		array('name' => 'sosort', 'field' => 'fiscal_position_sortorder'),
		array('name' => 'sodisable', 'field' => 'fiscal_position_disable'),
);



$search_fields = array(
'fiscal_position_descr',
);
...
if (empty($sorted['sql'])) {
	$sql .= " ORDER BY fiscal_position_sortorder,fiscal_position_descr,id_fiscal_position";
} else {
...
<div class="container-fluid gksitemheader">
  <div class="row align-items-center">
    ...
    <div class="col-sm-6" style="text-align:center">
      <a class="btn btn-primary gks_add_new_record" href="admin-eshop-fiscal-position-item.php?id=-1"><?php echo gks_lang('Προσθήκη νέας φορολογικής θέσης');?></a>
...
<table class="table table-sm table-responsive1 table-striped table-bordered gkstable <?php
  ...
  ?>" border="0" cellspacing="0" cellpadding="5" align="center" id="table_gks_eshop_fiscal_position">
<thead>
  <tr >	
...
<tbody>
	...
	$i++;
?>
  <tr class="<?php echo ($i % 2 == 0) ? 'even' : 'odd'; ?>" data-id="<?php echo $row['id_fiscal_position'];?>">
    <th class="mytdcm" scope="row" nowrap><?php echo ($i + $showFrom);?></th>
    <td nowrap class="mytdcm p-0">
      <table class="tableids3col">
        <tr>
          <td><a href="admin-eshop-fiscal-position-item.php?id=<?php echo $row['id_fiscal_position'];?>"><i class="enterrow fas fa-pen" title="<?php echo gks_lang('Προβολή');?>"></i></a></td>
          <td><?php echo $row['id_fiscal_position'];?></td>
          <td><i class="fas fa-trash-alt deleterow perm_delete" data-id="<?php echo $row['id_fiscal_position'];?>" data-model="gks_eshop_fiscal_position"></i></td>
        </tr>      
      </table>
    </td>
...
<?php if ($perm_edit) {?>
    <td nowrap class="mytdcm sortorder_handle" title="<?php echo $row['fiscal_position_sortorder'];?>">
      <i class="fas fa-arrows-alt-v"></i>
      <span><?php echo $row['fiscal_position_sortorder'];?></span>
    </td>
<?php } ?>
    <td class="mytdcm"><?php echo myimg010r($row['fiscal_position_disable']);?></td>
...
jQuery(document).ready(function($) {
  ...
  $('#table_gks_eshop_fiscal_position > tbody').sortable({
    handle: '.sortorder_handle',
    update: function( event, ui ) {
      mylist = $(this).sortable('toArray', {attribute: 'data-id'});
      gks_sortorder_obj('gks_eshop_fiscal_position',mylist,'#table_gks_eshop_fiscal_position > tbody');
    }
  }); 
