...
$sql="select * from gks_whi_mov_links where whi_mov_id=".$id." and (download_status=1 or (download_status in (2,3) and download_end > date_sub(now(), interval 1 minute)))";
...
while ($row = $result->fetch_assoc()) {
  ...
  if ($download_size_until_now==0 and $relative_path!= '') {
    $path_scan=GKS_FileServerShare.'whi/mov/'.$id.'/'.$relative_path;
  ...
  $data[] =array(
    'id' => intval($row['id_whi_mov_links']),
