...
$my_page_title=gks_lang('Mellon Log');
...
if ($id>0) {
  $sql="select id_eftpos_transaction,xxx_transaction_id,send_array,response_array from gks_eftpos_transaction where id_eftpos_transaction=".$id." and payment_acquirer_with_id=3";
  ...
  $sql="select id_eftpos_transaction,xxx_transaction_id,send_array,response_array from gks_eftpos_transaction where xxx_transaction_id=".$mtid." and payment_acquirer_with_id=3";
  $result = $db_link->query($sql);
  if (!$result) { debug_mail(false,'error sql',$sql);die('sql error');}  
  if ($result->num_rows==0) {
...
if ($xxx_transaction_id>0) {
  $sql="select myjson from gks_mellon_transaction where id_mellon_transaction=".$xxx_transaction_id;
  $result = $db_link->query($sql);
  if (!$result) { debug_mail(false,'error sql',$sql);die('sql error');}  
  if ($result->num_rows==0) {
