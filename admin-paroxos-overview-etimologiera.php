...
$curr_paroxos_id=21;
//admin-paroxos-overview-etimologiera.php
...
$my_page_title=gks_lang('Επισκόπηση Παρόχου etimologiera');
$nav_active_array=array('accounting','accounting_paroxos_overview','accounting_paroxos_overview_etimologiera');
...
while ($row = $result->fetch_assoc()) {
  ...
  if (isset($data[$cid]['subs'][$sid])) {
    ...
    $data[$cid]['subs'][$sid]['d']['password']=$row['pc_password'];    
...
<?php } ?>              
              ...
              <div class="form-group row" id="paroxos_get_invoice_pending_buttons" style="display:none;">
                <div class="col-md-12 text-center">
                  ...
                  <a id="paroxos_get_invoice_pending_button2" href="admin-paroxos-overview-etimologiera-invoice-pending.php" class="btn btn-primary btn-sm"><?php echo gks_lang('Προβολή Λίστας');?></a>
                </div>
              </div> 
...
<?php } ?>              
              ...
              <div class="form-group row">
                <div class="col-md-12 text-center">
                  ...
                  <div style="margin:10px;">
                    <a href="admin-paroxos-overview-etimologiera-tf1-keys.php" class="btn btn-sm btn-primary"><i class="fas fa-eye"></i> <?php echo gks_lang('Προβολή λίστας κλειδιών');?></a>
                  </div>
                </div>
              </div>              
            </div>
        ...
        <div class="card-body"<?php echo gks_card_body('c'.$comp['id']);?>> 
          <div class="row">
            <div class="col-md-12">
              <div class="card gks_card_expand">
                <div class="card-header" style="text-align:center">
                  <?php echo gks_lang('Υπόλοιπο μονάδων')?>
                </div>
                <div class="card-body"<?php echo gks_card_body('u'.$comp['id']);?>>    
                  <div class="row">
                   
<?php
  if (isset($comp['subs'][0]['id_sub']) and $comp['subs'][0]['id_sub']==0 and //kentriko
      isset($comp['subs'][0]['d']['username']) and
      isset($comp['subs'][0]['d']['password']) and
      $comp['subs'][0]['d']['username']!='' AND
      $comp['subs'][0]['d']['password']!='') {
    
    $sub_url='/getCreditInfo';
    if ($comp['subs'][0]['d']['live']) {
    ...
    $headers[]='Authorization: Basic '.base64_encode($comp['subs'][0]['d']['username'].':'.$comp['subs'][0]['d']['password']);  
    ...
    //delete me
    /* $result1='{
"userRequestID": "a2caefc4‐7342‐4053‐ad8e‐ef7586635474",
"version": "4.0.3",
"response": {
  "vat": "979708945",
  "periodStart": "2024-10-04T00:00:00Z",
  "periodEnd": "2024-10-04T00:00:00Z",
  "retailCredits": 10000,
  "wholesaleCredits": 5000,
  "b2GCredits": 2500,
  "consumedTotal": 5624,
  "consumedRetail": 1972,
  "consumedWholesale": 1440,
  "consumedB2G": 193
}
}'; */

    
    if ($gks_curl_http_code==200 and trim($result)!='') {   
      $response_array = json_decode($result, true);
      if (isset($response_array['response']['paroxosError'])) {
        $temp=[];
        if (isset($response_array['response']['paroxosError']['code']))
          $temp[]='Code: '.$response_array['response']['paroxosError']['code'];
        if (isset($response_array['response']['paroxosError']['description']))
          $temp[]='Description: '.$response_array['response']['paroxosError']['description'];
        
        echo '<div class="col-md-12"><div class="alert alert-danger" role="alert">'.
        implode('<br>',$temp).
        '</div></div>';
      } else if (isset($response_array['response']['vat'])) {
?>        
        <div class="col-md-4"><?php 
          echo '<div><b>'.gks_lang('Περίοδος').'</b></div>'.
          '<div class="mytext">'.
          gks_lang('Από').': ';
          if (isset($response_array['response']['periodStart']))
            echo date('d/m/Y H:i:s',strtotime($response_array['response']['periodStart'])).'<br>';
          echo gks_lang('Έως').': ';
          if (isset($response_array['response']['periodEnd']))
             echo date('d/m/Y H:i:s',strtotime($response_array['response']['periodEnd']));
          echo '</div>';
          ?>
        </div>
        <div class="col-md-8">
          <div><b><?php echo gks_lang('Χρήση')?></b></div>
          <?php 
          $retailCredits=0;if (isset($response_array['response']['retailCredits'])) $retailCredits=intval($response_array['response']['retailCredits']);
          $wholesaleCredits=0;if (isset($response_array['response']['wholesaleCredits'])) $wholesaleCredits=intval($response_array['response']['wholesaleCredits']);
          $b2GCredits=0;if (isset($response_array['response']['b2GCredits'])) $b2GCredits=intval($response_array['response']['b2GCredits']);
          $consumedTotal=0;if (isset($response_array['response']['consumedTotal'])) $consumedTotal=intval($response_array['response']['consumedTotal']);
          $consumedRetail=0;if (isset($response_array['response']['consumedRetail'])) $consumedRetail=intval($response_array['response']['consumedRetail']);
          $consumedWholesale=0;if (isset($response_array['response']['consumedWholesale'])) $consumedWholesale=intval($response_array['response']['consumedWholesale']);
          $consumedB2G=0;if (isset($response_array['response']['consumedB2G'])) $consumedB2G=intval($response_array['response']['consumedB2G']);
          
          $totalCredits=$retailCredits+$wholesaleCredits+$b2GCredits;
          
          ?>
          <style>
            .gsk_margin_progress {margin: 0px 0px 10px 0px;height:30px;}
          </style>

          <div class="mytext">
            <?php echo gks_lang('Σύνολο').': '.$consumedTotal.'/'.$totalCredits;
            $pososto=0;
            if ($totalCredits<>0) $pososto=100*$consumedTotal/$totalCredits;
            ?>
          </div>
          <div class="progress gsk_margin_progress">
            <div class="progress-bar progress-bar-striped" role="progressbar" style="width: <?php echo $pososto;?>%" aria-valuenow="<?php echo $pososto;?>" aria-valuemin="0" aria-valuemax="100"><?php echo number_format($pososto,1,',','');?>%</div>
          </div>

          <div class="mytext">
            <?php echo gks_lang('Λιανική').': '.$consumedRetail.'/'.$retailCredits;
            $pososto=0;
            if ($retailCredits<>0) $pososto=100*$consumedRetail/$retailCredits;
            ?>
          </div>
          <div class="progress gsk_margin_progress">
            <div class="progress-bar progress-bar-striped bg-success" role="progressbar" style="width:<?php echo $pososto;?>%" aria-valuenow="<?php echo $pososto;?>" aria-valuemin="0" aria-valuemax="100"><?php echo number_format($pososto,1,',','');?>%</div>
          </div>

         <div class="mytext">
            <?php echo gks_lang('Χονδρική').': '.$consumedWholesale.'/'.$wholesaleCredits;
            $pososto=0;
            if ($wholesaleCredits<>0) $pososto=100*$consumedWholesale/$wholesaleCredits;
            ?>
          </div>
          <div class="progress gsk_margin_progress">
            <div class="progress-bar progress-bar-striped bg-info" role="progressbar" style="width: <?php echo $pososto;?>%" aria-valuenow="<?php echo $pososto;?>" aria-valuemin="0" aria-valuemax="100"><?php echo number_format($pososto,1,',','');?>%</div>
          </div>

         <div class="mytext">
            <?php echo gks_lang('B2G').': '.$consumedB2G.'/'.$b2GCredits;
            $pososto=0;
            if ($b2GCredits<>0) $pososto=100*$consumedB2G/$b2GCredits;
            ?>
          </div>
          <div class="progress gsk_margin_progress">
            <div class="progress-bar progress-bar-striped bg-warning" role="progressbar" style="width: <?php echo $pososto;?>%" aria-valuenow="<?php echo $pososto;?>" aria-valuemin="0" aria-valuemax="100"><?php echo number_format($pososto,1,',','');?>%</div>
          </div>
...
jQuery(document).ready(function($) {
  ...
  $('#paroxos_get_invoice_pending').click(function() {
    ...
    $.ajax({
  		url: '/my/admin-paroxos-overview-etimologiera-exec.php',
  		type: 'POST',
  		cache: false,
  		dataType: 'json',
  		data: datasend,
...
<?php echo gks_lang_big_texts('admin-paroxos-overview-etimologiera');?>
