...
$my_page_title=gks_lang('Λήψη κουπονιών από WooCommerce');
db_open();
stat_record();
...
if (count($pids)==0) {
  $return = array('success' => false, 'message' => base64_encode(gks_lang('Δεν βρέθηκαν κουπόνια')));
  echo json_encode($return); die(); 
}
...
foreach ($pids as $value) {
  ...
  '".$db_link->escape_string($guid)."','woo','pending','get_coupon','".$db_link->escape_string($id)."','".$db_link->escape_string($value)."'
  )";
